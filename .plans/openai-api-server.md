# OpenAI-Compatible API Server for Sinoclaw Agent
...
SSE streaming. By exposing this endpoint, sinoclaw-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  sinoclaw-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.sinoclaw/.env`
2. Run `sinoclaw gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with sinoclaw-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns sinoclaw-agent as a model) |
...
But sinoclaw-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "sinoclaw-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured sinoclaw-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of sinoclaw-agent's core prompt
...
Once implemented, sinoclaw-agent works as a drop-in backend for:
