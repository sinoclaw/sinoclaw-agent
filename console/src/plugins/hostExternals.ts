 ...
 * `window.Sinoclaw` so plugin bundles can register routes and tool renderers
...
/** Shared host dependencies exposed to plugin bundles via `window.Sinoclaw.host`. */
...
export function installHostExternals(): void {
  ...
  if (!window.Sinoclaw) {
    (window as any).Sinoclaw = {} as WindowNamespace;
  }

  if (!window.Sinoclaw.host) {
    window.Sinoclaw.host = {
      React,
      ReactDOM,
      antd,
      antdIcons,
  ...
  if (!window.Sinoclaw.registerRoutes) {
    window.Sinoclaw.registerRoutes = (pluginId, routes) => {
  ...
  if (!window.Sinoclaw.registerToolRender) {
    window.Sinoclaw.registerToolRender = (pluginId, renderers) => {
