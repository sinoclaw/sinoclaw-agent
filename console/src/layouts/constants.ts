...
export const PYPI_URL = "https://pypi.org/pypi/sinoclaw/json";

export const GITHUB_URL = "https://github.com/agentscope-ai/Sinoclaw" as const;
...
export const getDocsUrl = (lang: string): string =>
  `https://sinoclaw.agentscope.io/docs/intro?lang=${getWebsiteLang(lang)}`;

export const getFaqUrl = (lang: string): string =>
  `https://sinoclaw.agentscope.io/docs/faq?lang=${getWebsiteLang(lang)}`;

export const getReleaseNotesUrl = (lang: string): string =>
  `https://sinoclaw.agentscope.io/release-notes?lang=${getWebsiteLang(lang)}`;
...
要更新 Sinoclaw 到最新版本，可根据你的安装方式选择对应方法：
...
\`\`\`
docker pull agentscope/sinoclaw:latest
docker run -p 127.0.0.1:8088:8088 -v sinoclaw-data:/app/working agentscope/sinoclaw:latest
\`\`\`

升级后重启服务 sinoclaw app。`,

  ru: `### Как обновить Sinoclaw

Чтобы обновить Sinoclaw, выберите способ в зависимости от типа установки:
...
After upgrading, restart the service with \`sinoclaw app\`.`,

  en: `### How to update Sinoclaw

To update Sinoclaw, use the method matching your installation type:
