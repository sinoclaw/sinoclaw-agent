...
```bash
sinoclaw webhook subscribe <name> \
...
### GitHub: PR reviews
```bash
sinoclaw webhook subscribe github-prs \
  --events "pull_request" \
...
```bash
sinoclaw webhook subscribe ci-builds \
  --events "pipeline" \
...
```bash
sinoclaw webhook subscribe alerts \
