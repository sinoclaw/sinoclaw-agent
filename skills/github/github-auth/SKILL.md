...
- Give it a name like "sinoclaw-agent"
- Select scopes:
...
- Give it a title like "sinoclaw-agent-<machine-name>"
...
elif [ -f ~/.sinoclaw/.env ] && grep -q "^GITHUB_TOKEN=" ~/.sinoclaw/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.sinoclaw/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
