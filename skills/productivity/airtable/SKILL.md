...
4. Store the token in `~/.sinoclaw/.env` (or via `sinoclaw setup`):
   ```
...
- **`AIRTABLE_API_KEY` flows from `~/.sinoclaw/.env` into the subprocess automatically** when this skill is loaded — no need to re-export it before each `curl` call.
