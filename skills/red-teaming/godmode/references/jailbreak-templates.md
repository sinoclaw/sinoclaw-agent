...
## 5. GODMODE FAST — Sinoclaw 4 405B (Zero Refusal)

**Model:** `nousresearch/sinoclaw-4-405b`
**Strategy:** Sinoclaw models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Sinoclaw Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.sinoclaw/config.yaml`:
