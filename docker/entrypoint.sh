...
if [ "$(id -u)" = "0" ]; then
    if [ -n "$HERMES_UID" ] && [ "$HERMES_UID" != "$(id -u sinoclaw)" ]; then
        echo "Changing sinoclaw UID to $HERMES_UID"
        usermod -u "$HERMES_UID" sinoclaw
    fi

    if [ -n "$HERMES_GID" ] && [ "$HERMES_GID" != "$(id -g sinoclaw)" ]; then
        echo "Changing sinoclaw GID to $HERMES_GID"
        # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already exist
        # as "dialout" in the Debian-based container image)
        groupmod -o -g "$HERMES_GID" sinoclaw 2>/dev/null || true
    fi
