...
Uses a file-based lock (~/.sinoclaw/cron/.tick.lock) so only one tick
...
# Without this, standalone invocations (e.g. after `sinoclaw update` reloads
# the module) fail with ModuleNotFoundError for sinoclaw_time et al.
sys.path.insert(0, str(Path(__file__).parent.parent))

from sinoclaw_constants import get_sinoclaw_home
from sinoclaw_cli.config import load_config, _expand_env_vars
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    2. Per-platform ``sinoclaw tools`` config for the ``cron`` platform.
    ...
    if per_job:
        return per_job
    try:
        from sinoclaw_cli.tools_config import _get_platform_tools  # lazy: avoid heavy import at cron module load
...
_sinoclaw_home: Path | None = None


def _get_sinoclaw_home() -> Path:
    """Resolve Sinoclaw home dynamically while preserving test monkeypatch hooks."""
    return _sinoclaw_home or get_sinoclaw_home()


def _get_lock_paths() -> tuple[Path, Path]:
    """Resolve cron lock paths at call time so profile/env changes are honored."""
    sinoclaw_home = _get_sinoclaw_home()
    lock_dir = sinoclaw_home / "cron"
...
def _get_script_timeout() -> int:
    ...
    env_value = os.getenv("SINOCLAW_CRON_SCRIPT_TIMEOUT", "").strip()
    if env_value:
        try:
            ...
            logger.warning("Invalid SINOCLAW_CRON_SCRIPT_TIMEOUT=%r; using config/default", env_value)
...
def _run_job_script(script_path: str) -> tuple[bool, str]:
    ...
    * anything else — run with the current Python interpreter
    ...
    scripts_dir = _get_sinoclaw_home() / "scripts"
...
def run_job(job: dict) -> tuple[bool, str, str, Optional[str]]:
    ...
    if job.get("no_agent"):
        ...
        now_iso = _sinoclaw_now().strftime("%Y-%m-%d %H:%M:%S")
    ...
    if script_path:
        ...
        if _ran_ok and not _parse_wake_gate(_script_output):
            ...
            silent_doc = (
                f"# Cron Job: {job_name}\n\n"
                f"**Job ID:** {job_id}\n"
                f"**Run Time:** {_sinoclaw_now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
    ...
    origin = _resolve_origin(job)
    _cron_session_id = f"cron_{job_id}_{_sinoclaw_now().strftime('%Y%m%d_%H%M%S')}"
    ...
    os.environ["SINOCLAW_CRON_SESSION"] = "1"
    ...
    # gateway message. Do not seed SINOCLAW_SESSION_* contextvars from the
    ...
    #     (notify_on_complete / watch_patterns) reads SINOCLAW_SESSION_PLATFORM
    #     and SINOCLAW_SESSION_CHAT_ID to populate watcher_platform / chat_id,
    #     which would route completion notifications to the origin chat
    #     instead of via SINOCLAW_CRON_AUTO_DELIVER_* below.
    #   - tools/tts_tool.py: picks Opus vs MP3 based on
    #     SINOCLAW_SESSION_PLATFORM == "telegram".
    ...
    #     SINOCLAW_SESSION_PLATFORM.
    #   - tools/send_message_tool.py: mirror source labelling and the
    #     send_message gate read SINOCLAW_SESSION_PLATFORM.
    # Cron output delivery itself reads job["origin"] directly via
    # _resolve_origin(job) and the SINOCLAW_CRON_AUTO_DELIVER_* vars set
    # below, so clearing SINOCLAW_SESSION_* here does not affect delivery.
    ...
    _cron_delivery_vars = (
        "SINOCLAW_CRON_AUTO_DELIVER_PLATFORM",
        "SINOCLAW_CRON_AUTO_DELIVER_CHAT_ID",
        "SINOCLAW_CRON_AUTO_DELIVER_THREAD_ID",
    )
    ...
    try:
        ...
        try:
            load_dotenv(str(_get_sinoclaw_home() / ".env"), override=True, encoding="utf-8")
        except UnicodeDecodeError:
            load_dotenv(str(_get_sinoclaw_home() / ".env"), override=True, encoding="latin-1")

        delivery_target = _resolve_delivery_target(job)
        if delivery_target:
            _VAR_MAP["SINOCLAW_CRON_AUTO_DELIVER_PLATFORM"].set(delivery_target["platform"])
            _VAR_MAP["SINOCLAW_CRON_AUTO_DELIVER_CHAT_ID"].set(str(delivery_target["chat_id"]))
            _VAR_MAP["SINOCLAW_CRON_AUTO_DELIVER_THREAD_ID"].set(
                ""
                if delivery_target.get("thread_id") is None
                else str(delivery_target["thread_id"])
            )

        model = job.get("model") or os.getenv("SINOCLAW_MODEL") or ""

        # Load config.yaml for model, reasoning, prefill, toolsets, provider routing
        _cfg = {}
        try:
            import yaml
            _cfg_path = str(_get_sinoclaw_home() / "config.yaml")
        ...
        prefill_file = os.getenv("SINOCLAW_PREFILL_MESSAGES_FILE", "") or _cfg.get("prefill_messages_file", "")
        if prefill_file:
            pfpath = Path(prefill_file).expanduser()
            if not pfpath.is_absolute():
                pfpath = _get_sinoclaw_home() / pfpath
            if pfpath.exists():
                try:
        ...
        try:
            # Do not inject SINOCLAW_INFERENCE_PROVIDER here. resolve_runtime_provider()
        ...
        agent = AIAgent(
            ...
            # SINOCLAW_HOME. When a workdir is configured, also inject project
        ...
        # override via SINOCLAW_CRON_TIMEOUT env var.  0 = unlimited.
        #
        # Uses the agent's built-in activity tracker (updated by
        # _touch_activity() on every tool call, API call, and stream delta).
        _raw_cron_timeout = os.getenv("SINOCLAW_CRON_TIMEOUT", "").strip()
        if _raw_cron_timeout:
            try:
                _cron_timeout = float(_raw_cron_timeout)
            except (ValueError, TypeError):
                logger.warning(
                    "Invalid SINOCLAW_CRON_TIMEOUT=%r; using default 600s",
                    _raw_cron_timeout,
                )
                _cron_timeout = 600.0
        else:
...
**Job ID:** {job_id}
**Run Time:** {_sinoclaw_now().strftime('%Y-%m-%d %H:%M:%S')}
...
def tick(verbose: bool = True, adapters=None, loop=None) -> int:
    ...
    try:
        ...
        if verbose and not due_jobs:
            logger.info("%s - No jobs due", _sinoclaw_now().strftime('%H:%M:%S'))
            return 0

        if verbose:
            logger.info("%s - %s job(s) due", _sinoclaw_now().strftime('%H:%M:%S'), len(due_jobs))
        ...
        # Set SINOCLAW_CRON_MAX_PARALLEL=1 to restore old serial behaviour.
        _max_workers: Optional[int] = None
        try:
            _env_par = os.getenv("SINOCLAW_CRON_MAX_PARALLEL", "").strip()
            if _env_par:
                _max_workers = int(_env_par) or None
        except (ValueError, TypeError):
            logger.warning("Invalid SINOCLAW_CRON_MAX_PARALLEL value; defaulting to unbounded")
