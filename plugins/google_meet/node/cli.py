"""`sinoclaw meet node ...` subcommand tree.

Wired into the existing ``sinoclaw meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``sinoclaw meet node`` argparse object — typically
...
def node_command(args: argparse.Namespace) -> int:
    """Dispatch for ``sinoclaw meet node ...``.
    ...
    if cmd == "run":
        ...
        print(f"             sinoclaw meet node approve <name> ws://<host>:{args.port} {token}")
        try:
