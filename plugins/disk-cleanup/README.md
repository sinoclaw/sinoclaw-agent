...
Auto-tracks and cleans up ephemeral files created during Sinoclaw Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$SINOCLAW_HOME` and `/tmp/sinoclaw-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `SINOCLAW_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `SINOCLAW_HOME` or `/tmp/sinoclaw-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$SINOCLAW_HOME/disk-cleanup/` is itself excluded
- `$SINOCLAW_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
