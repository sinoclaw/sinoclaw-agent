"""disk_cleanup — ephemeral file cleanup for Sinoclaw Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under SINOCLAW_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly SINOCLAW_HOME and /tmp/sinoclaw-*
Never touches: ~/.sinoclaw/logs/ or any system directory.
"""
...
def get_state_dir() -> Path:
    """State dir — separate from ``$SINOCLAW_HOME/logs/``."""
    return get_sinoclaw_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$SINOCLAW_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under SINOCLAW_HOME or ``/tmp/sinoclaw-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    sinoclaw_home = get_sinoclaw_home()
    try:
        path.resolve().relative_to(sinoclaw_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/sinoclaw-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("sinoclaw-"):
        return True
    return False
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside SINOCLAW_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under SINOCLAW_HOME (but leave SINOCLAW_HOME itself and
    ...
    _PROTECTED_TOP_LEVEL = {
        ...
        "sinoclaw-agent", "backups", "profiles", ".worktrees",
    }
    empty_removed = 0
    try:
        for dirpath in sorted(sinoclaw_home.rglob("*"), reverse=True):
            if not dirpath.is_dir() or dirpath == sinoclaw_home:
                continue
            try:
                rel_parts = dirpath.relative_to(sinoclaw_home).parts
            except ValueError:
                continue
...
def guess_category(path: Path) -> Optional[str]:
    ...
    sinoclaw_home = get_sinoclaw_home()
    try:
        rel = path.resolve().relative_to(sinoclaw_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "sinoclaw-agent",
        }:
            return None
        ...
        # Path isn't under SINOCLAW_HOME (e.g. /tmp/sinoclaw-*) — fall through.
        pass
