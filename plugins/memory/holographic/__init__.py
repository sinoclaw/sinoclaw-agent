...
Config in $SINOCLAW_HOME/config.yaml (profile-scoped):
  plugins:
    sinoclaw-memory-store:
      db_path: $SINOCLAW_HOME/memory_store.db   # omit to use the default
      auto_extract: false
      default_trust: 0.5
...
def _load_plugin_config() -> dict:
    ...
    try:
        ...
        return cfg_get(all_config, "plugins", "sinoclaw-memory-store", default={}) or {}
    except Exception:
        return {}
...
class HolographicMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        # Expand $SINOCLAW_HOME in user-supplied paths so config values like
        # "$SINOCLAW_HOME/memory_store.db" or "~/.sinoclaw/memory_store.db" both
        # resolve to the active profile's directory.
        if isinstance(db_path, str):
            db_path = db_path.replace("$SINOCLAW_HOME", _sinoclaw_home)
            db_path = db_path.replace("${SINOCLAW_HOME}", _sinoclaw_home)
