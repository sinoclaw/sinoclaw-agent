...
def cmd_enable(args) -> None:
    ...
    cfg = _read_config()
    host = _host_key()
    label = f"[{host}] " if host != "sinoclaw" else ""
...
def cmd_sync(args) -> None:
    ...
    Scans all Sinoclaw profiles and creates host blocks for any that don't
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'sinoclaw honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print("  Honcho not configured on default profile. Run 'sinoclaw honcho setup' first.\n")
        return
    ...
    for p in profiles:
        ...
        if clone_honcho_for_profile(p.name):
            print(f"  + {p.name} -> sinoclaw.{p.name}")
            created += 1
        else:
            skipped += 1
...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current Sinoclaw profile."""
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives Sinoclaw persistent cross-session memory.")
    ...
    if is_local:
        ...
        if not cfg.get("apiKey"):
            ...
            print("  Run 'sinoclaw honcho setup' again once you have a key.\n")
            return
    ...
    current_ai = sinoclaw_host.get("aiPeer") or cfg.get("aiPeer", "sinoclaw")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        sinoclaw_host["aiPeer"] = new_ai

    current_workspace = sinoclaw_host.get("workspace") or cfg.get("workspace", "sinoclaw")
    ...
    current_dialectic = str(sinoclaw_host.get("dialecticCadence") or cfg.get("dialecticCadence") or "3")
    print("\n  Dialectic cadence:")
    ...
    try:
        ...
        sinoclaw_host["dialecticCadence"] = 3
...
def _active_profile_name() -> str:
    """Return the active Sinoclaw profile name (respects --target-profile override)."""
    if _profile_override:
...
def cmd_status(args) -> None:
    ...
    if not cfg:
        ...
        print("  Run 'sinoclaw honcho setup' to configure.\n")
        return
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        ...
        print(f"  AI peer:     {ai}")
        print("    Sinoclaw' identity in Honcho. Seed with 'sinoclaw honcho identity <file>'.")
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ...
        print("    AI-to-AI inference. Sinoclaw asks Honcho's AI peer a question")
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'sinoclaw honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'sinoclaw honcho identity <file>' to seed one.")
        print()
        return
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → Sinoclaw + Honcho."""
    from pathlib import Path
    ...
    print("\nHoncho migration: OpenClaw native memory → Sinoclaw\n" + "─" * 50)
    print()
    ...
    if has_key:
        ...
        print("  Honcho is a cloud memory service that gives Sinoclaw persistent memory")
        ...
        print()
        answer = _prompt("  Run 'sinoclaw honcho setup' now?", default="y")
        if answer.lower() in ("y", "yes"):
            ...
            print()
            print("  Run 'sinoclaw honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files:
        ...
        print()
        print("  These are picked up automatically the first time you run 'sinoclaw'")
        print("  with Honcho configured and no prior session history.")
        print("  (Sinoclaw calls migrate_memory_files() on first session init.)")
        print()
        ...
        if has_key:
            ...
            print("  Run 'sinoclaw honcho setup' first, then re-run this step.")
    else:
    ...
    print()
    print("  In Sinoclaw, they are seeded once into Honcho's AI peer through the")
    ...
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'sinoclaw honcho setup' first, then seed manually:")
            for f in agent_files:
    ...
    print("    Sinoclaw:   cloud-backed Honcho peers. Files can stay on disk as source")
    ...
    print("    Sinoclaw:   Honcho context fetched async at turn end, injected next turn.")
    ...
    print("    Sinoclaw:   Honcho observes every message and updates representations")
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        print("  Running 'sinoclaw memory setup'...\n")
...
def register_cli(subparser) -> None:
    ...
    subs.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to Sinoclaw Honcho",
    )
