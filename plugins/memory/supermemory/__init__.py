...
_DEFAULT_CONTAINER_TAG = "sinoclaw"
...
def _load_supermemory_config(sinoclaw_home: str) -> dict:
    config = _default_config()
    config_path = Path(sinoclaw_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, sinoclaw_home: str) -> None:
    config_path = Path(sinoclaw_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `sinoclaw memory setup`.
        # All other options are documented for $SINOCLAW_HOME/supermemory.json
    ...
    def save_config(self, values, sinoclaw_home):
        ...
        _save_supermemory_config(sanitized, sinoclaw_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from sinoclaw_constants import get_sinoclaw_home
        self._sinoclaw_home = kwargs.get("sinoclaw_home") or str(get_sinoclaw_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._sinoclaw_home)
    ...
    def sync_turn(self, user_content: str, assistant_content: str, *, session_id: str = "") -> None:
        ...
        metadata = {"source": "sinoclaw", "type": "conversation_turn"}
        ...
        def _run():
            try:
                self._client.add_memory(
                    content.strip(),
                    metadata={"source": "sinoclaw_memory", "target": target, "type": "explicit_memory"},
