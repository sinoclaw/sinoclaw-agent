(function () {
  "use strict";
  // sinoclaw-achievements dashboard plugin
  // Originally authored by @PCinkusz — https://github.com/PCinkusz/sinoclaw-achievements (MIT).
  // Bundled into sinoclaw-agent. Upstream repo remains the staging ground for new
  ...
  if (!SDK || !window.__SINOCLAW_PLUGINS__) return;
  ...
  async function api(path, options) {
    const url = "/api/plugins/sinoclaw-achievements" + path;
  ...
  async function buildShareImage(achievement) {
    ...
    ctx.fillText("HERMES AGENT  ·  sinoclaw-agent.nousresearch.com", 70, H - 40);
  ...
  function ShareDialog({ achievement, onClose }) {
    ...
    function download() {
      ...
      a.href = url;
      a.download = "sinoclaw-achievement-" + (achievement.id || "badge") + ".png";
  ...
  window.__SINOCLAW_PLUGINS__.register("sinoclaw-achievements", AchievementsPage);
})();
