...
(`sinoclaw-dialog-bridge.invalid`). `Fetch.enable` intercepts those XHRs
...
- `toolsets.py` — register `browser_dialog` in `browser`, `sinoclaw-acp`, `sinoclaw-api-server`, core toolsets (gated on CDP reachability)
...
- `sinoclaw_cli/config.py` — add `browser.dialog_policy` and `browser.dialog_timeout_s` to `DEFAULT_CONFIG`
