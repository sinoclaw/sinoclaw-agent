...
description: "Building RL training environments, running evaluation benchmarks, and generating SFT data with the Sinoclaw-Agent Atropos integration"
---

# Environments, Benchmarks & Data Generation

Sinoclaw Agent includes a full environment framework that connects its tool-calling capabilities to the [Atropos](https://github.com/NousResearch/atropos) RL training framework. This enables three workflows:
...
The sinoclaw-agent layer (`environments/sinoclaw_base_env.py`). Adds:
- **Terminal backend configuration** — sets `TERMINAL_ENV` for sandboxed execution (local, Docker, Modal, Daytona, SSH, Singularity)
- **Tool resolution** — `_resolve_tools_for_group()` calls sinoclaw-agent's `get_tool_definitions()` to get the right tool schemas based on enabled/disabled toolsets
...
`HermesAgentLoop` (`environments/agent_loop.py`) is the reusable multi-turn agent engine. It runs the same tool-calling pattern as sinoclaw-agent's main loop:
...
| **Generic** | `call_tool(name, args)` — escape hatch for any sinoclaw-agent tool |
...
parser = get_parser("sinoclaw")  # or "mistral", "llama3_json", "qwen", "deepseek_v3", etc.
content, tool_calls = parser.parse(raw_model_output)
```

Available parsers: `sinoclaw`, `mistral`, `llama3_json`, `qwen`, `qwen3_coder`, `deepseek_v3`, `deepseek_v3_1`, `kimi_k2`, `longcat`, `glm45`, `glm47`.
...
pip install "sinoclaw-agent[yc-bench]"
...
- Set `tool_call_parser` in config to match your model's format (e.g., `"sinoclaw"`, `"qwen"`, `"mistral"`)
...
|-------|------|---------|-------------|
| `enabled_toolsets` | `List[str]` | `None` (all) | Which sinoclaw toolsets to enable |
...
| `tool_call_parser` | `str` | `"sinoclaw"` | Parser for Phase 2 raw output |
...
- [Modal](https://modal.com) account and CLI: `pip install "sinoclaw-agent[modal]"`
- `MODAL_TOKEN_ID` and `MODAL_TOKEN_SECRET` environment variables

### For YC-Bench

- `pip install "sinoclaw-agent[yc-bench]"` (installs the yc-bench CLI + SQLAlchemy)
...
├── sinoclaw_swe_env/             # SWE-bench training environment
│
