...
When you choose Anthropic OAuth through `hermes model`, Hermes prefers Claude Code's own credential store over copying the token into `~/.sinoclaw/.env`. That keeps refreshable Claude credentials refreshable.
