...
description: "Catalog of bundled skills that ship with Sinoclaw Agent"
---

# Bundled Skills Catalog

Hermes ships with a large built-in skill library copied into `~/.sinoclaw/skills/` on install. This page catalogs the bundled skills that live in the repository under `skills/`.
...
| `sinoclaw-agent-spawning` | Spawn additional Sinoclaw Agent instances as autonomous subprocesses for independent long-running tasks. Supports non-interactive one-shot mode (-q) and interactive PTY mode for multi-turn collaboration. Different from delegate_task — this runs a full separate sinoclaw process. | `autonomous-ai-agents/sinoclaw-agent` |
...
| `sinoclaw-agent-setup` | Help users configure Sinoclaw Agent — CLI usage, setup wizard, model/provider selection, tools, skills, voice/STT/TTS, gateway, and troubleshooting. | `dogfood/sinoclaw-agent-setup` |
...
| `native-mcp` | Built-in MCP (Model Context Protocol) client that connects to external MCP servers, discovers their tools, and registers them as native Sinoclaw Agent tools. Supports stdio and HTTP transports with automatic reconnection, security filtering, and zero-config tool injection. | `mcp/native-mcp` |
...
| `sinoclaw-atropos-environments` | Build, test, and debug Sinoclaw Agent RL environments for Atropos training. Covers the HermesAgentBaseEnv interface, reward functions, agent loop integration, evaluation with tools, wandb logging, and the three CLI modes (serve/process/evaluate). Use when creating, reviewing, or f… | `mlops/training/sinoclaw-atropos-environments` |
...
| `plan` | Plan mode for Hermes — inspect context, write a markdown plan into `.sinoclaw/plans/` in the active workspace/backend working directory, and do not execute the work. | `software-development/plan` |
...
| Skill | Description | Path |
|-------|-------------|------|
| `agentmail` | Give the agent its own dedicated email inbox via AgentMail. Send, receive, and manage email autonomously using agent-owned email addresses (e.g. sinoclaw-agent@agentmail.to). | `email/agentmail` |
...
| Skill | Description | Path |
|-------|-------------|------|
| `openclaw-migration` | Migrate a user's OpenClaw customization footprint into Sinoclaw Agent. Imports Sinoclaw-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports what could not be migrated and why. | `migration/openclaw-migration` |
