...
sidebar_position: 2
title: "Installation"
description: "Install Sinoclaw Agent on Linux, macOS, WSL2, or Android via Termux"
---

# Installation

Get Sinoclaw Agent up and running in under two minutes with the one-line installer, or follow the manual steps for full control.
...
:::warning Windows
Native Windows is **not supported**. Please install [WSL2](https://learn.microsoft.com/en-us/windows/wsl/install) and run Sinoclaw Agent from there. The install command above works inside WSL2.
:::

### What the Installer Does

The installer handles everything automatically — all dependencies (Python, Node.js, ripgrep, ffmpeg), the repo clone, virtual environment, global `sinoclaw` command setup, and LLM provider configuration. By the end, you're ready to chat.
...
:::tip
You do **not** need to activate the venv to use `sinoclaw`. The entry point has a hardcoded shebang pointing to the venv Python, so it works globally once symlinked.
:::
...
mkdir -p ~/.sinoclaw/{cron,sessions,logs,memories,skills,pairing,hooks,image_cache,audio_cache,whatsapp/session}
...
Open `~/.sinoclaw/.env` and add at minimum an LLM provider key:
...
```bash
sinoclaw config set OPENROUTER_API_KEY sk-or-v1-your-key-here
```

### Step 8: Add `sinoclaw` to Your PATH

```bash
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/sinoclaw" ~/.local/bin/sinoclaw
```
...
```bash
sinoclaw model       # Select your LLM provider and model
```

### Step 10: Verify the Installation

```bash
sinoclaw version    # Check that the command is available
sinoclaw doctor     # Run diagnostics to verify everything is working
sinoclaw status     # Check your configuration
sinoclaw chat -q "Hello! What tools do you have available?"
```
...
cp cli-config.yaml.example ~/.sinoclaw/config.yaml
touch ~/.sinoclaw/.env
echo 'OPENROUTER_API_KEY=sk-or-v1-your-key' >> ~/.sinoclaw/.env

# Make sinoclaw available globally
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/sinoclaw" ~/.local/bin/sinoclaw
...
| Problem | Solution |
|---------|----------|
| `sinoclaw: command not found` | Reload your shell (`source ~/.bashrc`) or check PATH |
