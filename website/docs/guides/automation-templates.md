...
```bash
sinoclaw cron create "0 2 * * *" \
  "You are a project manager triaging the NousResearch/sinoclaw-agent GitHub repo.

1. Run: gh issue list --repo NousResearch/sinoclaw-agent --state open --json number,title,labels,author,createdAt --limit 30
...
```bash
sinoclaw webhook subscribe github-pr-review \
  --events "pull_request" \
...
```bash
sinoclaw cron create "0 9 * * 1" \
  "Scan the NousResearch/sinoclaw-agent repo for documentation drift.

1. Run: gh pr list --repo NousResearch/sinoclaw-agent --state merged --json number,title,files,mergedAt --limit 30
2. Filter to PRs merged in the last 7 days
3. For each merged PR, check if it modified:
   - Tool schemas (tools/*.py) — may need docs/reference/tools-reference.md update
   - CLI commands (sinoclaw_cli/commands.py, sinoclaw_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (sinoclaw_cli/config.py) — may need docs/user-guide/configuration.md update
...
```bash
sinoclaw cron create "0 6 * * *" \
  "Run a dependency security audit on the sinoclaw-agent project.

1. cd ~/.sinoclaw/sinoclaw-agent && source .venv/bin/activate
...
```bash
sinoclaw webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
sinoclaw webhook subscribe alert-triage \
...
```python title="~/.sinoclaw/scripts/check-uptime.py"
...
```bash
sinoclaw cron create "every 30m" \
  ...
  --script ~/.sinoclaw/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
sinoclaw webhook subscribe github-issues \
  --events "issues" \
...
```bash
sinoclaw webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
sinoclaw webhook subscribe stripe-payments \
...
```bash
sinoclaw cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the sinoclaw-agent codebase.
