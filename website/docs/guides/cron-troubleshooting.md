...
```bash
date
sinoclaw cron list   # Compare next_run times with local time
```
...
sinoclaw cron edit <job_id> --script ~/.sinoclaw/scripts/your-script.py
```
...
```bash
sinoclaw cron list                    # Show all jobs, states, next_run times
sinoclaw cron run <job_id>            # Schedule for next tick (for testing)
sinoclaw cron edit <job_id>           # Fix configuration issues
sinoclaw logs                         # View recent Hermes logs
