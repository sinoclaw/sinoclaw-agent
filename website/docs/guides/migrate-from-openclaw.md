...
| `--no-backup` | Skip the pre-migration zip snapshot of `~/.sinoclaw/` (by default a single restore-point archive is written before apply, under `~/.sinoclaw/backups/pre-migration-*.zip`; restorable with `hermes import`). |
