...
```bash
mkdir -p ~/.sinoclaw/scripts
```

```python title="~/.sinoclaw/scripts/watch-site.py"
...
```bash
/cron add "every 1h" "If the script output says CHANGE DETECTED, summarize what changed on the page and why it might matter. If it says NO_CHANGE, respond with just [SILENT]." --script ~/.sinoclaw/scripts/watch-site.py --name "Pricing monitor" --deliver telegram
```
...
```bash
/cron add "every 6h" "Check the GitHub repository NousResearch/sinoclaw-agent for:
- New issues opened in the last 6 hours
- New PRs opened or merged in the last 6 hours
- Any new releases

Use the terminal to run gh commands:
  gh issue list --repo NousResearch/sinoclaw-agent --state open --json number,title,author,createdAt --limit 10
  gh pr list --repo NousResearch/sinoclaw-agent --state all --json number,title,author,createdAt,mergedAt --limit 10
...
```python title="~/.sinoclaw/scripts/collect-prices.py"
import json, os, urllib.request
from datetime import datetime

DATA_DIR = os.path.expanduser("~/.sinoclaw/data/prices")
...
If there's a significant move, explain what happened." \
  --script ~/.sinoclaw/scripts/collect-prices.py \
  --name "Price tracker" \
  --deliver telegram
```
