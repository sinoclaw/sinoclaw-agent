...
|--------|---------------|---------|----------|
| **[Gateway hooks](#gateway-event-hooks)** | `HOOK.yaml` + `handler.py` in `~/.sinoclaw/hooks/` | Gateway only | Logging, alerts, webhooks |
...
| **[Shell hooks](#shell-hooks)** | `hooks:` block in `~/.sinoclaw/config.yaml` pointing at shell scripts | CLI + Gateway | Drop-in scripts for blocking, auto-formatting, context injection |
...
Each hook is a directory under `~/.sinoclaw/hooks/` containing two files:
...
```yaml
# ~/.sinoclaw/hooks/long-task-alert/HOOK.yaml
...
```python
# ~/.sinoclaw/hooks/long-task-alert/handler.py
import os
import httpx
...
```yaml
# ~/.sinoclaw/hooks/command-logger/HOOK.yaml
name: command-logger
...
```python
# ~/.sinoclaw/hooks/command-logger/handler.py
import json
from datetime import datetime
from pathlib import Path
...
```yaml
# ~/.sinoclaw/hooks/session-webhook/HOOK.yaml
...
```python
# ~/.sinoclaw/hooks/session-webhook/handler.py
import httpx
...
A popular pattern from the community: drop a Markdown checklist at `~/.sinoclaw/BOOT.md`, and have the agent run it once every time the gateway starts. Useful for "on every boot, check overnight cron failures and ping me on Discord if anything failed," or "summarize the last 24h of deploy.log and post it to Slack #ops."
...
1. A file at `~/.sinoclaw/BOOT.md` with natural-language startup instructions.
...
Create `~/.sinoclaw/BOOT.md`. Write it as if you were giving instructions to a human assistant:
...
```text
~/.sinoclaw/hooks/boot-md/
├── HOOK.yaml
└── handler.py
```

**`~/.sinoclaw/hooks/boot-md/HOOK.yaml`**

```yaml
name: boot-md
description: Run ~/.sinoclaw/BOOT.md on gateway startup
events:
  - gateway:startup
```

**`~/.sinoclaw/hooks/boot-md/handler.py`**

```python
"""Run ~/.sinoclaw/BOOT.md on every gateway startup."""
...
Delete `~/.sinoclaw/BOOT.md` to disable the checklist — the hook stays loaded but silently skips when the file isn't there.
...
1. On gateway startup, `HookRegistry.discover_and_load()` scans `~/.sinoclaw/hooks/`
...
Shell hooks are registered by calling `agent.shell_hooks.register_from_config(cfg)` at both CLI startup (`sinoclaw_cli/main.py`) and gateway startup (`gateway/run.py`). They compose naturally with Python plugin hooks — both flow through the same dispatcher.

### Comparison at a glance

| Dimension | Shell hooks | [Plugin hooks](#plugin-hooks) | [Gateway hooks](#gateway-event-hooks) |
|-----------|-------------|-------------------------------|---------------------------------------|
| Declared in | `hooks:` block in `~/.sinoclaw/config.yaml` | `register()` in a `plugin.yaml` plugin | `HOOK.yaml` + `handler.py` directory |
| Lives under | `~/.sinoclaw/agent-hooks/` (by convention) | `~/.sinoclaw/plugins/<name>/` | `~/.sinoclaw/hooks/<name>/` |
...
hooks:
  post_tool_call:
    - matcher: "write_file|patch"
      command: "~/.sinoclaw/agent-hooks/auto-format.sh"
```

```bash
#!/usr/bin/env bash
# ~/.sinoclaw/agent-hooks/auto-format.sh
payload="$(cat -)"
...
hooks:
  pre_tool_call:
    - matcher: "terminal"
      command: "~/.sinoclaw/agent-hooks/block-rm-rf.sh"
      timeout: 5
```

```bash
#!/usr/bin/env bash
# ~/.sinoclaw/agent-hooks/block-rm-rf.sh
payload="$(cat -)"
...
hooks:
  pre_llm_call:
    - command: "~/.sinoclaw/agent-hooks/inject-cwd-context.sh"
```

```bash
#!/usr/bin/env bash
# ~/.sinoclaw/agent-hooks/inject-cwd-context.sh
...
hooks:
  subagent_stop:
    - command: "~/.sinoclaw/agent-hooks/log-orchestration.sh"
```

```bash
#!/usr/bin/env bash
# ~/.sinoclaw/agent-hooks/log-orchestration.sh
log=~/.sinoclaw/logs/orchestration.log
...
Each unique `(event, command)` pair prompts the user for approval the first time Hermes sees it, then persists the decision to `~/.sinoclaw/shell-hooks-allowlist.json`. Subsequent runs (CLI or gateway) skip the prompt.
