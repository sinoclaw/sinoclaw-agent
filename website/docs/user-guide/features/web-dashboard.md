...
```bash
# Custom port
sinoclaw dashboard --port 8080

# Bind to all interfaces (use with caution on shared networks)
sinoclaw dashboard --host 0.0.0.0

# Start without opening browser
sinoclaw dashboard --no-open
```
...
```bash
pip install sinoclaw-agent[web]
```

If you installed with `pip install sinoclaw-agent[all]`, the web dependencies are already included.
...
Create a YAML file in `~/.sinoclaw/dashboard-themes/`:
