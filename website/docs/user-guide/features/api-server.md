...
sidebar_position: 14
title: "API Server"
description: "Expose sinoclaw-agent as an OpenAI-compatible API for any frontend"
---

# API Server

The API server exposes sinoclaw-agent as an OpenAI-compatible HTTP endpoint. Any frontend that speaks the OpenAI format — Open WebUI, LobeChat, LibreChat, NextChat, ChatBox, and hundreds more — can connect to sinoclaw-agent and use it as a backend.
...
curl http://localhost:8642/v1/chat/completions \
  ...
  -d '{"model": "sinoclaw-agent", "messages": [{"role": "user", "content": "Hello!"}]}'
```
...
Lists the agent as an available model. The advertised model name defaults to the [profile](/docs/user-guide/features/profiles) name (or `sinoclaw-agent` for the default profile). Required by most frontends for model discovery.
...
When a frontend sends a `system` message (Chat Completions) or `instructions` field (Responses API), sinoclaw-agent **layers it on top** of its core system prompt. Your agent keeps all its tools, memory, and skills — the frontend's system prompt adds extra instructions.
...
:::warning Security
The API server gives full access to sinoclaw-agent's toolset, **including terminal commands**. When binding to a non-loopback address like `0.0.0.0`, `API_SERVER_KEY` is **required**. Also keep `API_SERVER_CORS_ORIGINS` narrow to control browser access.
...
| `API_SERVER_MODEL_NAME` | _(profile name)_ | Model name on `/v1/models`. Defaults to profile name, or `sinoclaw-agent` for default profile. |
...
sinoclaw profile create alice
sinoclaw profile create bob
