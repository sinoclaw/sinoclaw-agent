...
```bash
# Required
EMAIL_ADDRESS=sinoclaw@gmail.com
...
| **Duplicate replies** | Ensure only one gateway instance is running. Check `sinoclaw gateway status`. |
...
- The password is stored in `~/.sinoclaw/.env` — protect this file (`chmod 600`)
