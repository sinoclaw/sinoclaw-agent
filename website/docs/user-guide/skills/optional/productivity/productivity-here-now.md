...
| | |
|---|---|
| Source | Optional — install with `sinoclaw skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${SINOCLAW_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${SINOCLAW_SKILL_DIR}/scripts/drive.sh` for private Drive storage
...
```bash
DRIVE="${SINOCLAW_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
