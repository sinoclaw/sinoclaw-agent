...
:::warning Early BETA
Native Windows support is **early beta**. It installs, runs, and passes our Windows-footgun lint, but it hasn't been road-tested at the scale our Linux/macOS/WSL2 paths have. Expect rough edges — especially around subprocess handling, path quirks, and non-ASCII console output. Please [file issues](https://github.com/NousResearch/sinoclaw-agent/issues) with repro steps when you hit something. If you want a battle-tested setup today, use the [Linux/macOS installer under WSL2](./windows-wsl-quickstart.md) instead.
:::
...
```powershell
& ([scriptblock]::Create((irm https://raw.githubusercontent.com/NousResearch/sinoclaw-agent/main/scripts/install.ps1))) -NoVenv -SkipSetup -Branch main
```
...
| `-InstallDir` | `%LOCALAPPDATA%\hermes\sinoclaw-agent` | Override code location |
...
5. **Clones the repo** to `%LOCALAPPDATA%\hermes\sinoclaw-agent` and creates a virtualenv inside it.
...
|---|---|---|
| CLI (`hermes chat`, `sinoclaw setup`, `sinoclaw gateway`, …) | ✓ | ✓ |
...
1. `SINOCLAW_GIT_BASH_PATH` environment variable if set.
...
The installer sets `SINOCLAW_GIT_BASH_PATH` explicitly so fresh PowerShell sessions don't have to re-discover. Override it if you want Hermes to use a specific bash — for example, your system Git Bash or a WSL-hosted bash via a symlink.
...
The fix is in `sinoclaw_cli/stdio.py::configure_windows_stdio()`, called early in every entry point (`cli.py::main`, `sinoclaw_cli/main.py::main`, `gateway/run.py::main`). It:
...
**Opt out:** `SINOCLAW_DISABLE_WINDOWS_UTF8=1` in the environment falls back to the legacy cp1252 stdio path. Useful for bisecting an encoding bug; unlikely to be the right setting in normal operation.
...
| Path | Contents |
|---|---|
| `%LOCALAPPDATA%\hermes\sinoclaw-agent\` | Git checkout + venv. Safe to `Remove-Item -Recurse` and reinstall. |
...
**Override `SINOCLAW_HOME`:** set the environment variable to point at a different data dir. Works the same as on Linux.
...
| Variable | Effect |
|---|---|
| `SINOCLAW_GIT_BASH_PATH` | Override bash.exe discovery. Point at any bash — full Git-for-Windows, WSL bash via symlink, MSYS2, Cygwin. The installer sets this automatically. |
| `SINOCLAW_DISABLE_WINDOWS_UTF8` | Set to `1` to disable the UTF-8 stdio shim and fall back to the locale code page. Useful for bisecting an encoding bug. |
...
That's the clean path — removes the schtasks entry, Startup folder shortcut, `hermes.cmd` shim, deletes `%LOCALAPPDATA%\hermes\sinoclaw-agent\`, and trims the User PATH. It leaves `%USERPROFILE%\.hermes\` alone (your config, auth, skills, sessions, logs) in case you're reinstalling.
