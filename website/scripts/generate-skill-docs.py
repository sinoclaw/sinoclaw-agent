...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/NousResearch/sinoclaw-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    sinoclaw_meta = (fm.get("metadata") or {}).get("hermes") or {}
    tags = sinoclaw_meta.get("tags") or []
    related = sinoclaw_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `sinoclaw skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
    ...
    return (
        ...
        "\n"
        ":::info\n"
        "The following is the complete skill definition that Sinoclaw loads when this skill is triggered. This is what the agent sees as instructions when the skill is active.\n"
        ":::\n"
        "\n"
        f"{body_clean}\n"
    )
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        "",
        "Sinoclaw ships with a large built-in skill library copied into `~/.sinoclaw/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Sinoclaw also syncs bundled skills on `sinoclaw update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.sinoclaw/skills/` tree, it is still shipped with Sinoclaw; restore it with `sinoclaw skills reset <name> --restore`.",
        "",
