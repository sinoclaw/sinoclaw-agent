...
let
  ...
  # to a dev checkout — the loader reads them from SINOCLAW_BUNDLED_PLUGINS.
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    mkdir -p $out/share/sinoclaw-agent $out/bin
    cp -r ${bundledSkills} $out/share/sinoclaw-agent/skills
    cp -r ${bundledPlugins} $out/share/sinoclaw-agent/plugins
    cp -r ${hermesWeb} $out/share/sinoclaw-agent/web_dist
    ...
    ${lib.concatMapStringsSep "\n"
      (name: ''
        makeWrapper ${hermesVenv}/bin/${name} $out/bin/${name} \
          ...
          --set SINOCLAW_BUNDLED_SKILLS $out/share/sinoclaw-agent/skills \
          --set SINOCLAW_BUNDLED_PLUGINS $out/share/sinoclaw-agent/plugins \
          --set SINOCLAW_WEB_DIST $out/share/sinoclaw-agent/web_dist \
          --set SINOCLAW_TUI_DIR $out/ui-tui \
          --set SINOCLAW_PYTHON ${hermesVenv}/bin/python3 \
  ...
  passthru = {
    ...
    devShellHook = ''
      STAMP=".nix-stamps/sinoclaw-agent"
      STAMP_VALUE="${pyprojectHash}:${uvLockHash}"
      if [ ! -f "$STAMP" ] || [ "$(cat "$STAMP")" != "$STAMP_VALUE" ]; then
        echo "sinoclaw-agent: installing Python dependencies..."
        ...
        source .venv/bin/activate
        export SINOCLAW_PYTHON=${hermesVenv}/bin/python3
      fi
    '';
  };
