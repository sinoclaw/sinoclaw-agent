...
本项目 fork 自 [NousResearch/hermes-agent](https://github.com/nousresearch/hermes-agent)，用于品牌定制和二次开发。
...
上游更新可能包含对 Hermes → Sinoclaw 的修改。如果有冲突，搜索 `Hermes`、`hermes`、`hermes_cli` 等关键词，确保全部替换为对应的 Sinoclaw 名称。
...
```bash
# 示例：上游新增了 hermes_util.py
mv hermes_util.py sinoclaw_util.py
# 同时更新所有 import 引用
find . -name "*.py" -exec sed -i 's/hermes_util/sinoclaw_util/g' {} +
```
