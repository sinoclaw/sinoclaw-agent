"""Centralized logging setup for Sinoclaw Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.sinoclaw/logs/`` (profile-aware via ``get_sinoclaw_home()``).
...
from sinoclaw_constants import get_config_path, get_sinoclaw_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_sinoclaw_session_injector", False):
    ...
    _session_record_factory._sinoclaw_session_injector = True  # type: ignore[attr-defined]
...
def setup_logging(
    *,
    ...
    """Configure the Sinoclaw logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    sinoclaw_home
        Override for the Sinoclaw home directory.  Falls back to
        ``get_sinoclaw_home()`` (profile-aware).
    log_level
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_sinoclaw_verbose", False):
                return

    handler = logging.StreamHandler()
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._sinoclaw_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    def __init__(self, *args, **kwargs):
        from sinoclaw_cli.config import is_managed
