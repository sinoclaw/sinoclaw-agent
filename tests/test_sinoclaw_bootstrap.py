"""Tests for sinoclaw_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Sinoclaw entry point
(hermes, sinoclaw-agent, sinoclaw-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Sinoclaw entry point imports sinoclaw_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported sinoclaw_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("sinoclaw_bootstrap", None)
    import sinoclaw_bootstrap  # noqa: WPS433
    return sinoclaw_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Sinoclaw CLI
...
class TestEntryPointsImportBootstrap:
    """Every Sinoclaw entry point must import sinoclaw_bootstrap as its
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import sinoclaw_bootstrap' and that
        ...
        """
        # Resolve relative to the sinoclaw-agent repo root.  Tests live
        # at tests/test_sinoclaw_bootstrap.py, so go up one dir.
        import pathlib
        ...
        assert first_import_name == "sinoclaw_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'sinoclaw_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import sinoclaw_bootstrap' line to be the first import."
        )
