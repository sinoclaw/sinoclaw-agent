...
from sinoclaw_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(sinoclaw_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <sinoclaw_home>/plugins/<name> and opt it in."""
    plugin_dir = sinoclaw_home / "plugins" / name
...
def test_hook_receives_expected_kwargs(tmp_path, monkeypatch):
    ...
    sinoclaw_home.mkdir(exist_ok=True)
    _make_enabled_plugin(
        sinoclaw_home, "capture_hook",
        register_body=(
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("SINOCLAW_HOME", str(tmp_path / "sinoclaw_empty"))
