...
def test_normalize_retain_tags_accepts_csv_and_dedupes():
    assert _normalize_retain_tags("agent:fakeassistantname, source_system:sinoclaw-agent, agent:fakeassistantname") == [
        "agent:fakeassistantname",
        "source_system:sinoclaw-agent",
    ]


def test_normalize_retain_tags_accepts_json_array_string():
    value = json.dumps(["agent:fakeassistantname", "source_system:sinoclaw-agent"])
    assert _normalize_retain_tags(value) == ["agent:fakeassistantname", "source_system:sinoclaw-agent"]
...
class TestPostSetup:
    def test_local_embedded_setup_materializes_profile_env(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("sinoclaw_cli.memory_setup._curses_select", lambda *args, **kwargs: next(selections))
        monkeypatch.setattr("shutil.which", lambda name: None)
        monkeypatch.setattr("builtins.input", lambda prompt="": "")
        monkeypatch.setattr("sys.stdin.isatty", lambda: True)
        monkeypatch.setattr("getpass.getpass", lambda prompt="": "sk-local-test")
        saved_configs = []
        monkeypatch.setattr("sinoclaw_cli.config.save_config", lambda cfg: saved_configs.append(cfg.copy()))

        provider = HindsightMemoryProvider()
        provider.post_setup(str(sinoclaw_home), {"memory": {}})

        assert saved_configs[-1]["memory"]["provider"] == "hindsight"
        env_text = (sinoclaw_home / ".env").read_text()
    ...
    def test_local_embedded_setup_respects_existing_profile_name(self, tmp_path, monkeypatch):
        ...
        provider = HindsightMemoryProvider()
        provider.save_config({"profile": "coder"}, str(sinoclaw_home))
        provider.post_setup(str(sinoclaw_home), {"memory": {}})
    ...
    def test_local_embedded_setup_blank_inputs_preserve_existing_config(self, tmp_path, monkeypatch):
        ...
        user_home.mkdir()
        monkeypatch.setenv("HOME", str(user_home))
        monkeypatch.setattr("plugins.memory.hindsight.get_sinoclaw_home", lambda: sinoclaw_home)
        ...
        provider = HindsightMemoryProvider()
        provider.save_config(existing_config, str(sinoclaw_home))
        ...
        monkeypatch.setattr("sinoclaw_cli.memory_setup._curses_select", lambda *args, **kwargs: kwargs.get("default", 0))
        monkeypatch.setattr("shutil.which", lambda name: None)
        monkeypatch.setattr("builtins.input", lambda prompt="": "")
        monkeypatch.setattr("sys.stdin.isatty", lambda: True)
        monkeypatch.setattr("getpass.getpass", lambda prompt="": "")
        ...
        saved = json.loads((sinoclaw_home / "hindsight" / "config.json").read_text())
...
class TestSyncTurn:
    def test_sync_turn_retains_metadata_rich_turn(self, provider_with_config):
        ...
        assert item["context"] == "conversation between Sinoclaw Agent and the User"
    ...
    def test_resume_creates_new_document(self, tmp_path, monkeypatch):
        ...
        config_path.parent.mkdir(parents=True, exist_ok=True)
        config_path.write_text(json.dumps(config))
        monkeypatch.setattr("plugins.memory.hindsight.get_sinoclaw_home", lambda: tmp_path)

        p1 = HindsightMemoryProvider()
        p1.initialize(session_id="resumed-session", sinoclaw_home=str(tmp_path), platform="cli")
        ...
        p2.initialize(session_id="resumed-session", sinoclaw_home=str(tmp_path), platform="cli")
...
class TestBankIdTemplate:
    ...
    def test_resolve_with_profile(self):
        result = _resolve_bank_id_template(
            "sinoclaw-{profile}", fallback="hermes",
            profile="coder", workspace="", platform="", user="", session="",
        )
        assert result == "sinoclaw-coder"
    ...
    def test_resolve_invalid_template_returns_fallback(self):
        ...
        result = _resolve_bank_id_template(
            "sinoclaw-{unknown}", fallback="hermes",
            profile="", workspace="", platform="", user="", session="",
        )
        assert result == "hermes"
    ...
    def test_provider_uses_bank_id_template_from_config(self, tmp_path, monkeypatch):
        config = {
            ...
            "bank_id_template": "sinoclaw-{profile}",
        }
        config_path = tmp_path / "hindsight" / "config.json"
        config_path.parent.mkdir(parents=True, exist_ok=True)
        config_path.write_text(json.dumps(config))
        ...
        assert p._bank_id == "sinoclaw-coder"
        assert p._bank_id_template == "sinoclaw-{profile}"
