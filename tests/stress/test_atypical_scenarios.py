...
normal tests assume away:
  ...
  - Filesystem: SINOCLAW_HOME with spaces / unicode / symlinks.
...
def scenario(name):
    """Decorator: run `fn` in its own SINOCLAW_HOME, collect failures.
    ...
    def wrap(fn):
        def run():
            ...
            for m in list(sys.modules.keys()):
                if m.startswith(("sinoclaw_cli", "plugins", "gateway")):
                    del sys.modules[m]
            sys.path.insert(0, str(WT))
            from sinoclaw_cli import kanban_db as kb  # noqa: F401
            print(f"\n═══ {name} ═══")
            try:
                fn(home, kb)
                print(f"  ✔ {name}")
...
def _(home, kb):
    ...
    env = {**os.environ, "PYTHONPATH": str(WT), "SINOCLAW_HOME": home, "HOME": home}
    bad_metas = [
        "not-json",
...
def _(home, kb):
    ...
    try:
        ...
        # allow escape.
        try:
            from sinoclaw_cli.kanban_db import resolve_workspace
            ...
            home_abs = str(Path(os.environ["SINOCLAW_HOME"]).resolve())
            if not resolved_abs.startswith(home_abs) and resolved_abs.startswith("/tmp"):
                ...
                print(f"  ⚠ workspace resolved OUTSIDE sinoclaw_home: {resolved}")
...
@scenario("sinoclaw_home_with_spaces")
def _(home, kb):
    """SINOCLAW_HOME at a path with spaces — should work but catches
    ...
    os.environ["SINOCLAW_HOME"] = weird
    ...
    try:
        ...
        print(f"  SINOCLAW_HOME with spaces: OK at {weird}")
    finally:
        conn.close()
        shutil.rmtree(weird, ignore_errors=True)


@scenario("sinoclaw_home_with_unicode")
def _(home, kb):
    """SINOCLAW_HOME with non-ASCII chars."""
    ...
    try:
        ...
        print(f"  SINOCLAW_HOME with unicode path: OK at {weird}")
    finally:
        conn.close()
        shutil.rmtree(weird, ignore_errors=True)


@scenario("sinoclaw_home_via_symlink")
def _(home, kb):
    """SINOCLAW_HOME is a symlink to the real dir. _INITIALIZED_PATHS
    ...
    try:
        os.environ["SINOCLAW_HOME"] = link1
        ...
        os.environ["SINOCLAW_HOME"] = link2
        ...
        conn2.close()
        print("  symlinks to same SINOCLAW_HOME share DB correctly")
    finally:
...
def _idempotency_race_worker(sinoclaw_home: str, key: str, result_file: str,
...
def _(home, kb):
    ...
    try:
        from sinoclaw_cli import web_server as ws  # noqa
    except Exception:
        pass
