...
class TestDefaultInteractionDispatch:
    ...
    async def test_update_prompt_click_writes_response_file(self, tmp_path, monkeypatch):
        """update_prompt:y click writes 'y' to ~/.sinoclaw/.update_response."""
        adapter = self._make_adapter()
        sinoclaw_home = tmp_path / "sinoclaw_home"
        sinoclaw_home.mkdir()
        monkeypatch.setattr(
            "sinoclaw_constants.get_sinoclaw_home",
            lambda: sinoclaw_home,
        )
        ...
        response = sinoclaw_home / ".update_response"
