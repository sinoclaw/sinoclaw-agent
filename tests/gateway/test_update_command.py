...
class TestHandleUpdateCommand:
    ...
    async def test_managed_install_returns_package_manager_guidance(self, monkeypatch):
        ...
        assert "brew upgrade sinoclaw-agent" in result
    ...
    async def test_no_git_directory(self, tmp_path):
        ...
        runner = _make_runner()
        event = _make_event()
        # Point _sinoclaw_home to tmp_path and project_root to a dir without .git
        fake_root = tmp_path / "project"
        fake_root.mkdir()
        with patch("gateway.run._sinoclaw_home", tmp_path), \
        ...
        with patch("gateway.run._sinoclaw_home", tmp_path):
            ...
            # project_root will be the real sinoclaw-agent dir (which HAS .git).
    ...
    async def test_no_sinoclaw_binary(self, tmp_path):
        ...
        assert "sinoclaw update" in result
    ...
    async def test_fallback_to_sys_executable(self, tmp_path):
        ...
        # The update_cmd uses sys.executable -m sinoclaw_cli.main
        joined = " ".join(call_args) if isinstance(call_args, list) else call_args
        assert "sinoclaw_cli.main" in joined or "bash" in call_args[0]
    ...
    async def test_resolve_sinoclaw_bin_fallback(self):
        ...
        assert result == [sys.executable, "-m", "sinoclaw_cli.main"]
    ...
    async def test_writes_pending_marker(self, tmp_path):
        ...
        with patch("gateway.run._sinoclaw_home", sinoclaw_home), \
             patch("gateway.run.__file__", fake_file), \
             patch("shutil.which", side_effect=lambda x: "/usr/bin/sinoclaw" if x == "sinoclaw" else "/usr/bin/setsid"), \
        ...
        assert not (sinoclaw_home / ".update_exit_code").exists()
    ...
    async def test_popen_failure_cleans_up(self, tmp_path):
        ...
        assert not (sinoclaw_home / ".update_pending.json").exists()
        assert not (sinoclaw_home / ".update_exit_code").exists()
...
class TestSendUpdateNotification:
    ...
    async def test_defers_notification_while_update_still_running(self, tmp_path):
        ...
        (sinoclaw_home / ".update_output.txt").write_text("still running")
    ...
    async def test_recovers_from_claimed_pending_file(self, tmp_path):
        ...
        claimed_path = sinoclaw_home / ".update_pending.claimed.json"
    ...
    async def test_truncates_long_output(self, tmp_path):
        ...
        pending = {"platform": "telegram", "chat_id": "111", "user_id": "222"}
        (sinoclaw_home / ".update_pending.json").write_text(json.dumps(pending))
        (sinoclaw_home / ".update_output.txt").write_text("x" * 5000)
        (sinoclaw_home / ".update_exit_code").write_text("0")
    ...
    async def test_sends_failure_message_when_update_fails(self, tmp_path):
        ...
        pending = {"platform": "telegram", "chat_id": "111", "user_id": "222"}
        (sinoclaw_home / ".update_pending.json").write_text(json.dumps(pending))
        (sinoclaw_home / ".update_output.txt").write_text("Traceback: boom")
