...
Guards against the 60-vs-500 bug where a stale `.env SINOCLAW_MAX_ITERATIONS=60`
...
def _run_gateway_import(sinoclaw_home: Path, initial_env: dict[str, str]) -> dict[str, str]:
    ...
    script = textwrap.dedent(
        ...
        for k in (
            ...
            "SINOCLAW_AGENT_TIMEOUT",
            "SINOCLAW_AGENT_TIMEOUT_WARNING",
            "SINOCLAW_GATEWAY_BUSY_INPUT_MODE",
            "SINOCLAW_TIMEZONE",
        ):
            v = os.environ.get(k)
            if v is not None:
