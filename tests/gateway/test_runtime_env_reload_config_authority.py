...
SINOCLAW_MAX_ITERATIONS, but a later per-turn load_dotenv(..., override=True)
can restore a stale .env SINOCLAW_MAX_ITERATIONS value before the next turn.
"""
...
def test_reload_runtime_env_preserves_config_max_turns(tmp_path: Path, monkeypatch) -> None:
    ...
    assert os.environ["SINOCLAW_MAX_ITERATIONS"] == "9000"
...
def test_reload_runtime_env_keeps_env_max_iterations_when_config_omits_key(
    ...
    assert os.environ["SINOCLAW_MAX_ITERATIONS"] == "123"
