    ...
    with patch.dict(
        "sys.modules",
        {
            "sinoclaw_constants": MagicMock(
                get_sinoclaw_home=MagicMock(return_value="/tmp/sinoclaw_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'sinoclaw-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'sinoclaw-release' patched"
    }
