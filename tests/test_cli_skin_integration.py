...
class TestCompactBannerSkinIntegration:
    def test_default_compact_banner_keeps_legacy_nous_sinoclaw_branding(self):
        set_active_skin("default")

        with patch("cli.shutil.get_terminal_size", return_value=SimpleNamespace(columns=90)), \
             patch("cli.format_banner_version_label", return_value="Sinoclaw Agent v0.1.0 (test)"):
            banner = _build_compact_banner()

        assert "NOUS HERMES" in banner

    def test_poseidon_compact_banner_uses_skin_branding_instead_of_nous_sinoclaw(self):
    ...
    def test_compact_banner_shows_version_label(self):
        ...
        with patch("cli.shutil.get_terminal_size", return_value=SimpleNamespace(columns=90)), \
             patch("cli.format_banner_version_label", return_value="Sinoclaw Agent v1.0 (test) · upstream abc12345"):
            banner = _build_compact_banner()
