...
def _clean_state():
    ...
    saved = {}
    for k in ("SINOCLAW_INTERACTIVE", "SINOCLAW_GATEWAY_SESSION", "SINOCLAW_EXEC_ASK", "HERMES_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["SINOCLAW_INTERACTIVE"] = "1"
        session_key = os.getenv("SINOCLAW_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No SINOCLAW_INTERACTIVE or SINOCLAW_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
