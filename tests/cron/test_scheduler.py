...
class TestRunJobSessionPersistence:
    ...
    def test_run_job_enabled_toolsets_resolves_from_platform_config_when_not_set(self, tmp_path):
        ...
        resolves them from ``sinoclaw tools`` platform config for ``cron``
        ...
        # The cron default is _SINOCLAW_CORE_TOOLS with _DEFAULT_OFF_TOOLSETS
    ...
    def test_run_job_per_job_toolsets_win_over_platform_config(self, tmp_path):
        ...
        over the platform-level ``sinoclaw tools`` config."""
        job = {
            "id": "override-job",
            "name": "test",
            "prompt": "hello",
        ...
        fake_db, patches = self._make_run_job_patches(tmp_path)
        # Even if the user has ``sinoclaw tools`` configured to enable web+file
    ...
    def test_run_job_sets_auto_delivery_env_from_dotenv_home_channel(self, tmp_path, monkeypatch):
        ...
        class FakeAgent:
            ...
            def run_conversation(self, *args, **kwargs):
                ...
                seen["platform"] = get_session_env("SINOCLAW_CRON_AUTO_DELIVER_PLATFORM") or None
                seen["chat_id"] = get_session_env("SINOCLAW_CRON_AUTO_DELIVER_CHAT_ID") or None
    ...
    def test_run_job_clears_stale_auto_delivery_thread_id_between_jobs(self, tmp_path, monkeypatch):
        ...
        class FakeAgent:
            ...
            def run_conversation(self, *args, **kwargs):
                ...
                seen.append(
                    {
                        "platform": get_session_env("SINOCLAW_CRON_AUTO_DELIVER_PLATFORM") or None,
                        "chat_id": get_session_env("SINOCLAW_CRON_AUTO_DELIVER_CHAT_ID") or None,
                        "thread_id": get_session_env("SINOCLAW_CRON_AUTO_DELIVER_THREAD_ID") or None,
                    }
                )
                return {"final_response": "ok"}
...
class TestRunJobConfigEnvVarExpansion:
    ...
    def test_model_env_ref_in_config_yaml_is_expanded(self, tmp_path, monkeypatch):
        ...
        (tmp_path / "config.yaml").write_text("model: ${_SINOCLAW_TEST_CRON_MODEL}\n")
        monkeypatch.setenv("_SINOCLAW_TEST_CRON_MODEL", "gpt-4o-mini-cron-test")
    ...
    def test_fallback_model_env_ref_in_config_yaml_is_expanded(self, tmp_path, monkeypatch):
        ...
        (tmp_path / "config.yaml").write_text(
            "fallback_providers:\n"
            "  - provider: openrouter\n"
            "    model: ${_SINOCLAW_TEST_CRON_FALLBACK}\n"
        )
        monkeypatch.setenv("_SINOCLAW_TEST_CRON_FALLBACK", "gpt-4o-fallback-test")
    ...
    def test_unexpanded_ref_passthrough_when_var_unset(self, tmp_path, monkeypatch):
        ...
        (tmp_path / "config.yaml").write_text("model: ${_SINOCLAW_TEST_CRON_UNSET_VAR}\n")
        monkeypatch.delenv("_SINOCLAW_TEST_CRON_UNSET_VAR", raising=False)
        ...
        assert kwargs["model"] == "${_SINOCLAW_TEST_CRON_UNSET_VAR}"
...
class TestParallelTick:
    ...
    def test_max_parallel_env_var(self, monkeypatch):
        """SINOCLAW_CRON_MAX_PARALLEL=1 should restore serial behaviour."""
        monkeypatch.setenv("SINOCLAW_CRON_MAX_PARALLEL", "1")
        call_times = []
