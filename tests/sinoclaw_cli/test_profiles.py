...
def profile_env(tmp_path, monkeypatch):
    ...
    * SINOCLAW_HOME  -> tmp_path/.hermes  (so get_sinoclaw_home() agrees)
...
class TestCreateProfile:
    ...
    def test_clone_all_excludes_default_infrastructure(self, profile_env):
        """--clone-all from default profile excludes sinoclaw-agent, .worktrees,
        ...
        (default_home / "sinoclaw-agent" / ".git").mkdir(parents=True)
        (default_home / "sinoclaw-agent" / "venv" / "bin").mkdir(parents=True)
        (default_home / "sinoclaw-agent" / "README.md").write_text("repo")
        ...
        assert not (profile_dir / "sinoclaw-agent").exists()
...
class TestNoSkillsOptOut:
    ...
    def test_seed_profile_skills_respects_marker(self, profile_env):
        ...
        called directly (e.g. by `sinoclaw update`'s all-profile sync loop)."""
        profile_dir = create_profile("orchestrator", no_alias=True, no_skills=True)
...
class TestGetActiveProfileName:
    ...
    def test_default_sinoclaw_home_returns_default(self, profile_env):
        # SINOCLAW_HOME points to tmp_path/.hermes which is the default
