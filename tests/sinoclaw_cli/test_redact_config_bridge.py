...
`SINOCLAW_REDACT_SECRETS` at module-import time. `sinoclaw_cli/main.py` at
...
env var in `sinoclaw_cli/main.py` BEFORE the `setup_logging()` call.
"""
import os
import subprocess
import sys
...
def test_redact_secrets_false_in_config_yaml_is_honored(tmp_path):
    ...
    # Spawn a fresh Python process that imports sinoclaw_cli.main and checks
    # _REDACT_ENABLED. Must be a subprocess — we need a clean module state.
    probe = textwrap.dedent(
        """\
        import sys, os
        # Make absolutely sure the env var is not pre-set
        os.environ.pop("SINOCLAW_REDACT_SECRETS", None)
        sys.path.insert(0, %r)
        import sinoclaw_cli.main  # triggers the bridge + setup_logging
        import agent.redact
        print(f"REDACT_ENABLED={agent.redact._REDACT_ENABLED}")
        print(f"ENV_VAR={os.environ.get('SINOCLAW_REDACT_SECRETS', '<unset>')}")
        """
    ) % str(REPO_ROOT)

    env = dict(os.environ)
    env["SINOCLAW_HOME"] = str(sinoclaw_home)
    env.pop("SINOCLAW_REDACT_SECRETS", None)
...
def test_dotenv_redact_secrets_beats_config_yaml(tmp_path):
    ...
    (sinoclaw_home / ".env").write_text("SINOCLAW_REDACT_SECRETS=true\n")
