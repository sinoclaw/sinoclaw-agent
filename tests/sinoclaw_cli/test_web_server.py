...
class TestWebServerEndpoints:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_sinoclaw_home):
        """Create a TestClient and isolate the state DB under the test SINOCLAW_HOME."""
        try:
        ...
        from sinoclaw_cli.web_server import app, _SESSION_HEADER_NAME, _SESSION_TOKEN

        monkeypatch.setattr(sinoclaw_state, "DEFAULT_DB_PATH", get_sinoclaw_home() / "state.db")
    ...
    def test_reveal_env_var(self, tmp_path):
        ...
        from sinoclaw_cli.web_server import _SESSION_HEADER_NAME, _SESSION_TOKEN
    ...
    def test_reveal_env_var_bad_token(self, tmp_path):
        ...
        from sinoclaw_cli.config import save_env_value
        from sinoclaw_cli.web_server import _SESSION_HEADER_NAME
...
class TestNewEndpoints:
    ...
    @pytest.fixture(autouse=True)
    def _setup(self, monkeypatch, _isolate_sinoclaw_home):
        try:
            from starlette.testclient import TestClient
        except ImportError:
            pytest.skip("fastapi/starlette not installed")
    ...
    def test_profiles_list_includes_default(self):
        from sinoclaw_constants import get_sinoclaw_home
        get_sinoclaw_home().mkdir(parents=True, exist_ok=True)
    ...
    def test_profiles_list_falls_back_when_profile_listing_fails(self, monkeypatch):
        from sinoclaw_constants import get_sinoclaw_home
        import sinoclaw_cli.profiles as profiles_mod
        ...
        named = sinoclaw_home / "profiles" / "multi-agent"
    ...
    def test_profile_setup_command_uses_named_profile_wrapper(self):
        ...
        (get_sinoclaw_home() / "profiles" / "coder").mkdir(parents=True)
    ...
    def test_profiles_create_with_clone_from_default_copies_default_skills(self, monkeypatch):
        ...
        default_skill = get_sinoclaw_home() / "skills" / "custom" / "new-skill"
        ...
        assert resp.status_code == 200
        cloned_skill = get_sinoclaw_home() / "profiles" / "cloned" / "skills" / "custom" / "new-skill" / "SKILL.md"
    ...
    def test_profiles_create_without_clone_seeds_bundled_skills(self, monkeypatch):
        ...
        assert resp.status_code == 200
        seeded_skill = get_sinoclaw_home() / "profiles" / "fresh" / "skills" / "software-development" / "plan" / "SKILL.md"
...
class TestNormaliseThemeDefinition:
    ...
    def test_rejects_missing_name(self):
        from sinoclaw_cli.web_server import _normalise_theme_definition
...
class TestDiscoverUserThemes:
    """Tests for _discover_user_themes() — scans ~/.sinoclaw/dashboard-themes/."""

    def test_returns_empty_when_dir_missing(self, tmp_path, monkeypatch):
        monkeypatch.setenv("SINOCLAW_HOME", str(tmp_path))
        from sinoclaw_cli import web_server
...
class TestPtyWebSocket:
    ...
    def test_streams_child_stdout_to_client(self, monkeypatch):
        ...
        with self.client.websocket_connect(self._url()) as conn:
            ...
            while time.monotonic() < deadline:
                ...
                if frame:
                    buf += frame
                if b"sinoclaw-ws-ok" in buf:
                    break
            assert b"sinoclaw-ws-ok" in buf
    ...
    def test_unavailable_platform_closes_with_message(self, monkeypatch):
        from sinoclaw_cli.pty_bridge import PtyUnavailableError
        ...
        import sinoclaw_cli.web_server as ws_mod
    ...
    def test_channel_param_propagates_sidecar_url(self, monkeypatch):
        ...
        SINOCLAW_TUI_SIDECAR_URL env var pointing back at /api/pub on the
    ...
    def test_pub_broadcasts_to_events_subscribers(self, monkeypatch):
        ...
        import time
        from urllib.parse import urlencode
        from sinoclaw_cli import web_server as ws_mod
