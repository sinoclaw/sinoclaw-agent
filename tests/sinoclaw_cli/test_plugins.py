...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    ``<sinoclaw_home>/config.yaml`` under ``plugins.enabled``. Plugins are
    ...
    *base* is expected to be ``<sinoclaw_home>/plugins/``; we derive
    ``<sinoclaw_home>`` from it by walking one level up.
    """
    ...
    if auto_enable:
        # Write/merge plugins.enabled in <SINOCLAW_HOME>/config.yaml.
        # Config is always read from SINOCLAW_HOME (not from the project
        # dir for project plugins), so that's where we opt in.
        import os
        sinoclaw_home_str = os.environ.get("SINOCLAW_HOME")
        if sinoclaw_home_str:
            sinoclaw_home = Path(sinoclaw_home_str)
        else:
            sinoclaw_home = base.parent
...
class TestPluginDiscovery:
    ...
    def test_discover_user_plugins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SINOCLAW_HOME", str(tmp_path / "sinoclaw_test"))
...
class TestPluginLoading:
    ...
    def test_load_missing_init(self, tmp_path, monkeypatch):
        ...
        # missing-init error.
        sinoclaw_home = tmp_path / "sinoclaw_test"
...
class TestPluginCommands:
    ...
    def test_get_plugin_context_engine_discovers_plugins_lazily(self, tmp_path, monkeypatch):
        ...
        sinoclaw_home = tmp_path / "sinoclaw_test"
        plugins_dir = sinoclaw_home / "plugins"
    ...
    def test_commands_in_list_plugins_output(self, tmp_path, monkeypatch):
        ...
        plugins_dir = tmp_path / "sinoclaw_test" / "plugins"
        # Set SINOCLAW_HOME BEFORE _make_plugin_dir so auto-enable targets
...
class TestPluginCommandResultResolution:
    ...
    def test_awaits_async_result_with_running_loop(self, monkeypatch):
        ...
        monkeypatch.setattr("sinoclaw_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
    ...
    def test_running_loop_timeout_does_not_hang_forever(self, monkeypatch):
        ...
        monkeypatch.setattr("sinoclaw_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
        monkeypatch.setattr("sinoclaw_cli.plugins._PLUGIN_COMMAND_AWAIT_TIMEOUT_SECS", 0.1)
