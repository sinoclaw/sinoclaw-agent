"""Tests for the Kanban DB layer (sinoclaw_cli.kanban_db)."""
...
def test_has_spawnable_ready_true_when_real_profile_present(kanban_home, monkeypatch):
    ...
    with kb.connect() as conn:
        ...
        kb.create_task(conn, title="sinoclaw-task", assignee="daily")
...
def test_scratch_workspace_created_under_sinoclaw_home(kanban_home):
    with kb.connect() as conn:
        t = kb.create_task(conn, title="x")
        task = kb.get_task(conn, t)
...
class TestSharedBoardPaths:
    ...
    must anchor at the **shared root**, not the active profile's SINOCLAW_HOME."""

    def _set_home(self, monkeypatch, tmp_path, sinoclaw_home):
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SINOCLAW_HOME", str(sinoclaw_home))
        monkeypatch.delenv("SINOCLAW_KANBAN_HOME", raising=False)
    ...
    def test_profile_worker_resolves_to_shared_root(
        ...
        # Reproduces the bug: dispatcher uses ~/.sinoclaw/kanban.db,
    ...
    def test_docker_custom_sinoclaw_home_uses_env_path_directly(
        self, tmp_path, monkeypatch
    ):
    ...
    def test_docker_profile_layout_uses_grandparent(
        ...
        # Docker profile shape: SINOCLAW_HOME=/opt/hermes/profiles/coder;
    ...
    def test_explicit_override_via_sinoclaw_kanban_home(
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SINOCLAW_HOME", str(profile_home))
        monkeypatch.setenv("SINOCLAW_KANBAN_HOME", str(override))
    ...
    def test_empty_override_falls_through(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SINOCLAW_KANBAN_HOME", "   ")
    ...
    def test_dispatcher_and_worker_share_a_real_database(
        ...
        # Worker switches to the profile SINOCLAW_HOME and reads.
        monkeypatch.setenv("SINOCLAW_HOME", str(profile_home))
        with kb.connect() as conn:
    ...
    def test_sinoclaw_kanban_db_pin_beats_kanban_home(
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SINOCLAW_HOME", str(default_home))
        monkeypatch.setenv("SINOCLAW_KANBAN_HOME", str(umbrella))
        monkeypatch.setenv("SINOCLAW_KANBAN_DB", str(pinned_db))

        assert kb.kanban_db_path() == pinned_db
        # workspaces_root still follows SINOCLAW_KANBAN_HOME -- the pins
        # are independent.
    ...
    def test_sinoclaw_kanban_workspaces_root_pin_beats_kanban_home(
        ...
        monkeypatch.setenv("SINOCLAW_HOME", str(default_home))
        monkeypatch.setenv("SINOCLAW_KANBAN_HOME", str(umbrella))
        monkeypatch.setenv("SINOCLAW_KANBAN_WORKSPACES_ROOT", str(pinned_ws))

        assert kb.workspaces_root() == pinned_ws
        # kanban_db_path still follows SINOCLAW_KANBAN_HOME.
    ...
    def test_empty_per_path_overrides_fall_through(
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SINOCLAW_HOME", str(default_home))
        monkeypatch.setenv("SINOCLAW_KANBAN_DB", "   ")
        monkeypatch.setenv("SINOCLAW_KANBAN_WORKSPACES_ROOT", "")
    ...
    def test_dispatcher_spawn_injects_kanban_db_and_workspaces_root(
        ...
        # The dispatcher's `_default_spawn` must inject SINOCLAW_KANBAN_DB
        ...
        env = captured["env"]
        assert env["SINOCLAW_KANBAN_DB"] == str(default_home / "kanban.db")
        assert env["SINOCLAW_KANBAN_WORKSPACES_ROOT"] == str(
            default_home / "kanban" / "workspaces"
        )
        assert env["SINOCLAW_KANBAN_TASK"] == "t_dispatch_env"
...
def test_connect_falls_back_to_delete_on_locking_protocol(kanban_home, caplog):
    ...
    (see sinoclaw-agent issue #22032).
    """
    ...
    with _patch("sinoclaw_cli.kanban_db.sqlite3.connect", side_effect=wal_blocking_connect):
