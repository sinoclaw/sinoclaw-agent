...
def test_visible_providers_include_nous_subscription_when_logged_in(monkeypatch):
    monkeypatch.setattr("sinoclaw_cli.tools_config.managed_nous_tools_enabled", lambda: True)
...
def test_visible_providers_hide_nous_subscription_when_feature_flag_is_off(monkeypatch):
    monkeypatch.setattr("sinoclaw_cli.tools_config.managed_nous_tools_enabled", lambda: False)
    config = {"model": {"provider": "nous"}}
...
def test_reconfigure_lists_enabled_web_without_existing_provider_config(monkeypatch):
    ...
    monkeypatch.setattr(
        "sinoclaw_cli.tools_config._toolset_has_keys",
    ...
    monkeypatch.setattr("sinoclaw_cli.tools_config._prompt_choice", fake_prompt_choice)
    monkeypatch.setattr(
        "sinoclaw_cli.tools_config._configure_tool_category_for_reconfig",
...
class TestImagegenBackendRegistry:
    ...
    def test_fal_backend_registered(self):
        from sinoclaw_cli.tools_config import IMAGEGEN_BACKENDS
    ...
    def test_image_gen_providers_tagged_with_fal_backend(self):
        ...
        from sinoclaw_cli.tools_config import TOOL_CATEGORIES
...
class TestImagegenModelPicker:
    ...
    def test_picker_writes_chosen_model_to_config(self):
        from sinoclaw_cli.tools_config import _configure_imagegen_model
        config = {}
        # Force _prompt_choice to pick index 1 (second-in-ordered-list).
        with patch("sinoclaw_cli.tools_config._prompt_choice", return_value=1):
    ...
    def test_picker_with_gpt_image_does_not_prompt_quality(self):
        ...
        config = {}
        with patch("sinoclaw_cli.tools_config._prompt_choice", side_effect=fake_prompt):
            _configure_imagegen_model("fal", config)
    ...
    def test_picker_repairs_corrupt_config_section(self):
        ...
        with patch("sinoclaw_cli.tools_config._prompt_choice", return_value=0):
            _configure_imagegen_model("fal", config)
...
def test_get_platform_tools_recovers_non_configurable_toolsets_from_composite():
    ...
    fake_toolsets["sinoclaw-_test_platform"] = {
    ...
    test_platforms = {
        "_test_platform": {"label": "Test", "default_toolset": "sinoclaw-_test_platform"},
    }

    with mock_patch("sinoclaw_cli.tools_config.PLATFORMS", {**PLATFORMS, **test_platforms}):
...
def test_get_platform_tools_second_pass_skips_fully_claimed_toolsets():
    ...
    be added by the second pass (prevents 'search', 'sinoclaw-acp' noise).
    """
...
def test_discord_toolsets_not_available_on_other_platforms():
    ...
    from sinoclaw_cli.tools_config import _toolset_allowed_for_platform
...
def test_save_platform_tools_strips_restricted_toolsets():
    ...
    from sinoclaw_cli.tools_config import _save_platform_tools
    config = {}
