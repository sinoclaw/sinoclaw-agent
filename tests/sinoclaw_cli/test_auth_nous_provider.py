...
def test_get_nous_auth_status_prefers_runtime_auth_store_over_stale_pool(tmp_path, monkeypatch):
    ...
    sinoclaw_home = tmp_path / "hermes"
    _setup_nous_auth(sinoclaw_home, access_token="at-fresh")
    monkeypatch.setenv("SINOCLAW_HOME", str(sinoclaw_home))
...
def test_get_nous_auth_status_reports_revoked_refresh_session(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("sinoclaw_cli.auth.resolve_nous_runtime_credentials", _boom)
...
class TestLoginNousSkipKeepsCurrent:
    ...
    def _setup_home_with_openrouter(self, tmp_path, monkeypatch):
        ...
        auth_path = sinoclaw_home / "auth.json"
        auth_path.write_text(json.dumps({
            "version": 1,
            "active_provider": "openrouter",
            "providers": {"openrouter": {"api_key": "sk-or-fake"}},
        }))
        return sinoclaw_home, config_path, auth_path
    ...
    def _patch_login_internals(self, monkeypatch, *, prompt_returns):
        ...
        import sinoclaw_cli.models as models_mod
        import sinoclaw_cli.nous_subscription as ns
    ...
    def test_skip_keep_current_preserves_provider_and_model(self, tmp_path, monkeypatch):
        ...
        import argparse
        import yaml
        from sinoclaw_cli.auth import PROVIDER_REGISTRY, _login_nous

        sinoclaw_home, config_path, auth_path = self._setup_home_with_openrouter(
...
def test_persist_nous_credentials_writes_both_pool_and_providers(tmp_path, monkeypatch):
    ...
    """
    from sinoclaw_cli.auth import persist_nous_credentials, NOUS_DEVICE_CODE_SOURCE
    ...
    payload = json.loads((sinoclaw_home / "auth.json").read_text())
...
def test_persist_nous_credentials_allows_recovery_from_401(tmp_path, monkeypatch):
    ...
    """
    from sinoclaw_cli.auth import persist_nous_credentials, resolve_nous_runtime_credentials
...
def test_refresh_token_reuse_detection_surfaces_actionable_message():
    ...
    """
    from sinoclaw_cli.auth import _refresh_access_token
...
def shared_store_env(tmp_path, monkeypatch):
    """Redirect SINOCLAW_SHARED_AUTH_DIR to a tmp_path.
    ...
    monkeypatch.setenv("SINOCLAW_SHARED_AUTH_DIR", str(shared_dir))
    return shared_dir


def test_shared_store_seat_belt_refuses_real_home_under_pytest(monkeypatch):
    """Without SINOCLAW_SHARED_AUTH_DIR override, the seat belt must trip.
    ...
    writing to the user's real ``~/.sinoclaw/shared/`` across CI runs.
    """
    from sinoclaw_cli.auth import _nous_shared_store_path

    monkeypatch.delenv("SINOCLAW_SHARED_AUTH_DIR", raising=False)
...
def test_shared_store_honors_env_override(tmp_path, monkeypatch):
    """SINOCLAW_SHARED_AUTH_DIR must redirect the path."""
    from sinoclaw_cli.auth import _nous_shared_store_path, NOUS_SHARED_STORE_FILENAME

    custom_dir = tmp_path / "custom_shared"
    monkeypatch.setenv("SINOCLAW_SHARED_AUTH_DIR", str(custom_dir))
...
def test_shared_store_read_missing_returns_none(shared_store_env):
    ...
    from sinoclaw_cli.auth import _read_shared_nous_state
...
def test_shared_store_read_malformed_returns_none(shared_store_env):
    ...
    from sinoclaw_cli.auth import _nous_shared_store_path, _read_shared_nous_state
...
def test_shared_store_write_skips_when_refresh_token_missing(shared_store_env):
    ...
    from sinoclaw_cli.auth import _nous_shared_store_path, _write_shared_nous_state
...
def test_try_import_shared_returns_none_when_store_missing(shared_store_env):
    ...
    from sinoclaw_cli.auth import _try_import_shared_nous_state
...
def test_try_import_shared_returns_none_on_refresh_failure(
    ...
    """
    from sinoclaw_cli import auth as auth_mod
...
def test_try_import_shared_rehydrates_on_success(shared_store_env, monkeypatch):
    ...
    assert result["client_id"] == "sinoclaw-cli"
...
def test_shared_store_survives_across_profile_switch(
    ...
    (different SINOCLAW_HOME) sees the same shared state and can rehydrate
    ...
    monkeypatch.setenv("SINOCLAW_HOME", str(profile_a))
    ...
    # Profile B: fresh SINOCLAW_HOME, no auth yet, but the shared store
    ...
    monkeypatch.setenv("SINOCLAW_HOME", str(profile_b))
