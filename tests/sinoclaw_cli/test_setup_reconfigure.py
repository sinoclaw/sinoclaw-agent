...
- Bare `sinoclaw setup` drops straight into the full reconfigure wizard
  (every prompt shows the current value as its default).
- `sinoclaw setup --quick` runs the narrower "fill in missing items" flow.
- `sinoclaw setup --reconfigure` is a backwards-compat alias for the
  bare-setup default.
...
def _enter_existing_install_patches(stack, **extra):
    ...
    for target, kwargs in [
        ("sinoclaw_cli.setup.ensure_sinoclaw_home", {}),
        ("sinoclaw_cli.setup.is_interactive_stdin", {"return_value": True}),
        ("sinoclaw_cli.config.is_managed", {"return_value": False}),
        ("sinoclaw_cli.setup.load_config", {"return_value": {}}),
        ("sinoclaw_cli.setup.save_config", {}),
        ("sinoclaw_cli.setup.get_env_value", {"return_value": None}),
        ("sinoclaw_cli.auth.get_active_provider", {"return_value": "openrouter"}),
        ("sinoclaw_cli.setup._print_setup_summary", {}),
        ("sinoclaw_cli.setup._offer_launch_chat", {}),
        ("sinoclaw_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
...
def _enter_fresh_install_patches(stack, **extra):
    for target, kwargs in [
        ...
        ("sinoclaw_cli.setup.load_config", {"return_value": {}}),
        ("sinoclaw_cli.setup.save_config", {}),
        ("sinoclaw_cli.auth.get_active_provider", {"return_value": None}),
        ("sinoclaw_cli.setup.get_env_value", {"return_value": None}),
        ("sinoclaw_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
        stack.enter_context(patch(target, **kwargs))
...
class TestExistingInstallDefault:
    """Bare `sinoclaw setup` on an existing install = full reconfigure wizard."""
    ...
    def test_bare_setup_runs_full_reconfigure_without_menu(self, existing_install):
        ...
        with ExitStack() as stack:
            m = _enter_existing_install_patches(
                stack,
                prompt_choice="sinoclaw_cli.setup.prompt_choice",
                quick="sinoclaw_cli.setup._run_quick_setup",
                model="sinoclaw_cli.setup.setup_model_provider",
                terminal="sinoclaw_cli.setup.setup_terminal_backend",
                agent="sinoclaw_cli.setup.setup_agent_settings",
                gateway="sinoclaw_cli.setup.setup_gateway",
                tools="sinoclaw_cli.setup.setup_tools",
            )
    ...
    def test_reconfigure_flag_is_backwards_compat_noop(self, existing_install):
        """`sinoclaw setup --reconfigure` behaves the same as bare `sinoclaw setup`."""
    ...
    def test_bare_setup_runs_first_time_flow(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("sinoclaw_cli.setup.prompt_choice", {"return_value": 0}),
                first="sinoclaw_cli.setup._run_first_time_quick_setup",
            )
            from sinoclaw_cli.setup import run_setup_wizard
            run_setup_wizard(args)
...
class TestArgparse:
    ...
    def test_reconfigure_flag_reaches_cmd_setup(self, monkeypatch):
        import sys
        from sinoclaw_cli.main import main

        captured = {}
        monkeypatch.setattr(
            "sinoclaw_cli.setup.run_setup_wizard",
