...
def _make_sinoclaw_tree(root: Path) -> None:
    ...
    # sinoclaw-agent repo (should be EXCLUDED)
    (root / "sinoclaw-agent").mkdir(exist_ok=True)
    (root / "sinoclaw-agent" / "run_agent.py").write_text("# big file\n")
    (root / "sinoclaw-agent" / ".git").mkdir()
    (root / "sinoclaw-agent" / ".git" / "HEAD").write_text("ref: refs/heads/main\n")
...
class TestShouldExclude:
    def test_excludes_sinoclaw_agent(self):
        from sinoclaw_cli.backup import _should_exclude
        assert _should_exclude(Path("sinoclaw-agent/run_agent.py"))
        assert _should_exclude(Path("sinoclaw-agent/.git/HEAD"))
...
class TestBackup:
    def test_creates_zip(self, tmp_path, monkeypatch):
        ...
        sinoclaw_home.mkdir()
        _make_sinoclaw_tree(sinoclaw_home)
    ...
    def test_excludes_sinoclaw_agent(self, tmp_path, monkeypatch):
        """Backup does NOT include sinoclaw-agent/ directory."""
        sinoclaw_home = tmp_path / ".sinoclaw"
        sinoclaw_home.mkdir()
        _make_sinoclaw_tree(sinoclaw_home)
        ...
        with zipfile.ZipFile(out_zip, "r") as zf:
            names = zf.namelist()
            agent_files = [n for n in names if "sinoclaw-agent" in n]
            assert agent_files == [], f"sinoclaw-agent files leaked into backup: {agent_files}"
    ...
    def test_default_output_path(self, tmp_path, monkeypatch):
        ...
        zips = list(tmp_path.glob("sinoclaw-backup-*.zip"))
        assert len(zips) == 1
...
class TestImport:
    ...
    def test_restores_files(self, tmp_path, monkeypatch):
        ...
        assert (sinoclaw_home / "config.yaml").read_text() == "model:\n  provider: openrouter\n"
        assert (sinoclaw_home / ".env").read_text() == "OPENROUTER_API_KEY=sk-test\n"
        assert (sinoclaw_home / "skills" / "my-skill" / "SKILL.md").read_text() == "# My Skill\n"
        assert (sinoclaw_home / "profiles" / "coder" / "config.yaml").exists()
    ...
    def test_strips_sinoclaw_prefix(self, tmp_path, monkeypatch):
        ...
        assert (sinoclaw_home / "config.yaml").read_text() == "model: test\n"
        assert (sinoclaw_home / "skills" / "a" / "SKILL.md").read_text() == "# A\n"
    ...
    def test_confirmation_prompt_abort(self, tmp_path, monkeypatch):
        ...
        (sinoclaw_home / "config.yaml").write_text("existing: true\n")
        monkeypatch.setenv("SINOCLAW_HOME", str(sinoclaw_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (sinoclaw_home / "config.yaml").read_text() == "existing: true\n"
    ...
    def test_force_skips_confirmation(self, tmp_path, monkeypatch):
        ...
        assert (sinoclaw_home / "config.yaml").read_text() == "model: restored\n"
    ...
    def test_restores_secret_files_with_0600_perms(self, tmp_path, monkeypatch):
        ...
        for rel in (".env", "auth.json", "state.db", "profiles/coder/.env"):
            mode = (sinoclaw_home / rel).stat().st_mode & 0o777
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SINOCLAW_HOME", str(src_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "source")

        # Backup
        out_zip = tmp_path / "roundtrip.zip"
        from sinoclaw_cli.backup import run_backup, run_import
        ...
        monkeypatch.setenv("SINOCLAW_HOME", str(dst_home))
        ...
        # sinoclaw-agent should NOT be present
        assert not (dst_home / "sinoclaw-agent").exists()
...
class TestFormatSize:
    def test_bytes(self):
        from sinoclaw_cli.backup import _format_size
...
class TestValidation:
    ...
    def test_detect_prefix_hermes(self):
        ...
        import io
        from sinoclaw_cli.backup import _detect_prefix
...
class TestBackupEdgeCases:
    ...
    def test_output_is_directory(self, tmp_path, monkeypatch):
        ...
        zips = list(out_dir.glob("sinoclaw-backup-*.zip"))
        assert len(zips) == 1
    ...
    def test_empty_sinoclaw_home(self, tmp_path, monkeypatch):
        ...
        (sinoclaw_home / "__pycache__").mkdir()
        (sinoclaw_home / "__pycache__" / "foo.pyc").write_bytes(b"\x00")
    ...
    def test_permission_error_during_backup(self, tmp_path, monkeypatch):
        ...
        bad_file = sinoclaw_home / "secret.db"
    ...
    def test_pre1980_timestamp_skipped(self, tmp_path, monkeypatch):
        ...
        old_file = sinoclaw_home / "ancient.txt"
...
class TestImportEdgeCases:
    ...
    def test_eof_during_confirmation(self, tmp_path, monkeypatch):
        ...
        sinoclaw_home = tmp_path / ".sinoclaw"
        sinoclaw_home.mkdir()
        (sinoclaw_home / "config.yaml").write_text("existing\n")
        monkeypatch.setenv("SINOCLAW_HOME", str(sinoclaw_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_keyboard_interrupt_during_confirmation(self, tmp_path, monkeypatch):
        ...
        sinoclaw_home = tmp_path / ".sinoclaw"
        sinoclaw_home.mkdir()
        (sinoclaw_home / ".env").write_text("KEY=val\n")
        monkeypatch.setenv("SINOCLAW_HOME", str(sinoclaw_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_permission_error_during_import(self, tmp_path, monkeypatch):
        ...
        locked_dir = sinoclaw_home / "locked"
        locked_dir.mkdir()
    ...
    def test_progress_with_many_files(self, tmp_path, monkeypatch):
        ...
        assert (sinoclaw_home / "config.yaml").exists()
        assert (sinoclaw_home / "sessions" / "s0599.json").exists()
...
class TestProfileRestoration:
    ...
    def test_import_creates_profile_wrappers(self, tmp_path, monkeypatch):
        ...
        assert (sinoclaw_home / "profiles" / "coder" / "config.yaml").exists()
        assert (sinoclaw_home / "profiles" / "researcher" / "config.yaml").exists()
    ...
    def test_import_without_profiles_module(self, tmp_path, monkeypatch):
        ...
        import sinoclaw_cli.backup as backup_mod
        original_import = __builtins__.__import__ if hasattr(__builtins__, '__import__') else __import__

        def fake_import(name, *a, **kw):
            if name == "sinoclaw_cli.profiles":
...
class TestSafeCopyDb:
    def test_copies_valid_database(self, tmp_path):
        from sinoclaw_cli.backup import _safe_copy_db
...
class TestQuickSnapshot:
    ...
    def test_creates_snapshot(self, sinoclaw_home):
        from sinoclaw_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(sinoclaw_home=sinoclaw_home)
        assert snap_id is not None
        snap_dir = sinoclaw_home / "state-snapshots" / snap_id
        assert snap_dir.is_dir()
        assert (snap_dir / "manifest.json").exists()

    def test_label_in_id(self, sinoclaw_home):
        from sinoclaw_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(label="before-upgrade", sinoclaw_home=sinoclaw_home)
        assert "before-upgrade" in snap_id

    def test_state_db_safely_copied(self, sinoclaw_home):
        from sinoclaw_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(sinoclaw_home=sinoclaw_home)
        db_copy = sinoclaw_home / "state-snapshots" / snap_id / "state.db"
    ...
    def test_copies_nested_files(self, sinoclaw_home):
        from sinoclaw_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(sinoclaw_home=sinoclaw_home)
        assert (sinoclaw_home / "state-snapshots" / snap_id / "cron" / "jobs.json").exists()

    def test_missing_files_skipped(self, sinoclaw_home):
        from sinoclaw_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(sinoclaw_home=sinoclaw_home)
        with open(sinoclaw_home / "state-snapshots" / snap_id / "manifest.json") as f:
            meta = json.load(f)
    ...
    def test_empty_home_returns_none(self, tmp_path):
        ...
        empty.mkdir()
        assert create_quick_snapshot(sinoclaw_home=empty) is None

    def test_list_snapshots(self, sinoclaw_home):
        from sinoclaw_cli.backup import create_quick_snapshot, list_quick_snapshots
        id1 = create_quick_snapshot(label="first", sinoclaw_home=sinoclaw_home)
        id2 = create_quick_snapshot(label="second", sinoclaw_home=sinoclaw_home)

        snaps = list_quick_snapshots(sinoclaw_home=sinoclaw_home)
        assert len(snaps) == 2
        assert snaps[0]["id"] == id2  # most recent first
        assert snaps[1]["id"] == id1

    def test_list_limit(self, sinoclaw_home):
        from sinoclaw_cli.backup import create_quick_snapshot, list_quick_snapshots
        for i in range(5):
            create_quick_snapshot(label=f"s{i}", sinoclaw_home=sinoclaw_home)
        snaps = list_quick_snapshots(limit=3, sinoclaw_home=sinoclaw_home)
        assert len(snaps) == 3

    def test_restore_config(self, sinoclaw_home):
        from sinoclaw_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(sinoclaw_home=sinoclaw_home)

        (sinoclaw_home / "config.yaml").write_text("model:\n  provider: anthropic\n")
        assert "anthropic" in (sinoclaw_home / "config.yaml").read_text()

        result = restore_quick_snapshot(snap_id, sinoclaw_home=sinoclaw_home)
        assert result is True
        assert "openrouter" in (sinoclaw_home / "config.yaml").read_text()

    def test_restore_state_db(self, sinoclaw_home):
        from sinoclaw_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(sinoclaw_home=sinoclaw_home)

        conn = sqlite3.connect(str(sinoclaw_home / "state.db"))
        conn.execute("INSERT INTO sessions VALUES ('s2', 'new')")
        conn.commit()
        conn.close()

        restore_quick_snapshot(snap_id, sinoclaw_home=sinoclaw_home)
    ...
    def test_restore_nonexistent(self, sinoclaw_home):
        from sinoclaw_cli.backup import restore_quick_snapshot
        assert restore_quick_snapshot("nonexistent", sinoclaw_home=sinoclaw_home) is False

    def test_auto_prune(self, sinoclaw_home):
        from sinoclaw_cli.backup import create_quick_snapshot, list_quick_snapshots, _QUICK_DEFAULT_KEEP
        for i in range(_QUICK_DEFAULT_KEEP + 5):
            create_quick_snapshot(label=f"snap-{i:03d}", sinoclaw_home=sinoclaw_home)
        snaps = list_quick_snapshots(limit=100, sinoclaw_home=sinoclaw_home)
        assert len(snaps) <= _QUICK_DEFAULT_KEEP

    def test_manual_prune(self, sinoclaw_home):
        from sinoclaw_cli.backup import create_quick_snapshot, prune_quick_snapshots, list_quick_snapshots
        for i in range(10):
            create_quick_snapshot(label=f"s{i}", sinoclaw_home=sinoclaw_home)
        deleted = prune_quick_snapshots(keep=3, sinoclaw_home=sinoclaw_home)
        assert deleted == 7
        assert len(list_quick_snapshots(sinoclaw_home=sinoclaw_home)) == 3

    def test_snapshot_includes_pairing_directories(self, sinoclaw_home):
        ...
        (sinoclaw_home / "platforms" / "pairing").mkdir(parents=True)
        (sinoclaw_home / "platforms" / "pairing" / "telegram-approved.json").write_text(
            '{"12345": {"user_name": "alice"}}'
        )
        (sinoclaw_home / "platforms" / "pairing" / "discord-approved.json").write_text(
            '{"67890": {"user_name": "bob"}}'
        )
        # Legacy pairing store (old location)
        (sinoclaw_home / "pairing").mkdir()
        (sinoclaw_home / "pairing" / "matrix-approved.json").write_text(
            '{"@charlie:server": {"user_name": "charlie"}}'
        )
        # Feishu's separate JSON
        (sinoclaw_home / "feishu_comment_pairing.json").write_text(
    ...
    def test_restore_recovers_pairing_data(self, sinoclaw_home):
        ...
        assert restore_quick_snapshot(snap_id, sinoclaw_home=sinoclaw_home) is True
        assert approved.exists()
        assert '"alice"' in approved.read_text()
        assert feishu.exists()
        assert '"user_xyz"' in feishu.read_text()

    def test_empty_pairing_dir_does_not_fail(self, sinoclaw_home):
...
class TestPreUpdateBackup:
    ...
    def test_backup_contents_match_full_backup(self, sinoclaw_home):
        ...
        assert "skills/my-skill/SKILL.md" in names
        assert "profiles/coder/config.yaml" in names
        # sinoclaw-agent repo excluded
        assert not any(n.startswith("sinoclaw-agent/") for n in names)
        # __pycache__ excluded
        assert not any("__pycache__" in n for n in names)
        # pid files excluded
        assert "gateway.pid" not in names

    def test_does_not_recurse_into_prior_backups(self, sinoclaw_home):
        """The ``backups/`` directory must be excluded so that each backup
        doesn't grow exponentially by including all prior backups."""
        from sinoclaw_cli.backup import create_pre_update_backup
        # First backup
        out1 = create_pre_update_backup(sinoclaw_home=sinoclaw_home)
        assert out1 is not None
        # Second backup — must not include the first
        out2 = create_pre_update_backup(sinoclaw_home=sinoclaw_home)
    ...
    def test_rotation_keeps_only_n(self, sinoclaw_home):
        ...
        created = []
        for _ in range(5):
            out = create_pre_update_backup(sinoclaw_home=sinoclaw_home, keep=3)
            created.append(out)
            _t.sleep(1.05)  # ensure distinct seconds in timestamp

        remaining = sorted(
            p.name for p in (sinoclaw_home / "backups").iterdir()
    ...
    def test_rotation_preserves_manual_files(self, sinoclaw_home):
        """Hand-dropped zips in ``backups/`` must not be touched by
        rotation — it only prunes files matching ``pre-update-*.zip``."""
        import time as _t
        from sinoclaw_cli.backup import create_pre_update_backup

        (sinoclaw_home / "backups").mkdir(exist_ok=True)
        manual = sinoclaw_home / "backups" / "my-manual.zip"
        manual.write_bytes(b"manual backup")

        for _ in range(5):
            create_pre_update_backup(sinoclaw_home=sinoclaw_home, keep=2)
            _t.sleep(1.05)
    ...
    def test_returns_none_if_root_missing(self, tmp_path):
        from sinoclaw_cli.backup import create_pre_update_backup
        assert create_pre_update_backup(sinoclaw_home=tmp_path / "does-not-exist") is None

    def test_keep_zero_does_not_delete_freshly_created_backup(self, sinoclaw_home):
        ...
        """
        from sinoclaw_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(sinoclaw_home=sinoclaw_home, keep=0)
        assert out is not None
        assert out.exists(), (
    ...
    def test_keep_negative_does_not_delete_freshly_created_backup(self, sinoclaw_home):
    ...
    def test_keep_zero_still_prunes_older_backups(self, sinoclaw_home):
        ...
        first = create_pre_update_backup(sinoclaw_home=sinoclaw_home, keep=5)
        _t.sleep(1.05)
        second = create_pre_update_backup(sinoclaw_home=sinoclaw_home, keep=5)
        _t.sleep(1.05)
        third = create_pre_update_backup(sinoclaw_home=sinoclaw_home, keep=0)
...
class TestRunPreUpdateBackup:
    ...
    @pytest.fixture
    def sinoclaw_home(self, tmp_path, monkeypatch):
        root = tmp_path / ".sinoclaw"
        root.mkdir()
        _make_sinoclaw_tree(root)
        # Point SINOCLAW_HOME at the temp dir so config + backup paths resolve here
        monkeypatch.setenv("SINOCLAW_HOME", str(root))
        # Make Path.home() point at tmp_path for anything that uses it
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        # Bust caches for sinoclaw_cli.config + sinoclaw_constants so they pick up SINOCLAW_HOME
        for mod in list(__import__("sys").modules.keys()):
            if mod.startswith("sinoclaw_cli.config") or mod == "sinoclaw_constants":
                del __import__("sys").modules[mod]
        return root

    def test_backup_flag_creates_backup(self, sinoclaw_home, capsys):
        """--backup forces the pre-update backup for one run even when config is off."""
        from sinoclaw_cli.main import _run_pre_update_backup
    ...
    def test_default_disabled_is_silent(self, sinoclaw_home, capsys):
        ...
        out = capsys.readouterr().out
        assert out == ""
        assert not (sinoclaw_home / "backups").exists() or not list(
            (sinoclaw_home / "backups").glob("pre-update-*.zip")
        )

    def test_no_backup_flag_skips(self, sinoclaw_home, capsys):
        from sinoclaw_cli.main import _run_pre_update_backup
    ...
    def test_config_enabled_creates_backup(self, sinoclaw_home, capsys):
        ...
        import yaml
        (sinoclaw_home / "config.yaml").write_text(yaml.safe_dump({
            "_config_version": 22,
            "updates": {"pre_update_backup": True},
        }))
        import sys as _sys
        for mod in list(_sys.modules.keys()):
            if mod.startswith("sinoclaw_cli.config"):
    ...
    def test_config_disabled_is_silent(self, sinoclaw_home, capsys):
        ...
        out = capsys.readouterr().out
        assert out == ""
        assert not list((sinoclaw_home / "backups").glob("pre-update-*.zip")) \
            if (sinoclaw_home / "backups").exists() else True

    def test_cli_flag_overrides_enabled_config(self, sinoclaw_home, capsys):
...
class TestPreMigrationBackup:
    ...
    def test_does_not_recurse_into_prior_backups(self, sinoclaw_home):
        from sinoclaw_cli.backup import create_pre_migration_backup
        out1 = create_pre_migration_backup(sinoclaw_home=sinoclaw_home)
        assert out1 is not None
        out2 = create_pre_migration_backup(sinoclaw_home=sinoclaw_home)
        assert out2 is not None
        with zipfile.ZipFile(out2) as zf:
            names = zf.namelist()
        assert not any(n.startswith("backups/") for n in names)
    ...
    def test_rotation_keeps_only_n(self, sinoclaw_home):
        ...
        created = []
        for _ in range(7):
            out = create_pre_migration_backup(sinoclaw_home=sinoclaw_home, keep=3)
            if out is not None:
                created.append(out)
            _t.sleep(1.05)  # timestamp resolution

        remaining = sorted((sinoclaw_home / "backups").glob("pre-migration-*.zip"))
        assert len(remaining) <= 3, f"expected <=3 backups retained, got {len(remaining)}"

    def test_missing_sinoclaw_home_returns_none(self, tmp_path):
        """Fresh install with no ~/.sinoclaw yet — nothing to back up."""
        from sinoclaw_cli.backup import create_pre_migration_backup
        missing = tmp_path / "does-not-exist"
        out = create_pre_migration_backup(sinoclaw_home=missing)
        assert out is None

    def test_does_not_touch_pre_update_backups(self, sinoclaw_home):
        """Pre-migration rotation must only prune pre-migration-*.zip files,
        leaving pre-update-*.zip backups untouched."""
        from sinoclaw_cli.backup import create_pre_update_backup, create_pre_migration_backup
        update_backup = create_pre_update_backup(sinoclaw_home=sinoclaw_home, keep=5)
        assert update_backup is not None and update_backup.exists()
        # Spin up a lot of migration backups with keep=1
        import time as _t
        for _ in range(3):
            out = create_pre_migration_backup(sinoclaw_home=sinoclaw_home, keep=1)
            assert out is not None
            _t.sleep(1.05)
