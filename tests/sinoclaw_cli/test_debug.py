...
class TestCaptureLogSnapshot:
    ...
    def test_reads_small_file(self, sinoclaw_home):
        from sinoclaw_cli.debug import _capture_log_snapshot
    ...
    def test_empty_primary_reports_file_empty(self, sinoclaw_home):
    ...
    def test_race_truncate_after_resolve_reports_empty(self, sinoclaw_home, monkeypatch):
        """If the log is truncated between resolve and stat, say 'empty', not 'missing'."""
        log_path = sinoclaw_home / "logs" / "agent.log"
        from sinoclaw_cli import debug
    ...
    def test_keeps_first_line_when_truncation_on_boundary(self, sinoclaw_home):
        ...
        (sinoclaw_home / "logs" / "agent.log").write_text(line * num_lines)
    ...
    def test_drops_partial_when_truncation_mid_line(self, sinoclaw_home):
...
# ---------------------------------------------------------------------------
# Capture log redaction (force=True applies regardless of SINOCLAW_REDACT_SECRETS)
# ---------------------------------------------------------------------------
...
class TestCaptureLogSnapshotRedaction:
    ...
    def test_default_redacts_tail_and_full_text(self, sinoclaw_home_with_secret):
        from sinoclaw_cli.debug import _capture_log_snapshot
    ...
    def test_redact_false_passes_through(self, sinoclaw_home_with_secret):
        from sinoclaw_cli.debug import _capture_log_snapshot
    ...
    def test_force_true_works_when_redaction_disabled(
        self, sinoclaw_home_with_secret, monkeypatch
    ):
        ...
        input unchanged when SINOCLAW_REDACT_SECRETS=false, and the share-time
        ...
        monkeypatch.setenv("SINOCLAW_REDACT_SECRETS", "false")

        from sinoclaw_cli.debug import _capture_log_snapshot

        assert os.environ.get("SINOCLAW_REDACT_SECRETS", "") == "false"
    ...
    def test_capture_default_log_snapshots_threads_redact(
        self, sinoclaw_home_with_secret
    ):
        from sinoclaw_cli.debug import _capture_default_log_snapshots
...
class TestRunDebugShare:
    ...
    def test_share_sweeps_expired_pastes(self, sinoclaw_home, capsys):
        ...
        with patch("sinoclaw_cli.dump.run_dump"), \
             patch("sinoclaw_cli.debug._sweep_expired_pastes", return_value=(0, 0)) as mock_sweep, \
    ...
    def test_share_survives_sweep_failure(self, sinoclaw_home, capsys):
        ...
        with patch("sinoclaw_cli.dump.run_dump"), \
             patch(
                 "sinoclaw_cli.debug._sweep_expired_pastes",
    ...
    def test_share_keeps_report_and_full_log_on_same_snapshot(self, sinoclaw_home, capsys):
        ...
        from sinoclaw_cli.debug import run_debug_share, collect_debug_report as real_collect_debug_report
        ...
        with patch("sinoclaw_cli.dump.run_dump"), \
             patch("sinoclaw_cli.debug.collect_debug_report", side_effect=_wrapped_collect_debug_report), \
             patch("sinoclaw_cli.debug.upload_to_pastebin", side_effect=_mock_upload):
            run_debug_share(args)
...
class TestRunDebugShareRedaction:
    ...
    def test_default_share_redacts_uploaded_content(
        self, sinoclaw_home_with_secret, capsys
    ):
        ...
        with patch("sinoclaw_cli.dump.run_dump"), \
             patch("sinoclaw_cli.debug._sweep_expired_pastes", return_value=(0, 0)), \
             patch("sinoclaw_cli.debug.upload_to_pastebin", side_effect=fake_upload):
            run_debug_share(args)
...
class TestScheduleAutoDelete:
    ...
    def test_does_not_spawn_subprocess(self, sinoclaw_home):
    ...
    def test_records_pending_to_json(self, sinoclaw_home):
        """Scheduled URLs are persisted to pending.json with expiration."""
        from sinoclaw_cli.debug import _schedule_auto_delete, _pending_file
        import json
    ...
    def test_skips_non_paste_rs_urls(self, sinoclaw_home):
    ...
    def test_merges_with_existing_pending(self, sinoclaw_home):
        """Subsequent calls merge into existing pending.json."""
        from sinoclaw_cli.debug import _schedule_auto_delete, _load_pending
    ...
    def test_dedupes_same_url(self, sinoclaw_home):
...
class TestSweepExpiredPastes:
    ...
    def test_sweep_empty_is_noop(self, sinoclaw_home):
    ...
    def test_sweep_deletes_expired_entries(self, sinoclaw_home):
        ...
        with patch("sinoclaw_cli.debug.delete_paste", side_effect=fake_delete):
            deleted, remaining = _sweep_expired_pastes()
    ...
    def test_sweep_leaves_future_entries_alone(self, sinoclaw_home):
        from sinoclaw_cli.debug import _sweep_expired_pastes, _save_pending
        import time
        ...
        with patch("sinoclaw_cli.debug.delete_paste") as mock_delete:
            deleted, remaining = _sweep_expired_pastes()
    ...
    def test_sweep_survives_network_failure(self, sinoclaw_home):
        ...
        with patch(
            "sinoclaw_cli.debug.delete_paste",
    ...
    def test_sweep_drops_entries_past_grace_window(self, sinoclaw_home):
...
class TestRunDebugSweepsOnInvocation:
    ...
    def test_run_debug_calls_sweep(self, sinoclaw_home):
        from sinoclaw_cli.debug import run_debug

        args = MagicMock()
        args.debug_command = None  # default → prints help

        with patch("sinoclaw_cli.debug._sweep_expired_pastes") as mock_sweep:
            run_debug(args)

        mock_sweep.assert_called_once()

    def test_run_debug_survives_sweep_failure(self, sinoclaw_home, capsys):
