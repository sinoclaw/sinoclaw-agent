...
When ``SINOCLAW_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    ...
    * Profile     -> tmp_path/.sinoclaw/profiles/coder   (active, SINOCLAW_HOME points here)
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (SINOCLAW_HOME == global root), no fallback path runs.
    ...
    # to the SINOCLAW_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    sinoclaw_home = tmp_path / "classic"
    sinoclaw_home.mkdir()
    monkeypatch.setenv("SINOCLAW_HOME", str(sinoclaw_home))

    _write(sinoclaw_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
    ...
    from sinoclaw_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: SINOCLAW_HOME is set to a custom path that is NOT under
    # ~/.sinoclaw/profiles/ — get_default_sinoclaw_root() returns SINOCLAW_HOME
...
def test_write_credential_pool_targets_profile_not_global(profile_env):
    from sinoclaw_cli.auth import read_credential_pool, write_credential_pool
