...
def _patch_httpx(monkeypatch, response):
    ...
    monkeypatch.setattr("sinoclaw_cli.auth.httpx.Client", _factory)
...
def test_login_openai_codex_force_new_login_skips_existing_reuse_prompt(monkeypatch):
    ...
    monkeypatch.setattr("sinoclaw_cli.auth._save_codex_tokens", _fake_save)
    monkeypatch.setattr("sinoclaw_cli.auth._update_config_for_provider", lambda *args, **kwargs: "/tmp/config.yaml")
    monkeypatch.setattr(
        "builtins.input",
