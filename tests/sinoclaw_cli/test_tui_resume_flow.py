...
def main_mod(monkeypatch):
    import sinoclaw_cli.main as mod
...
def test_oneshot_filters_invalid_toolsets_before_redirect(monkeypatch, capsys):
    _stub_plugin_discovery(monkeypatch)
    from sinoclaw_cli.oneshot import _validate_explicit_toolsets
...
def test_launch_tui_exports_model_provider_and_toolsets(monkeypatch, main_mod):
    ...
    def fake_call(argv, cwd=None, env=None):
        ...
        active_path_during_call = Path(env["SINOCLAW_TUI_ACTIVE_SESSION_FILE"])
    ...
    active_path = Path(env["SINOCLAW_TUI_ACTIVE_SESSION_FILE"])
    assert active_path.name.startswith("sinoclaw-tui-active-session-")
...
def test_print_tui_exit_summary_includes_resume_and_token_totals(monkeypatch, capsys):
    import sinoclaw_cli.main as main_mod
    ...
    assert "sinoclaw-agent --tui --resume 20260409_000001_abc123" in out
    assert 'sinoclaw-agent --tui -c "demo title"' in out
...
def test_print_tui_exit_summary_prefers_actual_active_session_file(
    ...
    assert "sinoclaw-agent --tui --resume actual_session" in out
