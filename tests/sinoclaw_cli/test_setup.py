...
def test_select_provider_and_model_accepts_named_provider_from_providers_section(
    ...
    monkeypatch.setattr(
        "sinoclaw_cli.main._prompt_provider_choice",
...
def test_modal_setup_can_use_nous_subscription_without_modal_creds(tmp_path, monkeypatch, capsys):
    monkeypatch.setattr("sinoclaw_cli.setup.managed_nous_tools_enabled", lambda: True)
    monkeypatch.setenv("SINOCLAW_HOME", str(tmp_path))
    config = load_config()
...
def test_offer_launch_chat_falls_back_to_module(monkeypatch):
    ...
    assert exec_calls == [(sys.executable, [sys.executable, "-m", "sinoclaw_cli.main", "chat"])]
