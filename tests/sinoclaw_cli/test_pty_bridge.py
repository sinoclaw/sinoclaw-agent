"""Unit tests for sinoclaw_cli.pty_bridge — PTY spawning + byte forwarding.
...
class TestPtyBridgeIO:
    def test_reads_child_stdout(self):
        ...
        try:
            output = _read_until(bridge, b"sinoclaw-ok")
            assert b"sinoclaw-ok" in output
        finally:
            bridge.close()
...
class TestPtyBridgeEnv:
    ...
    def test_env_is_forwarded(self):
        bridge = PtyBridge.spawn(
            ["/bin/sh", "-c", "printf %s \"$SINOCLAW_PTY_TEST\""],
            env={**os.environ, "SINOCLAW_PTY_TEST": "pty-env-works"},
        )
        try:
