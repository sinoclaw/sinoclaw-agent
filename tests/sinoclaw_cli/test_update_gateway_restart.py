...
def _no_restart_verify_sleep(monkeypatch):
    """sinoclaw_cli/main.py uses time.sleep(3) after systemctl restart to
...
class TestCmdUpdateLaunchdRestart:
        ...
        def side_effect(cmd, **kwargs):
            ...
            if "systemctl" in joined and "list-units" in joined:
                if "--user" in joined:
                    return subprocess.CompletedProcess(
                        cmd, 0,
                        stdout="sinoclaw-gateway.service loaded active running\n",
                        stderr="",
                    )
                return subprocess.CompletedProcess(cmd, 0, stdout="", stderr="")
    ...
    def test_update_falls_back_to_systemctl_restart_when_sigusr1_times_out(
        ...
        monkeypatch.setattr(
            "sinoclaw_cli.gateway._graceful_restart_via_sigusr1",
    ...
    def test_update_bypasses_restartsec_after_graceful_drain(
        ...
        # Must have called ``reset-failed sinoclaw-gateway`` AND ``start
        # sinoclaw-gateway`` explicitly so systemd bypasses RestartSec.
        reset_calls = [c for c in calls if "reset-failed" in c and "sinoclaw-gateway" in c]
        start_calls = [
            c for c in calls
            if "start" in c and "sinoclaw-gateway" in c and "restart" not in c
        ]
        assert reset_calls, (
            f"Expected explicit `reset-failed sinoclaw-gateway` after graceful drain; "
            f"systemctl calls were: {calls}"
        )
        assert start_calls, (
            f"Expected explicit `start sinoclaw-gateway` after graceful drain to "
...
class TestCmdUpdateLegacyGatewayWarning:
    ...
    def test_update_does_not_flag_profile_units(
        ...
        (user_dir / "sinoclaw-gateway-coder.service").write_text(
            self._OUR_UNIT_TEXT, encoding="utf-8"
        )
        (user_dir / "sinoclaw-gateway.service").write_text(
            self._OUR_UNIT_TEXT, encoding="utf-8"
        )
...
class TestCmdUpdateResetFailedBeforeRestart:
    ...
    def test_reset_failed_runs_before_fallback_restart(
        ...
        assert any(
            "sinoclaw-gateway" in " ".join(str(c) for c in call)
            for call in reset_calls
        ), (
            "Expected `systemctl reset-failed sinoclaw-gateway` before the "
        ...
        for idx, call in enumerate(mock_run.call_args_list):
            ...
            if "systemctl" in joined and "reset-failed" in joined and first_reset_idx is None:
                first_reset_idx = idx
            if "systemctl" in joined and "restart" in joined and "sinoclaw-gateway" in joined:
    ...
    def test_reset_failed_also_runs_before_retry_restart(
        ...
        gateway_restarts = [
            ...
            if "sinoclaw-gateway" in " ".join(str(a) for a in c)
        ]
        gateway_resets = [
    ...
    def test_final_failure_message_tells_user_to_reset_failed(
        ...
        assert "sinoclaw-gateway" in captured
