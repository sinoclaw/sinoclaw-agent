...
def _isolated_sinoclaw_home(tmp_path):
    """Point SINOCLAW_HOME at a temp dir so tests never touch real config."""
...
class TestConfigYamlRouting:
    ...
    def test_terminal_vercel_runtime_goes_to_config_and_env(self, _isolated_sinoclaw_home):
...
class TestListNavigation:
    """sinoclaw config set must preserve YAML list fields when using numeric
    ...
    def test_indexed_set_preserves_sibling_list_entries(self, _isolated_sinoclaw_home):
        """Setting custom_providers.0.api_key must not destroy entry 1."""
        self._write_config(_isolated_sinoclaw_home, (
            "custom_providers:\n"
            "- name: provider-a\n"
            "  api_key: old-a\n"
        ...
        import yaml
        reloaded = yaml.safe_load(_read_config(_isolated_sinoclaw_home))
    ...
    def test_indexed_set_preserves_non_targeted_fields(self, _isolated_sinoclaw_home):
    ...
    def test_deeper_nesting_through_list(self, _isolated_sinoclaw_home):
