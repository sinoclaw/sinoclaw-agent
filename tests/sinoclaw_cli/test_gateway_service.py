...
class TestSystemdServiceRefresh:
    ...
    def test_systemd_stop_timeout_prints_status_guidance(self, monkeypatch, capsys):
        ...
        assert "sinoclaw gateway status" in output

    def test_systemd_restart_timeout_prints_status_guidance(self, monkeypatch, capsys):
        """`sinoclaw gateway restart` must not surface a raw TimeoutExpired traceback.

        The dashboard spawns `sinoclaw gateway restart` in the background; when a
    ...
    def test_run_gateway_refreshes_outdated_unit_on_boot(self, tmp_path, monkeypatch):
        ...
        via exit-code-75 (bypassing `sinoclaw gateway restart`)."""
        unit_path = tmp_path / "sinoclaw-gateway.service"
        unit_path.write_text("old unit\n", encoding="utf-8")
...
class TestRequireServiceInstalled:
    def test_exits_with_install_hint_when_unit_missing(self, tmp_path, monkeypatch, capsys):
        ...
        assert "sinoclaw gateway install" in out
...
class TestGatewaySystemServiceRouting:
    ...
    def test_gateway_install_reports_termux_manual_mode(self, monkeypatch, capsys):
        ...
        assert "Run manually: sinoclaw gateway" in out
    ...
    def test_gateway_status_on_termux_shows_manual_guidance(self, monkeypatch, capsys):
        ...
        assert "nohup sinoclaw gateway" in out
...
class TestPreflightUserSystemd:
    ...
    def test_raises_when_linger_disabled_and_loginctl_denied(self, monkeypatch):
        ...
        assert "sinoclaw gateway run" in msg  # foreground fallback mentioned
...
class TestProfileArg:
    ...
    def test_default_sinoclaw_home_returns_empty(self, tmp_path, monkeypatch):
        """Default ~/.sinoclaw should not produce a --profile flag."""
    ...
    def test_named_profile_returns_flag(self, tmp_path, monkeypatch):
        """~/.sinoclaw/profiles/mybot should return '--profile mybot'."""
    ...
    def test_nested_profile_path_returns_empty(self, tmp_path, monkeypatch):
        """~/.sinoclaw/profiles/mybot/subdir should NOT match — too deep."""
...
class TestRemapPathForUser:
    ...
    def test_remaps_path_under_current_home(self, monkeypatch, tmp_path):
        ...
        result = gateway_cli._remap_path_for_user(
            str(tmp_path / "root" / ".hermes" / "sinoclaw-agent"),
            str(tmp_path / "alice"),
        )
        assert result == str(tmp_path / "alice" / ".hermes" / "sinoclaw-agent")
    ...
    def test_noop_when_same_user(self, monkeypatch, tmp_path):
        ...
        original = str(tmp_path / "alice" / ".hermes" / "sinoclaw-agent")
...
class TestSystemUnitPathRemapping:
    ...
    def test_system_unit_has_no_root_paths(self, monkeypatch, tmp_path):
        ...
        root_home.mkdir()
        project = root_home / ".hermes" / "sinoclaw-agent"
        ...
        monkeypatch.setenv("SINOCLAW_HOME", str(root_home / ".hermes"))
        monkeypatch.setattr(gateway_cli, "get_sinoclaw_home", lambda: root_home / ".hermes")
        ...
        assert "WorkingDirectory=/home/alice/.sinoclaw/sinoclaw-agent" in unit
...
class TestDockerAwareGateway:
    ...
    def test_run_systemctl_raises_runtimeerror_when_missing(self, monkeypatch):
        ...
        with pytest.raises(RuntimeError, match="systemctl is not available"):
            gateway_cli._run_systemctl(["start", "sinoclaw-gateway"])
    ...
    def test_run_systemctl_passes_through_on_success(self, monkeypatch):
        ...
        result = gateway_cli._run_systemctl(["status", "sinoclaw-gateway"])
        assert result.returncode == 0
        assert len(calls) == 1
        assert "status" in calls[0]

    def test_install_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'sinoclaw gateway install' inside Docker exits 0 with container guidance."""
        import pytest
    ...
    def test_uninstall_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'sinoclaw gateway uninstall' inside Docker exits 0 with container guidance."""
        import pytest
    ...
    def test_start_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'sinoclaw gateway start' inside Docker exits 0 with container guidance."""
        import pytest
...
class TestLegacyHermesUnitDetection:
    ...
    _OUR_UNIT_TEXT = (
        ...
        "ExecStart=/usr/bin/python -m sinoclaw_cli.main gateway run --replace\n"
    )
    ...
    def test_ignores_profile_unit_sinoclaw_gateway_coder(self, tmp_path, monkeypatch):
        ...
        Teknium's concern — ``sinoclaw-gateway-coder.service`` is our standard
        ...
        for base in (user_dir, system_dir):
            (base / "sinoclaw-gateway-coder.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
            (base / "sinoclaw-gateway-orcha.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
            (base / "sinoclaw-gateway.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
    ...
    def test_accepts_alternate_execstart_formats(self, tmp_path, monkeypatch):
        ...
        ExecStart variants we've seen in the wild:
          - python -m sinoclaw_cli.main gateway run
          - python path/to/sinoclaw_cli/main.py gateway run
          - sinoclaw gateway run   (direct binary)
          - python path/to/gateway/run.py
        """
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        variants = [
            "ExecStart=/venv/bin/python -m sinoclaw_cli.main gateway run --replace",
            "ExecStart=/venv/bin/python /opt/hermes/sinoclaw_cli/main.py gateway run",
            "ExecStart=/usr/local/bin/sinoclaw gateway run --replace",
...
class TestRemoveLegacyHermesUnits:
    ...
    def test_system_scope_without_root_defers_removal(self, tmp_path, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "sudo sinoclaw gateway migrate-legacy" in out
    ...
    def test_does_not_touch_profile_units_during_migration(
        ...
        """Teknium's constraint: profile units (sinoclaw-gateway-coder.service)
        ...
        profile_unit = user_dir / "sinoclaw-gateway-coder.service"
        profile_unit.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
        default_unit = user_dir / "sinoclaw-gateway.service"
...
class TestMigrateLegacyCommand:
    """Tests for the `sinoclaw gateway migrate-legacy` subcommand dispatch."""
    ...
    def test_migrate_legacy_subparser_accepts_dry_run_and_yes(self):
        ...
        import sinoclaw_cli.main as cli_main
        ...
        # public, just confirm that `sinoclaw gateway --help` shows it.
        import subprocess
        import sys

        project_root = cli_main.PROJECT_ROOT if hasattr(cli_main, "PROJECT_ROOT") else None
        if project_root is None:
            import sinoclaw_cli.gateway as gw
            project_root = gw.PROJECT_ROOT

        result = subprocess.run(
            [sys.executable, "-m", "sinoclaw_cli.main", "gateway", "--help"],
...
class TestGatewayStatusParser:
    def test_gateway_status_subparser_accepts_full_flag(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "sinoclaw_cli.main", "gateway", "status", "-l", "--help"],
...
class TestSystemScopeWizardPreCheck:
    ...
    def _setup_units(tmp_path, monkeypatch, system_present: bool, user_present: bool):
        ...
        usr_dir.mkdir()
        if system_present:
            (sys_dir / "sinoclaw-gateway.service").write_text("[Unit]\n")
        if user_present:
            (usr_dir / "sinoclaw-gateway.service").write_text("[Unit]\n")
        monkeypatch.setattr(
            gateway_cli,
            "get_systemd_unit_path",
            lambda system=False: (sys_dir if system else usr_dir) / "sinoclaw-gateway.service",
        )
    ...
    def test_non_root_with_explicit_system_arg_returns_true(self, tmp_path, monkeypatch):
        # Caller passed system=True explicitly (e.g. ``sinoclaw gateway start --system``).
        self._setup_units(tmp_path, monkeypatch, system_present=False, user_present=False)
        monkeypatch.setattr(gateway_cli.os, "geteuid", lambda: 1000)
...
class TestSystemScopeRemediationOutput:
    ...
    def test_start_remediation_mentions_sudo_systemctl_and_uninstall(self, capsys, monkeypatch):
        monkeypatch.setattr(gateway_cli, "get_service_name", lambda: "sinoclaw-gateway")
        ...
        assert "sudo systemctl start sinoclaw-gateway" in out
        assert "sudo sinoclaw gateway uninstall --system" in out
        assert "sinoclaw gateway install" in out
    ...
    def test_restart_remediation_uses_systemctl_restart(self, capsys, monkeypatch):
        ...
        assert "sudo systemctl restart sinoclaw-gateway" in out
    ...
    def test_stop_remediation_uses_systemctl_stop(self, capsys, monkeypatch):
        ...
        assert "sudo systemctl stop sinoclaw-gateway" in out


class TestGatewayCommandCatchesSystemScopeError:
    """The direct CLI path (``sinoclaw gateway start --system`` etc.) must
