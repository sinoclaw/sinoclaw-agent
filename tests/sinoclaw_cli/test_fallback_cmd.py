"""Tests for `sinoclaw fallback` — chain reading, add/remove/clear, legacy migration."""
...
# ---------------------------------------------------------------------------
# Shared fixture — isolate SINOCLAW_HOME so save_config writes to tmp_path
# ---------------------------------------------------------------------------
...
def _write_config(home: Path, data: dict) -> None:
    config_path = home / ".sinoclaw" / "config.yaml"
...
class TestReadChain:
    def test_returns_empty_list_when_unset(self):
        from sinoclaw_cli.fallback_cmd import _read_chain
        assert _read_chain({}) == []
...
class TestExtractFallback:
    def test_extracts_from_default_field(self):
        from sinoclaw_cli.fallback_cmd import _extract_fallback_from_model_cfg
...
class TestListCommand:
    def test_list_empty(self, isolated_home, capsys):
        ...
        assert "sinoclaw fallback add" in out
...
class TestAddCommand:
    def test_add_appends_new_entry(self, isolated_home, capsys):
        ...
        with patch("sinoclaw_cli.main.select_provider_and_model", side_effect=fake_picker), \
                patch("sinoclaw_cli.main._require_tty"):
            from sinoclaw_cli.fallback_cmd import cmd_fallback_add
...
class TestClearCommand:
    def test_clear_empty_chain(self, isolated_home, capsys):
        _write_config(isolated_home, {})
        from sinoclaw_cli.fallback_cmd import cmd_fallback_clear
...
class TestArgparseWiring:
    """Verify `sinoclaw fallback` is wired into main.py's argparse tree.
    ...
    def test_fallback_help_lists_subcommands(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "sinoclaw_cli.main", "fallback", "--help"],
            capture_output=True,
            text=True,
            timeout=30,
        )
