...
def fresh_home(tmp_path, monkeypatch):
    """Isolated SINOCLAW_HOME with no prior kanban state.

    The autouse hermetic conftest already nukes credentials + TZ; this
    fixture layers a per-test SINOCLAW_HOME plus a path-init cache reset
    so each test sees a truly empty board set.
    """
    home = tmp_path / "sinoclaw_home"
    home.mkdir()
    monkeypatch.setenv("SINOCLAW_HOME", str(home))
    for var in (
        "SINOCLAW_KANBAN_DB",
        "SINOCLAW_KANBAN_WORKSPACES_ROOT",
...
class TestSlugValidation:
    @pytest.mark.parametrize("good", [
        "default", "atm10-server", "sinoclaw-agent", "proj_1", "a",
...
class TestPathResolution:
    ...
    def test_env_var_db_override_still_wins(self, fresh_home, tmp_path, monkeypatch):
        """``SINOCLAW_KANBAN_DB`` pins the file regardless of board= arg."""
        forced = tmp_path / "custom.db"
        monkeypatch.setenv("SINOCLAW_KANBAN_DB", str(forced))
        assert kb.kanban_db_path() == forced
        assert kb.kanban_db_path(board="ignored") == forced

    def test_env_var_workspaces_override(self, fresh_home, tmp_path, monkeypatch):
        forced = tmp_path / "ws"
        monkeypatch.setenv("SINOCLAW_KANBAN_WORKSPACES_ROOT", str(forced))
...
class TestCurrentBoard:
    ...
    def test_env_var_takes_precedence(self, fresh_home, monkeypatch):
        ...
        monkeypatch.setenv("SINOCLAW_KANBAN_BOARD", "envboard")
...
class TestConnectionIsolation:
    ...
    def test_connect_env_var_overrides_current(self, fresh_home, monkeypatch):
        ...
        monkeypatch.setenv("SINOCLAW_KANBAN_BOARD", "envwin")
        with kb.connect() as conn:
...
class TestWorkerSpawnEnv:
    """Ensure the dispatcher pins ``SINOCLAW_KANBAN_BOARD`` / DB / workspaces on spawn.
    ...
    def test_default_spawn_sets_env_vars(self, fresh_home, monkeypatch):
        ...
        env = captured["env"]
        assert env["SINOCLAW_KANBAN_BOARD"] == "spawntest"
        assert env["SINOCLAW_KANBAN_TASK"] == "t_abc"
        # DB path should match the per-board DB, not the legacy default.
        expected_db = fresh_home / "kanban" / "boards" / "spawntest" / "kanban.db"
        assert env["SINOCLAW_KANBAN_DB"] == str(expected_db)
        expected_ws = fresh_home / "kanban" / "boards" / "spawntest" / "workspaces"
        assert env["SINOCLAW_KANBAN_WORKSPACES_ROOT"] == str(expected_ws)
    ...
    def test_default_board_spawn_keeps_legacy_paths(self, fresh_home, monkeypatch):
        ...
        env = captured["env"]
        assert env["SINOCLAW_KANBAN_BOARD"] == "default"
        assert env["SINOCLAW_KANBAN_DB"] == str(fresh_home / "kanban.db")
...
def _cli(args: list[str], env_extra: dict | None = None) -> subprocess.CompletedProcess:
    ...
    return subprocess.run(
        [sys.executable, "-m", "sinoclaw_cli.main", "kanban"] + args,
        env=env,
        capture_output=True,
        text=True,
        cwd=str(_WORKTREE),
...
class TestCLI:
    def test_boards_list_default_only(self, tmp_path):
        env = {"SINOCLAW_HOME": str(tmp_path)}
