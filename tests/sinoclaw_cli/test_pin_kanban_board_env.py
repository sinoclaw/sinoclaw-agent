...
def _isolate_kanban_board_env():
    """Snapshot `SINOCLAW_KANBAN_BOARD` and restore it after the test.
    ...
    """
    prev = os.environ.get("SINOCLAW_KANBAN_BOARD")
    ...
    try:
        ...
        if prev is None:
            ...
            os.environ["SINOCLAW_KANBAN_BOARD"] = prev


def test_pin_writes_resolved_board_when_env_unset(monkeypatch):
    main_mod = importlib.import_module("sinoclaw_cli.main")

    import sinoclaw_cli.kanban_db as kdb
    monkeypatch.setattr(kdb, "get_current_board", lambda: "space")

    main_mod._pin_kanban_board_env()

    assert main_mod.os.environ.get("SINOCLAW_KANBAN_BOARD") == "space"


def test_pin_does_not_overwrite_existing_env(monkeypatch):
    monkeypatch.setenv("SINOCLAW_KANBAN_BOARD", "preset")
    main_mod = importlib.import_module("sinoclaw_cli.main")
    ...
    assert main_mod.os.environ.get("SINOCLAW_KANBAN_BOARD") == "preset"
...
def test_pin_swallows_resolution_failures(monkeypatch):
    ...
    assert "SINOCLAW_KANBAN_BOARD" not in main_mod.os.environ
