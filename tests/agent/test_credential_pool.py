...
def test_load_pool_prefers_dotenv_over_stale_os_environ(tmp_path, monkeypatch):
    ...
    from a parent shell) must NOT shadow the fresh key in ~/.sinoclaw/.env when
    ...
    # User edited ~/.sinoclaw/.env with the fresh key
...
def test_load_pool_falls_back_to_os_environ_when_dotenv_empty(tmp_path, monkeypatch):
    """When ~/.sinoclaw/.env does not define OPENROUTER_API_KEY (typical Docker /
    ...
    (sinoclaw_home / ".env").write_text("SOME_OTHER_VAR=unrelated\n")
