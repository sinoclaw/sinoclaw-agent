...
class TestBuildNousSubscriptionPrompt:
    def test_includes_active_subscription_features(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "sinoclaw_cli.nous_subscription.get_nous_subscription_features",
...
class TestBuildContextFilesPrompt:
    def test_empty_dir_loads_seeded_global_soul(self, tmp_path):
        ...
        assert "Sinoclaw Agent" in result
    ...
    def test_loads_soul_md_from_sinoclaw_home_only(self, tmp_path, monkeypatch):
        monkeypatch.setenv("HERMES_HOME", str(tmp_path / "sinoclaw_home"))
        sinoclaw_home = tmp_path / "sinoclaw_home"
        sinoclaw_home.mkdir()
        (sinoclaw_home / "SOUL.md").write_text("Be concise and friendly.", encoding="utf-8")
    ...
    def test_empty_soul_md_adds_nothing(self, tmp_path, monkeypatch):
        ...
        sinoclaw_home = tmp_path / "sinoclaw_home"
        sinoclaw_home.mkdir()
        (sinoclaw_home / "SOUL.md").write_text("\n\n", encoding="utf-8")
        result = build_context_files_prompt(cwd=str(tmp_path))
        assert result == ""
    ...
    # --- .sinoclaw.md / HERMES.md discovery ---
    ...
    def test_sinoclaw_md_beats_all_others(self, tmp_path):
        ...
        result = build_context_files_prompt(cwd=str(tmp_path))
        assert "Sinoclaw wins" in result
...
class TestFindSinoclawMd:
    ...
    def test_stops_at_git_root(self, tmp_path):
        ...
        repo.mkdir()
        (repo / ".git").mkdir()
        assert _find_sinoclaw_md(repo) is None
