...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("sinoclaw/sinoclaw-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned sinoclaw/* and pr-* branches."""

    def test_prunes_orphaned_sinoclaw_branch(self, git_repo):
        """sinoclaw/sinoclaw-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "sinoclaw/sinoclaw-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "sinoclaw/sinoclaw-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "sinoclaw/sinoclaw-deadbeef" in result.stdout
        ...
        assert "sinoclaw/sinoclaw-deadbeef" in orphaned
        ...
        assert "sinoclaw/sinoclaw-deadbeef" not in result.stdout
