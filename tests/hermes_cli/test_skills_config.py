"""Tests for sinoclaw_cli/skills_config.py and skills_tool disabled filtering."""
import pytest
...
class TestSaveDisabledSkills:
    @patch("sinoclaw_cli.skills_config.save_config")
    def test_saves_global_sorted(self, mock_save):
        from sinoclaw_cli.skills_config import save_disabled_skills
        config = {}
...
class TestIsSkillDisabled:
    @patch("sinoclaw_cli.config.load_config")
    ...
    @patch("sinoclaw_cli.config.load_config")
    @patch.dict("os.environ", {"SINOCLAW_PLATFORM": "discord"})
...
class TestGetDisabledSkillNames:
    ...
    def test_session_platform_env_var(self, tmp_path, monkeypatch):
        """SINOCLAW_SESSION_PLATFORM should be used when SINOCLAW_PLATFORM is unset."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  disabled:\n"
    ...
    def test_sinoclaw_platform_takes_precedence(self, tmp_path, monkeypatch):
        """SINOCLAW_PLATFORM should win over SINOCLAW_SESSION_PLATFORM."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  platform_disabled:\n"
...
class TestGetCategories:
    def test_extracts_unique_categories(self):
        from sinoclaw_cli.skills_config import _get_categories
        skills = [
