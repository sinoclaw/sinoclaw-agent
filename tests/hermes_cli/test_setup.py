...
from sinoclaw_cli.setup import setup_model_provider
...
def _stub_tts(monkeypatch):
    ...
    monkeypatch.setattr("sinoclaw_cli.setup.prompt_choice", lambda q, c, d=0: (
        _maybe_keep_current_tts(q, c) if _maybe_keep_current_tts(q, c) is not None
        else d
    ))
    monkeypatch.setattr("sinoclaw_cli.setup.prompt_yes_no", lambda *a, **kw: False)
...
def test_setup_delegates_to_select_provider_and_model(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("sinoclaw_cli.main.select_provider_and_model", fake_select)
...
def test_select_provider_and_model_warns_if_named_custom_provider_disappears(
    ...
    monkeypatch.setattr("sinoclaw_cli.auth.resolve_provider", lambda provider: None)
    monkeypatch.setattr("sinoclaw_cli.main._prompt_provider_choice", fake_prompt_provider_choice)
    monkeypatch.setattr(
        "sinoclaw_cli.main._model_flow_named_custom",
...
def test_modal_setup_can_use_nous_subscription_without_modal_creds(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr("sinoclaw_cli.setup.prompt_choice", fake_prompt_choice)
    monkeypatch.setattr("sinoclaw_cli.setup.prompt", fake_prompt)
    monkeypatch.setattr("sinoclaw_cli.setup._prompt_container_resources", lambda config: None)
    monkeypatch.setattr(
        "sinoclaw_cli.setup.get_nous_subscription_features",
    ...
    from sinoclaw_cli.setup import setup_terminal_backend
...
def test_modal_setup_persists_direct_mode_when_user_chooses_their_own_account(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("sinoclaw_cli.setup.prompt_choice", fake_prompt_choice)
    monkeypatch.setattr("sinoclaw_cli.setup.prompt", lambda *args, **kwargs: next(prompt_values))
    monkeypatch.setattr("sinoclaw_cli.setup._prompt_container_resources", lambda config: None)
    monkeypatch.setattr(
        "sinoclaw_cli.setup.get_nous_subscription_features",
        lambda config: type("Features", (), {"nous_auth_present": True})(),
...
def test_resolve_sinoclaw_chat_argv_prefers_which(monkeypatch):
    from sinoclaw_cli import setup as setup_mod

    monkeypatch.setattr(setup_mod.shutil, "which", lambda name: "/usr/local/bin/sinoclaw" if name == "sinoclaw" else None)

    assert setup_mod._resolve_sinoclaw_chat_argv() == ["/usr/local/bin/sinoclaw", "chat"]


def test_resolve_sinoclaw_chat_argv_falls_back_to_module(monkeypatch):
    from sinoclaw_cli import setup as setup_mod

    monkeypatch.setattr(setup_mod.shutil, "which", lambda _name: None)
    monkeypatch.setattr(setup_mod.importlib.util, "find_spec", lambda name: object() if name == "sinoclaw_cli" else None)

    assert setup_mod._resolve_sinoclaw_chat_argv() == [sys.executable, "-m", "sinoclaw_cli.main", "chat"]
...
def test_offer_launch_chat_execs_fresh_process(monkeypatch):
    ...
    monkeypatch.setattr(setup_mod, "_resolve_sinoclaw_chat_argv", lambda: ["/usr/local/bin/sinoclaw", "chat"])
...
def test_offer_launch_chat_manual_fallback_when_unresolvable(monkeypatch, capsys):
    ...
    monkeypatch.setattr(setup_mod, "prompt_yes_no", lambda *_args, **_kwargs: True)
    monkeypatch.setattr(setup_mod, "_resolve_sinoclaw_chat_argv", lambda: None)

    setup_mod._offer_launch_chat()

    captured = capsys.readouterr()
    assert "Run 'sinoclaw chat' manually" in captured.out
