"""Tests for sinoclaw_cli/webhook.py — webhook subscription CLI."""
...
from sinoclaw_cli.webhook import (
    webhook_command,
    _load_subscriptions,
    _save_subscriptions,
    _subscriptions_path,
...
def _isolate(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "sinoclaw_cli.webhook._is_webhook_enabled", lambda: True
    )
...
class TestWebhookEnabledGate:
    def test_blocks_when_disabled(self, capsys, monkeypatch):
        monkeypatch.setattr("sinoclaw_cli.webhook._is_webhook_enabled", lambda: False)
    ...
    def test_real_check_disabled(self, monkeypatch):
        monkeypatch.setattr(
            "sinoclaw_cli.webhook._get_webhook_config",
            lambda: {},
        )
        monkeypatch.setattr(
            "sinoclaw_cli.webhook._is_webhook_enabled",
            lambda: bool({}.get("enabled")),
        )
        import sinoclaw_cli.webhook as wh_mod
