...
from sinoclaw_cli import config as sinoclaw_config
...
def test_stash_local_changes_if_needed_returns_specific_stash_commit(monkeypatch, tmp_path):
    ...
    def fake_run(cmd, **kwargs):
        calls.append((cmd, kwargs))
        if cmd[-2:] == ["status", "--porcelain"]:
            return SimpleNamespace(stdout=" M sinoclaw_cli/main.py\n?? notes.txt\n", returncode=0)
...
def test_restore_stashed_changes_always_resets_on_conflict(monkeypatch, tmp_path, capsys):
    ...
    def fake_run(cmd, **kwargs):
        ...
        if cmd[1:3] == ["diff", "--name-only"]:
            return SimpleNamespace(stdout="sinoclaw_cli/main.py\n", stderr="", returncode=0)
    ...
    assert "sinoclaw_cli/main.py" in out
...
def _setup_update_mocks(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(sinoclaw_config, "get_missing_env_vars", lambda required_only=True: [])
    monkeypatch.setattr(sinoclaw_config, "get_missing_config_fields", lambda: [])
    monkeypatch.setattr(sinoclaw_config, "check_config_version", lambda: (5, 5))
    monkeypatch.setattr(sinoclaw_config, "migrate_config", lambda **kw: {"env_added": [], "config_added": []})
