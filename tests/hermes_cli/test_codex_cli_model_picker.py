...
and haven't been migrated to the Sinoclaw auth store yet.

Root cause: list_authenticated_providers() checked the raw Sinoclaw auth
...
Sinoclaw auth store has no openai-codex tokens, and
...
def codex_cli_only_env(tmp_path, monkeypatch):
    ...
    NOT in the Sinoclaw auth store."""
...
def test_codex_cli_tokens_migrated_after_detection(codex_cli_only_env):
    ...
    migrated into the Sinoclaw auth store for subsequent fast lookups."""
    from sinoclaw_cli.model_switch import list_authenticated_providers

    # First call triggers migration
    list_authenticated_providers(current_provider="openai-codex")

    # Verify tokens are now in Sinoclaw auth store
    auth_path = codex_cli_only_env / "auth.json"
    store = json.loads(auth_path.read_text())
    providers = store.get("providers", {})
    assert "openai-codex" in providers, (
        f"openai-codex not migrated to Sinoclaw auth store: {list(providers.keys())}"
    )
...
def claude_code_only_env(tmp_path, monkeypatch):
    ...
    ~/.claude/.credentials.json (Claude Code) — not in env vars or Sinoclaw
    auth store."""
    sinoclaw_home = tmp_path / ".sinoclaw"
    sinoclaw_home.mkdir()
