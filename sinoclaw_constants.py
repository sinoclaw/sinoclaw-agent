"""Shared constants for Sinoclaw Agent.
...
def get_sinoclaw_home() -> Path:
    ...
    When ``SINOCLAW_HOME`` is unset but an ``active_profile`` file indicates
    a non-default profile is active, logs a loud one-shot warning to
    ``errors.log`` so cross-profile data corruption is diagnosable instead
    of silent.  Behavior is unchanged otherwise — we still return
    ``~/.sinoclaw`` — because raising here would brick 30+ module-level
    callers that import this at load time.  Subprocess spawners are
    expected to propagate ``SINOCLAW_HOME`` explicitly (see the systemd
    template in ``sinoclaw_cli/gateway.py`` and the kanban dispatcher in
    ``sinoclaw_cli/kanban_db.py``).  See https://github.com/NousResearch/sinoclaw-agent/issues/18594.
    """
    ...
    if not _profile_fallback_warned:
        ...
        if active and active != "default":
            ...
            import sys
            msg = (
                f"[SINOCLAW_HOME fallback] SINOCLAW_HOME is unset but active "
                f"profile is {active!r}. Falling back to ~/.sinoclaw, which "
                f"is the DEFAULT profile — not {active!r}. Any data this "
                f"process writes will land in the wrong profile. The "
                f"subprocess spawner should pass SINOCLAW_HOME explicitly "
                f"(see issue #18594)."
            )
            try:
...
def get_optional_skills_dir(default: Path | None = None) -> Path:
    ...
    if default is not None:
        return default
    return get_sinoclaw_home() / "optional-skills"
...
def display_sinoclaw_home() -> str:
    ...
    Uses ``~/`` shorthand for readability::
        ...
        default:  ``~/.sinoclaw``
        profile:  ``~/.sinoclaw/profiles/coder``
        custom:   ``/opt/sinoclaw-custom``

    Use this in **user-facing** print/log messages instead of hardcoding
    ``~/.sinoclaw``.  For code that needs a real ``Path``, use
    :func:`get_sinoclaw_home` instead.
    """
    home = get_sinoclaw_home()
    try:
...
def apply_ipv4_preference(force: bool = False) -> None:
    ...
    if getattr(socket.getaddrinfo, "_sinoclaw_ipv4_patched", False):
        return
    ...
    _ipv4_getaddrinfo._sinoclaw_ipv4_patched = True  # type: ignore[attr-defined]
