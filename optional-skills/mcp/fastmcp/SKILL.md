...
- smoke-test a server with the FastMCP CLI before wiring it into Sinoclaw or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Sinoclaw. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.sinoclaw/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.sinoclaw/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Sinoclaw integration, either:

- configure the server in `~/.sinoclaw/config.yaml` using the `native-mcp` skill, or
...
### Sinoclaw cannot see the deployed server

The server-building part may be correct while the Sinoclaw config is not. Load the `native-mcp` skill and configure the server in `~/.sinoclaw/config.yaml`, then restart Sinoclaw.
