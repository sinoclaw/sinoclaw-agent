---
name: telephony
description: Give Sinoclaw phone capabilities without core tool changes. Provision and persist a Twilio number, send and receive SMS/MMS, make direct calls, and place AI-driven outbound calls through Bland.ai or Vapi.
version: 1.0.0
...
This optional skill gives Sinoclaw practical phone capabilities while keeping telephony out of the core tool list.

It ships with a helper script, `scripts/telephony.py`, that can:
- save provider credentials into `~/.sinoclaw/.env`
...
It does **not** turn Sinoclaw into a real-time inbound phone gateway. Inbound SMS is handled by polling the Twilio REST API. That is enough for many workflows, including notifications and some one-time-code retrieval, without adding core webhook infrastructure.
...
4. Treat third-party phone numbers as sensitive operational data:
   - do not save them to Sinoclaw memory
...
### 1) "I want Sinoclaw to own a real phone number"
Use **Twilio**.
...
- pairs well with Sinoclaw `text_to_speech` plus a public file host or tunnel
...
```bash
SCRIPT="$(find ~/.sinoclaw/skills -path '*/telephony/scripts/telephony.py' -print -quit)"
```
...
Then save credentials into Sinoclaw:
...
3. Buy it and save it into `~/.sinoclaw/.env` + state:
```bash
python3 "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
```bash
python3 "$SCRIPT" twilio-call "+15551230000" --message "Hello! This is Sinoclaw calling with your status update." --voice Polly.Joanna
```

### E. Call with a prerecorded / custom voice message

This is the main path for reusing Sinoclaw's existing `text_to_speech` support.

Use this when:
- you want the call to use Sinoclaw's configured TTS voice rather than Twilio `<Say>`
...
Recommended Sinoclaw TTS -> Twilio Play workflow:

1. Generate the audio with Sinoclaw `text_to_speech`.
2. Make the resulting MP3 publicly reachable.
3. Place the Twilio call with `--audio-url`.

Example agent flow:
- Ask Sinoclaw to create the message audio with `text_to_speech`
...
Important note:
- Sinoclaw TTS is great for prerecorded outbound messages
...
- Sinoclaw STT/TTS alone is not being used here as a full duplex phone conversation engine; that would require a much heavier streaming/webhook integration than this skill is trying to introduce
...
7. Summarize the outcome without persisting third-party numbers to Sinoclaw memory.
...
- Do not store arbitrary third-party phone numbers in Sinoclaw memory.
...
3. persist that number to `~/.sinoclaw/.env`
