...
- `tmux` available for stable authenticated sessions during Sinoclaw terminal calls (desktop app flow only)
...
### Service Account (recommended for Sinoclaw)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.sinoclaw/.env` (the skill will prompt for this on first load).
...
## Sinoclaw Execution Pattern (desktop app flow)

Sinoclaw terminal commands are non-interactive by default and can lose auth context between calls.
