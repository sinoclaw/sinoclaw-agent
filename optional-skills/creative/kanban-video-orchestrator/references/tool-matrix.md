...
toolsets it needs. Only references skills that ship in the public sinoclaw-agent
...
### Visual / rendering skills (`sinoclaw-agent/skills/creative/`)
...
### Design / pre-production skills (`sinoclaw-agent/skills/creative/`)
...
### Audio / media skills (`sinoclaw-agent/skills/creative/` + `skills/media/`)
...
### Kanban infrastructure (`sinoclaw-agent/skills/devops/`)
...
key is present in `~/.sinoclaw/.env` (or macOS Keychain) before firing the kanban.
...
preference: macOS Keychain → `~/.sinoclaw/.env` → environment variable.
