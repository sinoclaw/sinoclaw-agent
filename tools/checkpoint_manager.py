...
DEFAULT_EXCLUDES = [
    ...
    ".hg/",
    ".svn/",
    # Worktrees (Sinoclaw convention — don't recursively snapshot siblings)
    ".worktrees/",
...
def _git_env(
    ...
    The shared store is internal Sinoclaw infrastructure — it must NOT inherit
...
def _migrate_legacy_store(base: Path) -> Optional[Path]:
    ...
    cleared with ``sinoclaw checkpoints clear-legacy``.
    ...
    if legacy_root is not None:
        logger.info(
            ...
            "Clear with `sinoclaw checkpoints clear-legacy` when safe.",
            legacy_root,
        )
    return legacy_root
...
def _init_store(store: Path, working_dir: str) -> Optional[str]:
    ...
    _run_git(["config", "user.name", "Sinoclaw Checkpoint"], store, cfg_wd)
...
# look for ``HEAD``/``info/exclude``/``SINOCLAW_WORKDIR``.  In v2 we also write
...
def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> Optional[str]:
    ...
    # Compat marker for tests that look at SINOCLAW_WORKDIR
    ...
    * its ``last_touch`` is older than ``retention_days`` days.
    ...
    for child in base.iterdir():
        ...
        if delete_orphans:
            ...
            wd_marker = child / "SINOCLAW_WORKDIR"
            if wd_marker.exists():
                try:
...
# ---------------------------------------------------------------------------
# Public helpers for `sinoclaw checkpoints` CLI
# ---------------------------------------------------------------------------
