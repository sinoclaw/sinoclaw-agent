...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return get_session_env("SINOCLAW_SESSION_PLATFORM", "") or ""
    except Exception:
        return os.getenv("SINOCLAW_SESSION_PLATFORM", "") or ""


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set SINOCLAW_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind SINOCLAW_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds SINOCLAW_SESSION_PLATFORM via
    ...
    """
    if os.getenv("SINOCLAW_CRON_SESSION"):
        return False
...
DANGEROUS_PATTERNS = [
    ...
    (r'\bhermes\s+gateway\s+(stop|restart)\b', "stop/restart sinoclaw gateway (kills running agents)"),
    (r'\bhermes\s+update\b', "hermes update (restarts gateway, kills running agents)"),
    # Gateway protection: never start gateway outside systemd management
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart sinoclaw-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart sinoclaw-gateway')"),
...
def check_dangerous_command(command: str, env_type: str,
    ...
    if is_truthy_value(os.getenv("SINOCLAW_YOLO_MODE")) or is_current_session_yolo_enabled():
        return {"approved": True, "message": None}
    ...
    is_cli = os.getenv("SINOCLAW_INTERACTIVE")
    ...
    if is_gateway or os.getenv("SINOCLAW_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    if is_truthy_value(os.getenv("SINOCLAW_YOLO_MODE")) or is_current_session_yolo_enabled() or approval_mode == "off":
        return {"approved": True, "message": None}

    is_cli = os.getenv("SINOCLAW_INTERACTIVE")
    is_gateway = _is_gateway_approval_context()
    is_ask = os.getenv("SINOCLAW_EXEC_ASK")
