...
LOCAL_STT_COMMAND_ENV = "SINOCLAW_LOCAL_STT_COMMAND"
LOCAL_STT_LANGUAGE_ENV = "SINOCLAW_LOCAL_STT_LANGUAGE"
    ...
    if explicit:
        if provider == "local":
            ...
            logger.warning(
                ...
                "(install faster-whisper or set SINOCLAW_LOCAL_STT_COMMAND)"
            )
            return "none"
...
def _transcribe_xai(file_path: str, model_name: str) -> Dict[str, Any]:
    ...
    language = str(
        ...
        or os.getenv("SINOCLAW_LOCAL_STT_LANGUAGE")
        or DEFAULT_LOCAL_STT_LANGUAGE
    ).strip()
