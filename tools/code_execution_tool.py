...
Lets the LLM write a Python script that calls Sinoclaw tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `sinoclaw_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `sinoclaw_tools.py` with file-based RPC stubs
...
# available on every platform Sinoclaw itself runs on.
...
# ---------------------------------------------------------------------------
# sinoclaw_tools.py code generator
# ---------------------------------------------------------------------------
...
def generate_sinoclaw_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the sinoclaw_tools.py stub module.
...
"""Auto-generated Sinoclaw tools RPC stubs."""
...
def _connect():
    ...
    SINOCLAW_RPC_SOCKET can be either:
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["SINOCLAW_RPC_SOCKET"]
...
def _execute_remote(
    ...
    The script and the generated sinoclaw_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/sinoclaw_exec_{sandbox_id}"
    ...
    try:
        ...
        tools_src = generate_sinoclaw_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/sinoclaw_tools.py", tools_src)
...
def execute_code(
    ...
    # --- Set up temp directory with sinoclaw_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="sinoclaw_sandbox_")
    ...
    # selector from SINOCLAW_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    if _use_tcp_rpc:
        ...
        sock_path = os.path.join(_sock_tmpdir, f"sinoclaw_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated sinoclaw_tools module.
        ...
        tools_src = generate_sinoclaw_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "sinoclaw_tools.py"), "w", encoding="utf-8") as f:
            f.write(tools_src)
        ...
        #   connect.  SINOCLAW_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        child_env["SINOCLAW_RPC_SOCKET"] = rpc_endpoint
        ...
        # the staging tmpdir so ``from sinoclaw_tools import ...`` resolves even
        # when the subprocess CWD is not tmpdir (project mode).
        _sinoclaw_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        _existing_pp = child_env.get("PYTHONPATH", "")
        _pp_parts = [tmpdir, _sinoclaw_root]
        if _existing_pp:
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.sinoclaw/.env')).
...
def _load_config() -> dict:
    ...
    ``sinoclaw --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (sinoclaw-agent's python). Reproducible and the
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``sinoclaw tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and sinoclaw-agent's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.sinoclaw/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        "Run a Python script that can call Sinoclaw tools programmatically. "
        ...
        f"Available via `from sinoclaw_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
        ...
        "Also available (no import needed — built into sinoclaw_tools):\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from sinoclaw_tools import {import_str}` "
