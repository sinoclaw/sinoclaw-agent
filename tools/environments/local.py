...
_SINOCLAW_PROVIDER_ENV_FORCE_PREFIX = "_HERMES_FORCE_"
...
_SINOCLAW_PROVIDER_ENV_BLOCKLIST = _build_provider_env_blocklist()
...
def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    ...
    for key, value in (base_env or {}).items():
        if key.startswith(_SINOCLAW_PROVIDER_ENV_FORCE_PREFIX):
            continue
        if key not in _SINOCLAW_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value

    for key, value in (extra_env or {}).items():
        if key.startswith(_SINOCLAW_PROVIDER_ENV_FORCE_PREFIX):
            real_key = key[len(_SINOCLAW_PROVIDER_ENV_FORCE_PREFIX):]
            sanitized[real_key] = value
        elif key not in _SINOCLAW_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value
...
def _find_bash() -> str:
    ...
    custom = os.environ.get("SINOCLAW_GIT_BASH_PATH")
...
def _make_run_env(env: dict) -> dict:
    ...
    for k, v in merged.items():
        if k.startswith(_SINOCLAW_PROVIDER_ENV_FORCE_PREFIX):
            real_key = k[len(_SINOCLAW_PROVIDER_ENV_FORCE_PREFIX):]
            run_env[real_key] = v
        elif k not in _SINOCLAW_PROVIDER_ENV_BLOCKLIST or _is_passthrough(k):
            run_env[k] = v
