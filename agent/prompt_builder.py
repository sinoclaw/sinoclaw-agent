...
from sinoclaw_constants import get_sinoclaw_home, get_skills_dir, is_wsl
...
def _find_sinoclaw_md(cwd: Path) -> Optional[Path]:
    ...
    for directory in [current, *current.parents]:
        for name in _SINOCLAW_MD_NAMES:
...
DEFAULT_AGENT_IDENTITY = (
    "You are Sinoclaw Agent, an intelligent AI assistant created by Sinoclaw Team. "
...
SINOCLAW_AGENT_HELP_GUIDANCE = (
    "If the user asks about configuring, setting up, or using Sinoclaw Agent "
    "itself, load the `sinoclaw-agent` skill with skill_view(name='sinoclaw-agent') "
    "before answering. Docs: https://sinoclaw-agent.nousresearch.com/docs"
)
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.sinoclaw/kanban.db`. Your task id is in "
    "`$SINOCLAW_KANBAN_TASK`; your workspace is `$SINOCLAW_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $SINOCLAW_KANBAN_WORKSPACE` before "
...
PLATFORM_HINTS = {
    ...
    "webui": (
        "You are in the Sinoclaw WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Sinoclaw itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Sinoclaw runs on.
    """
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Sinoclaw itself is running. The host OS, home, and cwd "
                f"of the Sinoclaw process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Sinoclaw "
...
def _skills_prompt_snapshot_path() -> Path:
    return get_sinoclaw_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.sinoclaw/skills/`` directory.  External dirs
    ...
    _platform_hint = (
        os.environ.get("SINOCLAW_PLATFORM")
        or get_session_env("SINOCLAW_SESSION_PLATFORM")
        or ""
    )
...
def load_soul_md() -> Optional[str]:
    """Load SOUL.md from SINOCLAW_HOME and return its content, or None.
    ...
    try:
        ...
        logger.debug("Could not ensure SINOCLAW_HOME before loading SOUL.md: %s", e)

    soul_path = get_sinoclaw_home() / "SOUL.md"
...
def _load_sinoclaw_md(cwd_path: Path) -> str:
    ...
    if not sinoclaw_md_path:
        return ""
    try:
        content = sinoclaw_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = sinoclaw_md_path.name
        try:
            rel = str(sinoclaw_md_path.relative_to(cwd_path))
        except ValueError:
            pass
...
def build_context_files_prompt(cwd: Optional[str] = None, skip_soul: bool = False) -> str:
    ...
    SOUL.md from SINOCLAW_HOME is independent and always included when present.
    ...
    # SOUL.md from SINOCLAW_HOME only — skip when already loaded as identity
    if not skip_soul:
