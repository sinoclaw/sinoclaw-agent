...
2. **Cross-platform compatibility** — Windows, macOS, different Linux distros, different terminal emulators. We want Sinoclaw to work everywhere.
...
Bundled skills (in `skills/`) ship with every Sinoclaw install. They should be **broadly useful to most users**:
...
If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Nous Research Discord](https://discord.gg/NousResearch). Users can install it with `sinoclaw skills install`.
...
Four fields are supported under `metadata.sinoclaw`:
...
The user may skip setup and keep loading the skill. Sinoclaw only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Sinoclaw tools (`web_extract`, `terminal`, `read_file`).
...
Sinoclaw uses a data-driven skin system — no code changes needed to add a new skin.
...
Sinoclaw runs on Linux, macOS, and Windows. When writing code that touches the OS:
...
Sinoclaw has terminal access. Security matters.
...
2. **Test manually**: Run `sinoclaw` and exercise the code path you changed
...
- Use [GitHub Issues](https://github.com/NousResearch/sinoclaw-agent/issues)
- Include: OS, Python version, Sinoclaw version (`sinoclaw version`), full error traceback
