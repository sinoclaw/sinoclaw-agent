"""Windows UTF-8 bootstrap for Sinoclaw entry points.
...
should be imported at the very top of every Sinoclaw entry point
(``hermes``, ``sinoclaw-agent``, ``sinoclaw-acp``, ``python -m gateway.run``,
...
# Apply on import — entry points just need ``import sinoclaw_bootstrap``
# (or ``from sinoclaw_bootstrap import apply_windows_utf8_bootstrap``) at
