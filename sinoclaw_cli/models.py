...
Add, remove, or reorder entries here — both `sinoclaw setup` and
...
_SINOCLAW_USER_AGENT = f"sinoclaw-cli/{_SINOCLAW_VERSION}"
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `sinoclaw model`
    flow without maintaining a separate static list.
    """
    from sinoclaw_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
# $SINOCLAW_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $SINOCLAW_HOME/models_dev_cache.json directly (no network) so this
    runs at import time without blocking. Falls back to ``_XAI_STATIC_FALLBACK``
    when the cache is empty or unreadable. Sinoclaw refreshes the cache from
...
def check_nous_free_tier() -> bool:
    ...
    try:
        from sinoclaw_cli.auth import get_provider_auth_state, resolve_nous_runtime_credentials
...
# this list:  sinoclaw model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `sinoclaw model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `sinoclaw model` TUI
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model a user would be offered first in the ``sinoclaw model`` picker.

    Used as a fallback when the user has configured a provider but never
    selected a model (e.g. ``sinoclaw auth add openai-codex`` without
    ``sinoclaw model``).
    """
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    sinoclaw-agent is tool-calling-first — every provider path assumes the model
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from sinoclaw_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — sinoclaw-agent
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    """
    try:
        from sinoclaw_cli.model_catalog import get_curated_nous_models
...
def fetch_ai_gateway_models(
    ...
    *,
    ...
    from sinoclaw_constants import AI_GATEWAY_BASE_URL
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``sinoclaw model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from sinoclaw_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def parse_model_input(raw: str, current_provider: str) -> tuple[str, str]:
    ...
    Supports ``provider:model`` syntax to switch providers at runtime::
        ...
        nous:sinoclaw-3                           →  ("nous", "sinoclaw-3")
...
def _resolve_static_model_alias(
    ...
    try:
        from sinoclaw_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Sinoclaw' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``sinoclaw_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Sinoclaw should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``sinoclaw auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.sinoclaw/.env``.
    ...
    """
    try:
        from sinoclaw_cli.auth import resolve_api_key_provider_credentials
    ...
    try:
        from sinoclaw_cli.auth import read_credential_pool
        from sinoclaw_cli.copilot_auth import (
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    on the platform appear in ``/model`` without a Sinoclaw release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
    ...
    if normalized == "openai-codex":
        ...
        # a Sinoclaw release). Falls back to the hardcoded catalog if no token
    ...
    if normalized == "nous":
        ...
        try:
            from sinoclaw_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _SINOCLAW_USER_AGENT}
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Sinoclaw' default Claude IDs elsewhere use
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _SINOCLAW_USER_AGENT}
...
def _ollama_cloud_cache_path() -> Path:
    ...
    return get_sinoclaw_home() / "ollama_cloud_models_cache.json"
    ...
    *,
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"Sinoclaw will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Sinoclaw cannot verify the model name."
