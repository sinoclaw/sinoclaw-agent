...
Usage:
    sinoclaw                     # Interactive chat (default)
    sinoclaw chat                # Interactive chat
    ...
    sinoclaw cron list           # List cron jobs
    sinoclaw cron status         # Check if cron scheduler is running
    sinoclaw doctor              # Check configuration and dependencies
    sinoclaw honcho setup                    # Configure Honcho AI memory integration
    sinoclaw honcho status                   # Show Honcho config and connection status
    sinoclaw honcho sessions                 # List directory → session name mappings
    sinoclaw honcho map <name>               # Map current directory to a session name
    sinoclaw honcho peer                     # Show peer names and dialectic settings
    sinoclaw honcho peer --user NAME         # Set user peer name
    sinoclaw honcho peer --ai NAME           # Set AI peer name
    sinoclaw honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    sinoclaw honcho mode                     # Show current memory mode
    sinoclaw honcho mode [hybrid|honcho|local]  # Set memory mode
    sinoclaw honcho tokens                   # Show token budget settings
    sinoclaw honcho tokens --context N       # Set session.context() token cap
    sinoclaw honcho tokens --dialectic N     # Set dialectic result char cap
    sinoclaw honcho identity                 # Show AI peer identity representation
    sinoclaw honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    sinoclaw honcho migrate                  # Step-by-step migration guide: OpenClaw native → Sinoclaw + Honcho
    sinoclaw version             Show version
    sinoclaw update              Update to latest version
    sinoclaw uninstall           Uninstall Sinoclaw Agent
    sinoclaw acp                 Run as an ACP server for editor integration
    sinoclaw sessions browse     Interactive session picker with search

    sinoclaw claw migrate --dry-run  # Preview migration without changes
"""
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (sinoclaw tools, sinoclaw setup, sinoclaw model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'sinoclaw {command_name}' requires an interactive terminal.\n"
...
def _apply_profile_override() -> None:
    ...
    # 2. If no flag, check active_profile in the sinoclaw root
...
# Initialize centralized file logging early — all `sinoclaw` subcommands
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        ...
        cli_args: the original CLI arguments (everything after 'sinoclaw')
    """
    import shutil
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'sinoclaw sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    if not _has_any_provider_configured():
        print()
        print("It looks like Sinoclaw isn't configured yet -- no API keys or providers found.")
        print()
        print("  Run:  sinoclaw setup")
        print()

        from sinoclaw_cli.setup import is_interactive_stdin, print_noninteractive_setup_guidance
...
def cmd_whatsapp(args):
    ...
    if (session_dir / "creds.json").exists():
        ...
        print("⚠ Pairing may not have completed. Run 'sinoclaw whatsapp' to try again.")
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``sinoclaw model``) and the setup wizard
    ...
    from sinoclaw_cli.config import get_compatible_custom_providers, load_config, get_env_value
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from sinoclaw_cli.auth import _prompt_model_selection, _save_model_choice, deactivate_provider
    from sinoclaw_cli.config import get_env_value, save_env_value
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from sinoclaw_cli.config import get_env_value, save_config, save_env_value
    from sinoclaw_cli.nous_subscription import (
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    _codex_token = None
    # Prefer credential pool (where `sinoclaw auth` stores device_code tokens),
...
def _model_flow_custom(config):
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `sinoclaw model` to set a model.")
...
def _model_flow_kimi(config, current_model=""):
    ...
    from sinoclaw_cli.config import get_env_value, save_env_value, load_config, save_config
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    """
    from sinoclaw_cli.auth import _prompt_model_selection, _save_model_choice, deactivate_provider
    from sinoclaw_cli.config import load_config, save_config, get_env_value, save_env_value
    from sinoclaw_cli.models import _PROVIDER_MODELS
    ...
    if selected:
        ...
        # Save the API key env var name so sinoclaw knows where to find it
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    from sinoclaw_cli.config import get_env_value, save_env_value, load_config, save_config
    from sinoclaw_cli.models import fetch_api_models, opencode_model_api_mode, normalize_opencode_model_id
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            print(f"    Sinoclaw will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env.")
            return True
        return False
...
def _model_flow_anthropic(config, current_model=""):
    ...
    if selected:
        ...
        # providers if the user switches without running 'sinoclaw model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``sinoclaw update --gateway`` so interactive prompts (stash restore,
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``sinoclaw web``).
    ...
    if r1.returncode != 0:
        print(f"  {'✗' if fatal else '⚠'} Web UI npm install failed"
              + ("" if fatal else " (sinoclaw web will not be available)"))
        if fatal:
...
def _update_via_zip(args):
    ...
    branch = "main"
    zip_url = f"https://github.com/NousResearch/sinoclaw-agent/archive/refs/heads/{branch}.zip"
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime("sinoclaw-update-autostash-%Y%m%d-%H%M%S")
...
def _restore_stashed_changes(
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes sinoclaw completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print("⚠ Local changes were restored, but Sinoclaw couldn't find the stash entry to drop.")
        ...
        if drop.returncode != 0:
            print("⚠ Local changes were restored, but Sinoclaw couldn't drop the saved stash entry.")
...
# =========================================================================
# Fork detection and upstream management for `sinoclaw update`
# =========================================================================
    ...
    if not has_upstream:
        ...
        if response in ("", "y", "yes"):
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print("  ✓ Added upstream: https://github.com/NousResearch/sinoclaw-agent.git")
                has_upstream = True
            else:
                print("  ✗ Failed to add upstream remote. Skipping upstream sync.")
                return
        else:
            print("  Skipped. Run 'git remote add upstream https://github.com/NousResearch/sinoclaw-agent.git' to add later.")
...
def _invalidate_update_cache():
    ...
    ``sinoclaw update``, every profile is now current.
    """
    homes = []
...
def cmd_update(args):
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            print("  curl -fsSL https://raw.githubusercontent.com/NousResearch/sinoclaw-agent/main/scripts/install.sh | bash")
            sys.exit(1)
    ...
    try:
        ...
        try:
            from sinoclaw_cli.profiles import list_profiles, get_active_profile_name, seed_profile_skills
        ...
        if needs_migration:
            ...
            if gateway_mode:
                ...
                print("    Run 'sinoclaw config migrate' later to apply any new config/env options.")
                response = "n"
            else:
                try:
            ...
            if response in ('', 'y', 'yes'):
                ...
                if gateway_mode and missing_env:
                    print("  ℹ API keys require manual entry: sinoclaw config migrate")
            else:
                print()
                print("Skipped. Run 'sinoclaw config migrate' later to configure.")
        else:
        ...
        # When running as ``sinoclaw update --gateway`` (spawned by the gateway's
        # /update command), this process lives inside the gateway's systemd
        # cgroup.  ``systemctl restart sinoclaw-gateway`` kills everything in the
        ...
        try:
            ...
            if supports_systemd_services():
                ...
                for scope, scope_cmd in [("user", ["systemctl", "--user"]), ("system", ["systemctl"])]:
                    try:
                        result = subprocess.run(
                            scope_cmd + ["list-units", "sinoclaw-gateway*", "--plain", "--no-legend", "--no-pager"],
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            if not parts:
                                continue
                            unit = parts[0]  # e.g. sinoclaw-gateway.service or sinoclaw-gateway-coder.service
            ...
            if is_macos():
                try:
                    from sinoclaw_cli.gateway import launchd_restart, get_launchd_label, get_launchd_plist_path
            ...
            if restarted_services or killed_pids:
                ...
                if killed_pids:
                    ...
                    if len(killed_pids) > 1:
                        print("    (or: sinoclaw -p <profile> gateway run  for each profile)")
        ...
        print("  sinoclaw model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``sinoclaw -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    if action is None:
        # Bare `sinoclaw profile` — show current profile status
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → sinoclaw -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(f"  Choose a custom alias:  sinoclaw profile alias {name} --name <custom>")
                    print(f"  Or access via flag:     sinoclaw -p {name} chat")
                else:
        ...
        name = args.profile_name
        from sinoclaw_cli.profiles import get_profile_dir, profile_exists, _read_config_model, _check_gateway_running, _count_skills
...
def cmd_dashboard(args):
    ...
    try:
        ...
        print("Install them with:  pip install sinoclaw-agent[web]")
        sys.exit(1)
...
def main():
    """Main entry point for sinoclaw CLI."""
...
"""
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with Sinoclaw Agent"
    )
    ...
    login_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use (default: sinoclaw-cli)"
    )
    login_parser.add_argument(
        "--scope",
        default=None,
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Sinoclaw Agent. Use 'sinoclaw debug share' to "
                    "upload a debug report (system info + recent logs) to a paste "
                    "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    sinoclaw debug share              Upload debug report and print URL
    sinoclaw debug share --lines 500  Include more log lines
    sinoclaw debug share --expire 30  Keep paste for 30 days
    sinoclaw debug share --local      Print report locally (no upload)
    sinoclaw debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'sinoclaw debug share'",
    )
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'sinoclaw tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # sinoclaw tools list [--platform cli]
    ...
    # sinoclaw tools disable <name...> [--platform cli]
    ...
    # sinoclaw tools enable <name...> [--platform cli]
    ...
    mcp_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Use 'sinoclaw mcp add' to connect to a new server, or\n"
            "'sinoclaw mcp serve' to expose Sinoclaw conversations over MCP."
        ),
    )
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            # Launch sinoclaw --resume <id> by replacing the current process
            print(f"Resuming session: {selected_id}")
            import shutil
            sinoclaw_bin = shutil.which("sinoclaw")
            if sinoclaw_bin:
                os.execvp(sinoclaw_bin, ["sinoclaw", "--resume", selected_id])
            else:
                # Fallback: re-invoke via python -m
                os.execvp(
                    sys.executable,
                    [sys.executable, "-m", "sinoclaw_cli.main", "--resume", selected_id],
                )
    ...
    logs_parser = subparsers.add_parser(
        ...
        epilog="""\
Examples:
    sinoclaw logs                    Show last 50 lines of agent.log
    sinoclaw logs -f                 Follow agent.log in real time
    sinoclaw logs errors             Show last 50 lines of errors.log
    sinoclaw logs gateway -n 100     Show last 100 lines of gateway.log
    sinoclaw logs --level WARNING    Only show WARNING and above
    sinoclaw logs --session abc123   Filter by session ID
    sinoclaw logs --component tools  Only show tool-related lines
    sinoclaw logs --since 1h         Lines from the last hour
    sinoclaw logs --since 30m -f     Follow, starting from 30 min ago
    sinoclaw logs list               List available log files with sizes
""",
    )
    ...
    # e.g. ``sinoclaw -c Pokemon Agent Dev`` → ``sinoclaw -c 'Pokemon Agent Dev'``
    ...
    # fails (e.g. 'sinoclaw -c model' where 'model' is consumed as the
