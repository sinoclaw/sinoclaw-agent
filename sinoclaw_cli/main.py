...
Usage:
    ...
    sinoclaw gateway status      # Show gateway status
    ...
    sinoclaw-agent honcho migrate                  # Step-by-step migration guide: OpenClaw native → Sinoclaw + Honcho
...
def _add_accept_hooks_flag(parser) -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(equivalent to SINOCLAW_ACCEPT_HOOKS=1 / hooks_auto_accept: true)."
        ),
    )
...
def _apply_profile_override() -> None:
    ...
    # Mirrors sinoclaw_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If SINOCLAW_HOME is already set and no explicit flag was given, trust it.
    ...
    if profile_name is None:
        try:
            ...
            active_path = get_default_sinoclaw_root() / "active_profile"
    ...
    if profile_name is not None:
        try:
            ...
            sinoclaw_home = resolve_profile_env(profile_name)
...
load_sinoclaw_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → SINOCLAW_REDACT_SECRETS env
# var BEFORE sinoclaw_logging imports agent.redact (which snapshots the flag at
# module-import time). Without this, config.yaml's toggle is ignored because
# the setup_logging() call below imports agent.redact, which reads the env var
# exactly once. Env var in .env still wins — this is config.yaml fallback only.
try:
    if "SINOCLAW_REDACT_SECRETS" not in os.environ:
        import yaml as _yaml_early

        _cfg_path = get_sinoclaw_home() / "config.yaml"
        if _cfg_path.exists():
            ...
            if isinstance(_early_sec_cfg, dict):
                ...
                if _early_redact is not None:
                    os.environ["SINOCLAW_REDACT_SECRETS"] = str(_early_redact).lower()
            del _early_sec_cfg
        del _cfg_path
except Exception:
...
try:
    from sinoclaw_cli.config import load_config as _load_config_early
    from sinoclaw_constants import apply_ipv4_preference as _apply_ipv4
...
from sinoclaw_constants import AI_GATEWAY_BASE_URL, OPENROUTER_BASE_URL
...
def _has_any_provider_configured() -> bool:
    ...
    from sinoclaw_cli.config import get_env_path, get_sinoclaw_home, load_config
    from sinoclaw_cli.auth import get_auth_status

    # Determine whether Sinoclaw itself has been explicitly configured (model
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_sinoclaw_config = _model_name and _model_name != _DEFAULT_MODEL
    ...
    auth_file = get_sinoclaw_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if Sinoclaw has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Sinoclaw to use their tokens.
    if _has_sinoclaw_config:
        try:
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @sinoclaw/ink is missing or node_modules is behind package-lock.json.
...
def _tui_build_needed(tui_dir: Path) -> bool:
    if _sinoclaw_ink_bundle_stale(tui_dir):
        return True
...
def _sinoclaw_ink_bundle_stale(tui_dir: Path) -> bool:
    ink_root = tui_dir / "packages" / "sinoclaw-ink"
...
def _ensure_tui_node() -> None:
    ...
    ``SINOCLAW_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("SINOCLAW_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "SINOCLAW_HOME": sinoclaw_home},
            capture_output=True,
            text=True,
            check=False,
        )
    ...
    extras.extend([Path(sinoclaw_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    ...
    if _tui_need_npm_install(tui_dir):
        if not os.environ.get("SINOCLAW_QUIET"):
    ...
    if tui_dev:
        if _sinoclaw_ink_bundle_stale(tui_dir):
            result = subprocess.run(
                [npm, "run", "build", "--prefix", "packages/sinoclaw-ink"],
                cwd=str(tui_dir),
                capture_output=True,
                text=True,
            )
            if result.returncode != 0:
                ...
                preview = "\n".join(combined.splitlines()[-30:])
                print("@sinoclaw/ink build failed.")
                if preview:
                    print(preview)
                sys.exit(1)
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from sinoclaw_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="sinoclaw-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["SINOCLAW_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env["SINOCLAW_PYTHON_SRC_ROOT"] = os.environ.get(
        "SINOCLAW_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("SINOCLAW_PYTHON", sys.executable)
    env.setdefault("SINOCLAW_CWD", os.getcwd())
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["SINOCLAW_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    if model:
        env["SINOCLAW_MODEL"] = model
        env["SINOCLAW_INFERENCE_MODEL"] = model
    if provider:
        env["SINOCLAW_TUI_PROVIDER"] = provider
        env["SINOCLAW_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["SINOCLAW_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["SINOCLAW_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["SINOCLAW_TUI_SKILLS"] = value
    if query:
        env["SINOCLAW_TUI_QUERY"] = query
    if image:
        env["SINOCLAW_TUI_IMAGE"] = image
    if checkpoints:
        env["SINOCLAW_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["SINOCLAW_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["SINOCLAW_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["SINOCLAW_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["SINOCLAW_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["SINOCLAW_ACCEPT_HOOKS"] = "1"
    ...
    if resume_session_id:
        env["SINOCLAW_TUI_RESUME"] = resume_session_id
...
def _pin_kanban_board_env() -> None:
    ...
    """
    if os.environ.get("SINOCLAW_KANBAN_BOARD"):
        return
    try:
        from sinoclaw_cli.kanban_db import get_current_board

        os.environ["SINOCLAW_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass
...
def cmd_chat(args):
    ...
    use_tui = getattr(args, "tui", False) or os.environ.get("SINOCLAW_TUI") == "1"
    ...
    if not _has_any_provider_configured():
        ...
        if reply in ("", "y", "yes"):
            ...
            return
        print()
        print("You can run 'sinoclaw setup' at any time to configure.")
        sys.exit(1)
    ...
    # user's ~/.sinoclaw/config.yaml and return built-in defaults. Set BEFORE
    # importing cli (which runs `CLI_CONFIG = load_cli_config()` at module
    # import time). Credentials in .env are still loaded — this flag only
    # ignores behavioral/config settings.
    if getattr(args, "ignore_user_config", False):
        os.environ["SINOCLAW_IGNORE_USER_CONFIG"] = "1"

    # --ignore-rules: skip auto-injection of AGENTS.md/SOUL.md/.cursorrules
    # (rules), memory entries, and any preloaded skills coming from user config.
    # Maps to AIAgent(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        os.environ["SINOCLAW_IGNORE_RULES"] = "1"
...
def cmd_gateway(args):
    ...
    from sinoclaw_cli.gateway import gateway_command
...
def cmd_whatsapp(args):
    ...
    from sinoclaw_cli.config import get_env_value, save_env_value
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Sinoclaw?")
        print()
    ...
    session_dir = get_sinoclaw_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in ("y", "yes"):
            ...
            print("  Start the gateway with: sinoclaw gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  sinoclaw gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Sinoclaw Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  sinoclaw gateway")
...
def select_provider_and_model(args=None):
    ...
    effective_provider = (
        config_provider or os.getenv("SINOCLAW_INFERENCE_PROVIDER") or "auto"
    )
    ...
    # a leftover OPENAI_BASE_URL in ~/.sinoclaw/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.sinoclaw/.env if the active provider is not 'custom'.
    ...
    """
    from sinoclaw_cli.config import get_env_value, save_env_value, load_config
...
#
# Sinoclaw uses lightweight "auxiliary" models for side tasks (vision analysis,
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Sinoclaw only falls back to a lightweight backend (OpenRouter,")
...
def _aux_flow_provider_model(
    ...
    from sinoclaw_cli.auth import _prompt_model_selection
    from sinoclaw_cli.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from sinoclaw_cli.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from sinoclaw_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    from sinoclaw_cli.models import ai_gateway_model_ids, get_pricing_for_provider
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from sinoclaw_cli.nous_subscription import prompt_enable_tool_gateway
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from sinoclaw_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from sinoclaw_cli.codex_models import get_codex_model_ids
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from sinoclaw_cli.models import fetch_api_models
...
def _model_flow_google_gemini_cli(_config, current_model=""):
    ...
    Flow:
      ...
      5. Save to ~/.sinoclaw/config.yaml.
    """
    from sinoclaw_cli.auth import (
...
def _model_flow_custom(config):
    ...
    """
    from sinoclaw_cli.auth import _save_model_choice, deactivate_provider
    from sinoclaw_cli.config import get_env_value, load_config, save_config
    ...
    from sinoclaw_cli.models import probe_api_models
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from sinoclaw_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from sinoclaw_cli.config import (
        get_env_value,
        save_env_value,
        load_config,
    ...
    from sinoclaw_cli import azure_detect
    import getpass
    ...
    print("Anthropic-style API endpoints.  Sinoclaw will probe your")
...
def _remove_custom_provider(config):
    ...
    try:
        ...
        idx = menu.show()
        from sinoclaw_cli.curses_ui import flush_stdin
...
def _model_flow_copilot(config, current_model=""):
    ...
    from sinoclaw_cli.config import save_env_value, load_config, save_config
    from sinoclaw_cli.models import (
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from sinoclaw_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from sinoclaw_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates Sinoclaw turns to `copilot --acp`.")
    print("  Sinoclaw currently starts its own ACP subprocess for each request.")
    print("  Sinoclaw uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print(
            "  Set SINOCLAW_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere."
        )
        return
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    ...
    recover from a malformed paste without editing ``~/.sinoclaw/.env`` by hand.
    ...
    from sinoclaw_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `sinoclaw setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "gemini" and existing_key:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    "   Sinoclaw typically makes 3-10 API calls per user turn "
                ...
                print()
                print(
                    "   To use Gemini with Sinoclaw, enable billing on your "
    ...
    if provider_id == "lmstudio":
        ...
        from sinoclaw_cli.models import fetch_lmstudio_models
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            print(
                f"    Sinoclaw will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from sinoclaw_cli.auth import get_anthropic_key
...
def cmd_logout(args):
    ...
    from sinoclaw_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from sinoclaw_cli.auth_commands import auth_command
...
def cmd_cron(args):
    """Cron job management."""
    from sinoclaw_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from sinoclaw_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    if sub == "manifest":
        from sinoclaw_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from sinoclaw_cli.kanban import kanban_command
...
def cmd_hooks(args):
    ...
    from sinoclaw_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from sinoclaw_cli.doctor import run_doctor
...
def cmd_dump(args):
    ...
    from sinoclaw_cli.dump import run_dump
...
def cmd_config(args):
    ...
    from sinoclaw_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up Sinoclaw home directory to a zip file."""
    if getattr(args, "quick", False):
        from sinoclaw_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from sinoclaw_cli.backup import run_backup
...
def cmd_import(args):
    ...
    from sinoclaw_cli.backup import run_import
...
def cmd_version(args):
    """Show version."""
    print(f"Sinoclaw Agent v{__version__} ({__release_date__})")
    ...
    try:
        from sinoclaw_cli.banner import check_for_updates
...
def cmd_uninstall(args):
    """Uninstall Sinoclaw Agent."""
    _require_tty("uninstall")
    from sinoclaw_cli.uninstall import run_uninstall
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    The Vite build outputs to ``sinoclaw_cli/web_dist/`` (per vite.config.ts
    outDir: "../sinoclaw_cli/web_dist"), NOT to ``web/dist/``.  Uses the Vite
    manifest as the sentinel because it is written last and therefore has the
    newest mtime of any build output.
    """
    dist_dir = web_dir.parent / "sinoclaw_cli" / "web_dist"
...
def _update_via_zip(args):
    ...
    try:
        tmp_dir = tempfile.mkdtemp(prefix="sinoclaw-update-")
        zip_path = os.path.join(tmp_dir, f"sinoclaw-agent-{branch}.zip")
        ...
        # GitHub ZIPs extract to sinoclaw-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"sinoclaw-agent-{branch}")
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "sinoclaw-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Sinoclaw behaves unexpectedly.")
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Sinoclaw couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Sinoclaw couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Sinoclaw behaves unexpectedly.")
    return True
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_sinoclaw_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_sinoclaw_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _invalidate_update_cache():
    ...
    default_home = get_default_sinoclaw_root()
...
def _load_installable_optional_extras() -> list[str]:
    ...
    # Entries look like "sinoclaw-agent[matrix]" or "package-name[extra]".
...
class _UpdateOutputStream:
    ...
    * Every write is also mirrored to an append-only log file
      (``~/.sinoclaw/logs/update.log``) that users can inspect after the
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.sinoclaw/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    try:
        ...
        # sinoclaw_cli.config.get_sinoclaw_home to simulate setup failure.
        from sinoclaw_cli.config import get_sinoclaw_home as _get_sinoclaw_home

        logs_dir = _get_sinoclaw_home() / "logs"
...
def _ensure_fhs_path_guard() -> None:
    ...
    path_comment = (
        "# Sinoclaw Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of SINOCLAW_HOME before running the update.
    ...
    # Render path using display_sinoclaw_home so the user sees ~/.sinoclaw/...
    try:
        from sinoclaw_constants import get_sinoclaw_home, display_sinoclaw_home

        home = get_sinoclaw_home()
        try:
            display_path = f"{display_sinoclaw_home()}/{out_path.relative_to(home)}"
        except ValueError:
...
def cmd_update(args):
    """Update Sinoclaw Agent to the latest version.

    Thin wrapper around ``_cmd_update_impl``: installs hangup protection,
    runs the update, then restores stdio on the way out (even on
    ``sys.exit`` or unhandled exceptions).
    """
    from sinoclaw_cli.config import is_managed, managed_error

    if is_managed():
        managed_error("update Sinoclaw Agent")
        return
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        # the old bytecode (e.g. get_sinoclaw_home added to sinoclaw_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # modules in this process.  Reload sinoclaw_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_sinoclaw_home() added since the last release.
        try:
            import importlib
            import sinoclaw_constants as _hc
        ...
        # seed_profile_skills() uses subprocess with an explicit SINOCLAW_HOME so
        # it is not affected by sync_skills()'s module-level SINOCLAW_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's SINOCLAW_HOME env var points at the default or a named profile.
        try:
        ...
        if gateway_mode:
            _exit_code_path = get_sinoclaw_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            # Discover all sinoclaw-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. sinoclaw-gateway.service or sinoclaw-gateway-coder.service
                            ...
                            # path in `sinoclaw gateway restart`
        ...
        try:
            from sinoclaw_cli.gateway import (
                ...
                _find_legacy_sinoclaw_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_sinoclaw_units():
                print()
                print("⚠ Legacy Sinoclaw gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_sinoclaw_units():
                ...
                print()
                print("    sinoclaw gateway migrate-legacy")
                print()
...
def cmd_profile(args):
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.sinoclaw)")
            else:
        ...
        from sinoclaw_cli.profiles import profile_exists
        ...
        from sinoclaw_cli.profiles import rename_profile
        ...
        from sinoclaw_cli.profiles import export_profile
        ...
        from sinoclaw_cli.profiles import import_profile
        ...
        import tempfile
        from sinoclaw_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="sinoclaw_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
        ...
        from sinoclaw_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        from sinoclaw_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("sinoclaw_requires"):
            print(f"Requires:     Sinoclaw {data['sinoclaw_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from sinoclaw_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.sinoclaw_requires:
        print(f"  Requires: Sinoclaw {mf.sinoclaw_requires}")
...
def cmd_dashboard(args):
    ...
    from sinoclaw_cli.web_server import start_server

    embedded_chat = args.tui or os.environ.get("SINOCLAW_DASHBOARD_TUI") == "1"
    start_server(
        host=args.host,
        port=args.port,
...
def cmd_completion(args, parser=None):
    ...
    from sinoclaw_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_logs(args):
    """View and filter Sinoclaw log files."""
    from sinoclaw_cli.logs import tail_log, list_logs
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from sinoclaw_cli.models import CANONICAL_PROVIDERS as _cp
...
def main():
    ...
    from sinoclaw_cli._parser import build_top_level_parser
    ...
    model_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use for Nous login (default: sinoclaw-cli)",
    )
    model_parser.add_argument(
    ...
    # =========================================================================
    from sinoclaw_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://sinoclaw-agent.nousresearch.com/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Sinoclaw Agent with an interactive wizard. "
        "Run a specific section: sinoclaw setup model|tts|terminal|gateway|tools|agent",
    )
    ...
    setup_parser.add_argument(
        ...
        "compatibility — a bare 'sinoclaw setup' now does this.",
    )
    setup_parser.add_argument(
        "--quick",
        action="store_true",
    ...
    slack_parser = subparsers.add_parser(
        ...
        description="Slack integration helpers for Sinoclaw.",
    )
    ...
    slack_manifest.add_argument(
        ...
        "writes to $SINOCLAW_HOME/slack-manifest.json.",
    )
    slack_manifest.add_argument(
        "--name",
        default=None,
        help='Bot display name (default: "Sinoclaw")',
    )
    slack_manifest.add_argument(
        "--description",
        default=None,
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: sinoclaw-cli)"
    )
    ...
    auth_spotify = auth_subparsers.add_parser(
        "spotify", help="Authenticate Sinoclaw with Spotify via PKCE"
    )
    ...
    auth_spotify.add_argument(
        "--client-id", help="Spotify app client_id (or set SINOCLAW_SPOTIFY_CLIENT_ID)"
    )
    auth_spotify.add_argument(
        "--redirect-uri",
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Sinoclaw Agent components",
    )
    ...
    cron_create.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.sinoclaw/scripts/. Default mode: "
    ...
    cron_edit.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.sinoclaw/scripts/. Pass empty string to clear. "
    ...
    # =========================================================================
    from sinoclaw_cli.kanban import build_parser as _build_kanban_parser
    ...
    hooks_parser = subparsers.add_parser(
        ...
        description=(
            "Inspect shell-script hooks declared in ~/.sinoclaw/config.yaml, "
            "test them against synthetic payloads, and manage the first-use "
            "consent allowlist at ~/.sinoclaw/shell-hooks-allowlist.json."
        ),
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with Sinoclaw Agent setup",
    )
    ...
    dump_parser = subparsers.add_parser(
        ...
        description="Output a compact, plain-text summary of your Sinoclaw setup "
...
""",
    ...
    backup_parser = subparsers.add_parser(
        "backup",
        help="Back up Sinoclaw home directory to a zip file",
        description="Create a zip archive of your entire Sinoclaw configuration, "
        "skills, sessions, and data (excludes the sinoclaw-agent codebase). "
    ...
    from sinoclaw_cli.checkpoints import register_cli as _register_checkpoints_cli
    ...
    import_parser = subparsers.add_parser(
        "import",
        help="Restore a Sinoclaw backup from a zip file",
        description="Extract a previously created Sinoclaw backup into your "
        "Sinoclaw home directory, restoring configuration, skills, "
        "sessions, and data",
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Sinoclaw Agent configuration",
    )
    ...
    def cmd_pairing(args):
        from sinoclaw_cli.pairing import pairing_command
    ...
    def cmd_skills(args):
        ...
        if getattr(args, "skills_action", None) == "config":
            ...
            from sinoclaw_cli.skills_config import skills_command as skills_config_command

            skills_config_command(args)
        else:
            from sinoclaw_cli.skills_hub import skills_command
    ...
    def cmd_plugins(args):
        from sinoclaw_cli.plugins_cmd import plugins_command
    ...
    try:
        from sinoclaw_cli.curator import register_cli as _register_curator_cli
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            if not existing:
                print(
                    f"\n  Nothing to reset — no memory files found in {display_sinoclaw_home()}/memories/\n"
                )
                return
            ...
            print(f"  Files were in: {display_sinoclaw_home()}/memories/\n")
        else:
            from sinoclaw_cli.memory_setup import memory_command
    ...
    def cmd_tools(args):
        ...
        if action in ("list", "disable", "enable"):
            from sinoclaw_cli.tools_config import tools_disable_enable_command

            tools_disable_enable_command(args)
        else:
            _require_tty("tools")
            from sinoclaw_cli.tools_config import tools_command
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Sinoclaw as an MCP server",
        description=(
            "Manage MCP server connections and run Sinoclaw as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'sinoclaw-agent mcp add' to connect to a new server, or\n"
            "'sinoclaw-agent mcp serve' to expose Sinoclaw conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Sinoclaw as an MCP server (expose conversations to other agents)",
    )
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to Sinoclaw",
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to Sinoclaw",
    ...
    def cmd_claw(args):
        from sinoclaw_cli.claw import claw_command
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Sinoclaw Agent to the latest version",
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall Sinoclaw Agent",
        description="Remove Sinoclaw Agent from your system. Can keep configs/data for reinstall.",
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run Sinoclaw Agent as an ACP (Agent Client Protocol) server",
        description="Start Sinoclaw Agent in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )
    _add_accept_hooks_flag(acp_parser)

    def cmd_acp(args):
        """Launch Sinoclaw Agent as an ACP server."""
        try:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated Sinoclaw instances",
    )
    ...
    profile_install = profile_subparsers.add_parser(
        ...
        description=(
            "Install a Sinoclaw profile distribution. SOURCE can be a git URL "
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Sinoclaw Agent web dashboard for managing config, API keys, and sessions",
    )
    ...
    logs_parser = subparsers.add_parser(
        "logs",
        help="View and filter Sinoclaw log files",
...
""",
    ...
    if getattr(args, "oneshot", None):
        from sinoclaw_cli.oneshot import run_oneshot
