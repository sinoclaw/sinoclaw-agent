"""`sinoclaw checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.sinoclaw/checkpoints/``.  Actions:

    sinoclaw checkpoints               # same as `status`
    sinoclaw checkpoints status        # total size, project count, breakdown
    sinoclaw checkpoints list          # per-project checkpoint counts + workdir
    sinoclaw checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    sinoclaw checkpoints clear [-f]    # nuke the entire base (asks first)
    sinoclaw checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    sinoclaw checkpoints
    sinoclaw checkpoints prune --retention-days 3 --max-size-mb 200
    sinoclaw checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: sinoclaw checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``sinoclaw checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `sinoclaw checkpoints` → status
