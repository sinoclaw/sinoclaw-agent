...
Used by ``sinoclaw kanban specify [task_id | --all]``. Takes a task that
...
* This module intentionally mirrors ``sinoclaw_cli/goals.py`` — same aux
...
_SYSTEM_PROMPT = """You are the Kanban triage specifier for the Sinoclaw Agent board.
...
def _profile_author() -> str:
    """Mirror of ``sinoclaw_cli.kanban._profile_author``. Kept local to
    avoid a circular import when kanban.py imports this module."""
    return (
        os.environ.get("SINOCLAW_PROFILE")
