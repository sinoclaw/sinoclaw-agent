...
Saves per-platform tool configuration to ~/.sinoclaw/config.yaml under
...
from sinoclaw_cli.cli_output import (  # noqa: E402 — late import block
...
# They're still in _SINOCLAW_CORE_TOOLS (available at runtime if enabled),
...
def _get_effective_configurable_toolsets():
    ...
    try:
        from sinoclaw_cli.plugins import discover_plugins, get_plugin_toolsets
...
from sinoclaw_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    ...
                    # extra keys required. SINOCLAW_CUA_DRIVER_VERSION is an
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    {"key": "SINOCLAW_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)", "url": "https://cloud.langfuse.com"},
                    {"key": "SINOCLAW_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)", "url": "https://cloud.langfuse.com"},
                ],
                "post_setup": "langfuse",
            },
            {
                ...
                "env_vars": [
                    {"key": "SINOCLAW_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)"},
                    {"key": "SINOCLAW_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)"},
                    {"key": "SINOCLAW_LANGFUSE_BASE_URL", "prompt": "Langfuse server URL (e.g. http://localhost:3000)", "default": "http://localhost:3000"},
                ],
                "post_setup": "langfuse",
            },
        ],
    ...
    if post_setup_key in ("agent_browser", "browserbase"):
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_sinoclaw_home()}/sinoclaw-agent && npm install")
                if result.stderr:
                    _print_info(f"      {result.stderr.strip()[:200]}")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in sinoclaw-agent directory)")
            return
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      docker pull ghcr.io/nousresearch/sinoclaw-agent:latest"
            )
            return
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.sinoclaw/config.yaml")
        ...
        try:
            from sinoclaw_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
    ...
    *,
    ...
    if toolset_names is None or not isinstance(toolset_names, list):
        ...
        if plat_info:
            ...
            default_ts = f"sinoclaw-{platform}"
    ...
    # "sinoclaw-cli" (which include all _SINOCLAW_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # (e.g. "sinoclaw-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
    ...
    skip |= {k for k in TOOLSETS if k.startswith("sinoclaw-")}
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (sinoclaw-cli, sinoclaw-telegram, etc.)
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    ...
    print()
    from sinoclaw_constants import display_sinoclaw_home
    print(color(f"  Tool configuration saved to {display_sinoclaw_home()}/config.yaml", Colors.DIM))
