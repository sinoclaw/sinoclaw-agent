...
from sinoclaw_cli.config import get_env_value, load_config
...
def get_nous_subscription_explainer_lines() -> list[str]:
    ...
    return [
        ...
        "Change these later with: sinoclaw setup tools, sinoclaw setup terminal, or sinoclaw status.",
    ]


def apply_nous_provider_defaults(config: Dict[str, object]) -> set[str]:
    """Apply provider-level Nous defaults shared by `sinoclaw setup` and `sinoclaw model`."""
    if not managed_nous_tools_enabled():
        return set()
