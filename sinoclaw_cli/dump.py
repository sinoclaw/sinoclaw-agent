"""
Dump command for sinoclaw CLI.

Outputs a compact, plain-text summary of the user's Sinoclaw setup
...
from sinoclaw_cli.config import get_sinoclaw_home, get_env_path, get_project_root, load_config
...
def _cron_summary(sinoclaw_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = sinoclaw_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"sinoclaw_home:      {display_sinoclaw_home()}")
    ...
    toolsets = config.get("toolsets", ["sinoclaw-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(sinoclaw_home)}")
    lines.append(f"  skills:             {_count_skills(sinoclaw_home)}")
