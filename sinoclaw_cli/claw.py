"""sinoclaw claw — OpenClaw migration commands.
...
Usage:
    ...
    sinoclaw claw migrate --preset full --overwrite  # Full migration, overwrite conflicts
...
def _warn_if_gateway_running(auto_yes: bool) -> None:
    ...
    print_info("Recommendation: stop the gateway first with 'sinoclaw stop'.")
    print()
...
def _find_migration_script() -> Path | None:
    """Find the openclaw_to_sinoclaw.py script in known locations."""
...
def _load_migration_module(script_path: Path):
    ...
    spec = importlib.util.spec_from_file_location("openclaw_to_sinoclaw", script_path)
...
def claw_command(args):
    ...
    if action == "migrate":
        ...
        print()
        print("Run 'sinoclaw claw <command> --help' for options.")
...
def _cmd_migrate(args):
    ...
    # 'sinoclaw claw cleanup' separately.
