...
def _write_codex_cli_tokens(
    ...
    *,
    ...
    When Sinoclaw refreshes a token it consumes the old refresh_token; if we
...
def resolve_codex_runtime_credentials(
    *,
    ...
    try:
        ...
        if cli_tokens:
            logger.info("Migrating Codex credentials from ~/.codex/ to Sinoclaw auth store")
            print("⚠️  Migrating Codex credentials to Sinoclaw's own auth store.")
            print("   This avoids conflicts with Codex CLI and VS Code.")
            print("   Run `sinoclaw auth` to create a fully independent session.\n")
    ...
    *,
    ...
    if effective_ca:
        ...
        if not os.path.isfile(ca_path):
            import logging
            logging.getLogger("sinoclaw.auth").warning(
...
def get_nous_auth_status() -> Dict[str, Any]:
    """Status snapshot for `sinoclaw status` output.

    Checks the credential pool first (where the dashboard device-code flow
    and ``sinoclaw auth`` store credentials), then falls back to the legacy
...
def login_command(args) -> None:
    ...
    print("The 'sinoclaw login' command has been removed.")
    print("Use 'sinoclaw auth' to manage credentials,")
