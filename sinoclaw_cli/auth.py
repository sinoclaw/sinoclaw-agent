"""
Multi-provider authentication system for Sinoclaw Agent.

Supports OAuth device code flows (Nous Portal, future: OpenAI Codex) and
traditional API key providers (OpenRouter, custom endpoints). Auth state
is persisted in ~/.sinoclaw/auth.json with cross-process file locking.
...
from sinoclaw_cli.config import get_sinoclaw_home, get_config_path, read_raw_config
...
DEFAULT_NOUS_CLIENT_ID = "sinoclaw-cli"
...
SPOTIFY_DOCS_URL = "https://sinoclaw-agent.nousresearch.com/docs/user-guide/features/spotify"
...
def get_anthropic_key() -> str:
    ...
    """
    from sinoclaw_cli.config import get_env_value
...
def _resolve_api_key_provider_secret(
    ...
    if provider_id == "copilot":
        ...
        try:
            from sinoclaw_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token
    ...
    for env_var in pconfig.api_key_env_vars:
        # Check both os.environ and ~/.sinoclaw/.env file
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Run `sinoclaw model` to re-authenticate."
...
def _oauth_trace_enabled() -> bool:
    raw = os.getenv("SINOCLAW_OAUTH_TRACE", "").strip().lower()
...
# =============================================================================
# Auth Store — persistence layer for ~/.sinoclaw/auth.json
# =============================================================================

def _auth_file_path() -> Path:
    path = get_sinoclaw_home() / "auth.json"
    # Seat belt: if pytest is running and SINOCLAW_HOME resolves to the real
    # user's auth store, refuse rather than silently corrupt it. This catches
    # tests that forgot to monkeypatch SINOCLAW_HOME, tests invoked without the
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        if resolved == real_home_auth:
            raise RuntimeError(
                ...
                "Set SINOCLAW_HOME to a tmp_path in your test fixture, or run "
...
def _global_auth_file_path() -> Optional[Path]:
    ...
    directory (classic mode, or custom SINOCLAW_HOME that is not a profile).
    ...
    try:
        ...
        global_root = get_default_sinoclaw_root()
    except Exception:
        return None
    profile_home = get_sinoclaw_home()
    try:
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    ``~/.sinoclaw/auth.json`` even when SINOCLAW_HOME is set to a profile
    path. The hermetic conftest does not redirect ``HOME``, so
    ``get_default_sinoclaw_root()`` for a profile-shaped SINOCLAW_HOME can
...
def read_credential_pool(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    ``sinoclaw auth add <provider>`` inside the profile, profile entries
...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    # not by the user explicitly configuring anthropic in Sinoclaw.
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    """
    Clear auth state for a provider. Used by `sinoclaw logout`.
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    """
    try:
        from sinoclaw_cli.config import validate_config_structure
        issues = validate_config_structure()
        if not issues:
            return ""

        lines = ["Config issue detected — run 'sinoclaw doctor' for full diagnostics:"]
        for ci in issues:
    ...
    *,
    ...
    if normalized != "auto":
        ...
        if _config_hint:
            ...
            msg += " Check 'sinoclaw model' for available providers, or run 'sinoclaw doctor' to diagnose config issues."
    ...
    raise AuthError(
        "No inference provider configured. Run 'sinoclaw model' to choose a "
        "provider and model, or set an API key (OPENROUTER_API_KEY, "
        "OPENAI_API_KEY, etc.) in ~/.sinoclaw/.env.",
...
def resolve_qwen_runtime_credentials(
    *,
    ...
    base_url = os.getenv("SINOCLAW_QWEN_BASE_URL", "").strip().rstrip("/") or DEFAULT_QWEN_BASE_URL
    return {
        "provider": "qwen-oauth",
        "base_url": base_url,
...
#
# Tokens live in ~/.sinoclaw/auth/google_oauth.json (managed by agent.google_oauth).
...
def get_gemini_oauth_auth_status() -> Dict[str, Any]:
    """Return a status dict for `sinoclaw auth list` / `sinoclaw status`."""
    try:
...
# Spotify auth — PKCE tokens stored in ~/.sinoclaw/auth.json
# =============================================================================
...
def _spotify_client_id(
    ...
    candidates = (
        explicit,
        get_env_value("SINOCLAW_SPOTIFY_CLIENT_ID"),
    ...
    for candidate in candidates:
        ...
        if cleaned:
            return cleaned
    raise AuthError(
        "Spotify client_id is required. Set SINOCLAW_SPOTIFY_CLIENT_ID or pass --client-id.",
        provider="spotify",
...
def _spotify_redirect_uri(
    ...
    candidates = (
        explicit,
        get_env_value("SINOCLAW_SPOTIFY_REDIRECT_URI"),
...
def _spotify_api_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("SINOCLAW_SPOTIFY_API_BASE_URL"),
...
def _spotify_accounts_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("SINOCLAW_SPOTIFY_ACCOUNTS_BASE_URL"),
...
def _refresh_spotify_oauth_state(
    ...
    *,
    ...
    if not refresh_token:
        raise AuthError(
            "Spotify refresh token missing. Run `sinoclaw auth spotify` again.",
            provider="spotify",
    ...
    if response.status_code >= 400:
        detail = response.text.strip()
        raise AuthError(
            "Spotify token refresh failed. Run `sinoclaw auth spotify` again."
            + (f" Response: {detail}" if detail else ""),
            provider="spotify",
            code="spotify_refresh_failed",
            relogin_required=True,
...
def resolve_spotify_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "Spotify is not authenticated. Run `sinoclaw auth spotify` first.",
                provider="spotify",
    ...
    if not access_token:
        raise AuthError(
            "Spotify access token missing. Run `sinoclaw auth spotify` again.",
            provider="spotify",
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    resulting client_id to ~/.sinoclaw/.env, and return it.
    ...
    print("       App name:     anything (e.g. sinoclaw-agent)")
    ...
    # Persist so subsequent `sinoclaw auth spotify` runs skip the wizard.
    save_env_value("SINOCLAW_SPOTIFY_CLIENT_ID", raw)
    ...
    if redirect_uri_hint and redirect_uri_hint != DEFAULT_SPOTIFY_REDIRECT_URI:
        save_env_value("SINOCLAW_SPOTIFY_REDIRECT_URI", redirect_uri_hint)

    print()
    print("Saved SINOCLAW_SPOTIFY_CLIENT_ID to ~/.sinoclaw/.env")
    print()
    return raw
...
def login_spotify_command(args) -> None:
    ...
    # with "SINOCLAW_SPOTIFY_CLIENT_ID is required".
    ...
    print()
    print("Open this URL to authorize Sinoclaw:")
    print(authorize_url)
    print()
...
# =============================================================================
# OpenAI Codex auth — tokens stored in ~/.sinoclaw/auth.json (not ~/.codex/)
#
# Sinoclaw maintains its own Codex OAuth session separate from the Codex CLI
# and VS Code extension. This prevents refresh token rotation conflicts
# where one app's refresh invalidates the other's session.
# =============================================================================

def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from Sinoclaw auth store (~/.sinoclaw/auth.json).
    ...
    if not state:
        raise AuthError(
            "No Codex credentials stored. Run `sinoclaw auth` to authenticate.",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "Codex auth state is missing tokens. Run `sinoclaw auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        raise AuthError(
            "Codex auth is missing access_token. Run `sinoclaw auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "Codex auth is missing refresh_token. Run `sinoclaw auth` to re-authenticate.",
            provider="openai-codex",
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None) -> None:
    """Save Codex OAuth tokens to Sinoclaw auth store (~/.sinoclaw/auth.json)."""
...
def refresh_codex_oauth_pure(
    ...
    *,
    ...
    """Refresh Codex OAuth tokens without mutating Sinoclaw auth state."""
    ...
    if response.status_code != 200:
        ...
        if code == "refresh_token_reused":
            message = (
                ...
                "then run `sinoclaw auth` to re-authenticate."
            )
            relogin_required = True
...
def _refresh_codex_auth_tokens(
    ...
    Saves the new tokens to Sinoclaw auth store automatically.
    """
...
def resolve_codex_runtime_credentials(
    *,
    ...
    """Resolve runtime credentials from Sinoclaw's own Codex token store."""
    data = _read_codex_tokens()
    tokens = dict(data["tokens"])
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = float(os.getenv("SINOCLAW_CODEX_REFRESH_TIMEOUT_SECONDS", "20"))

    should_refresh = bool(force_refresh)
    if (not should_refresh) and refresh_if_expiring:
        should_refresh = _codex_access_token_is_expiring(access_token, refresh_skew_seconds)
    if should_refresh:
        # Re-read under lock to avoid racing with other Sinoclaw processes
...
def _resolve_verify(
    *,
    ...
    effective_ca = (
        ...
        or os.getenv("SINOCLAW_CA_BUNDLE")
...
# so a new `sinoclaw --profile <name> auth add nous --type oauth` can one-tap
...
# ``get_default_sinoclaw_root()`` returns — ``~/.sinoclaw`` on Linux/macOS,
# ``%LOCALAPPDATA%\hermes`` on native Windows, or the Docker/custom root.
# It is OUTSIDE any named profile's SINOCLAW_HOME so named profiles (which
...
def _nous_shared_auth_dir() -> Path:
    ...
    :func:`sinoclaw_constants.get_default_sinoclaw_root` returns — so
    Linux/macOS classic installs land at ``~/.sinoclaw/shared/``, native
    Windows installs at ``%LOCALAPPDATA%\\hermes\\shared\\``, and
    Docker / custom ``SINOCLAW_HOME`` deployments at
    ``<SINOCLAW_HOME>/shared/``. Sits outside any named profile so all
    profiles under the same root share the store.
    """
    override = os.getenv("SINOCLAW_SHARED_AUTH_DIR", "").strip()
    if override:
        return Path(override).expanduser()
    from sinoclaw_constants import get_default_sinoclaw_root
    return get_default_sinoclaw_root() / "shared"


def _nous_shared_store_path() -> Path:
    path = _nous_shared_auth_dir() / NOUS_SHARED_STORE_FILENAME
    # Seat belt: if pytest is running and this resolves to a path under the
    # real user's Sinoclaw root, refuse rather than silently corrupt cross-profile
    # state. Tests must set SINOCLAW_SHARED_AUTH_DIR to a tmp_path (conftest
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        from sinoclaw_constants import get_default_sinoclaw_root
        real_home_shared = (
            get_default_sinoclaw_root() / "shared" / NOUS_SHARED_STORE_FILENAME
        ...
        if resolved == real_home_shared:
            raise RuntimeError(
                ...
                f"{path}. Set SINOCLAW_SHARED_AUTH_DIR to a tmp_path in your test fixture."
            )
    return path
...
def _nous_shared_store_lock(timeout_seconds: float = AUTH_LOCK_TIMEOUT_SECONDS):
    ...
    try:
        ...
        # No SINOCLAW_HOME yet (pre-setup): fall through without locking.
        yield
        return
...
def _refresh_access_token(
    *,
    ...
    # called POST /api/oauth/token with Sinoclaw's refresh_token without
    # persisting the rotated token back to auth.json — the server then
    # retires the original RT, Sinoclaw's next refresh uses it, and the whole
    ...
    if "reuse" in lowered or "reuse detected" in lowered:
        description = (
            ...
            "custom self-heal hook, or another Sinoclaw install sharing "
            "~/.sinoclaw/auth.json) called POST /api/oauth/token with Sinoclaw's "
            "refresh token without persisting the rotated token back.\n"
            "Nous refresh tokens are single-use — only Sinoclaw may call the "
            "refresh endpoint. For health checks, use `sinoclaw auth status` "
            "instead.\n"
            "Re-authenticate with: sinoclaw auth add nous"
        )
    ...
    *,
    ...
    for item in data:
        ...
        if isinstance(model_id, str) and model_id.strip():
            mid = model_id.strip()
            # Skip Sinoclaw models — they're not reliable for agentic tool-calling
...
def resolve_nous_access_token(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "Sinoclaw is not logged into Nous Portal.",
                provider="nous",
                relogin_required=True,
            )
        ...
        portal_base_url = (
            ...
            or os.getenv("SINOCLAW_PORTAL_BASE_URL")
...
def refresh_nous_oauth_from_state(
    ...
    *,
    ...
    return refresh_nous_oauth_pure(
        ...
        state.get("client_id", "sinoclaw-cli"),
...
def persist_nous_credentials(
    ...
    *,
    ...
    Historically ``sinoclaw auth add nous`` wrote a ``manual:device_code`` pool
    ...
    ``sinoclaw auth add nous --label <name>``).  It gets embedded in the
    ...
    # these credentials via `sinoclaw auth add nous --type oauth`. Best-
...
def resolve_nous_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError("Sinoclaw is not logged into Nous Portal.",
...
def get_codex_auth_status() -> Dict[str, Any]:
    ...
    Checks the credential pool first (where `sinoclaw auth` stores credentials),
    then falls back to the legacy provider state.
    """
    # Check credential pool first — this is where `sinoclaw auth` and
    # `sinoclaw model` store device_code tokens.
    try:
        from agent.credential_pool import load_pool
...
def get_external_process_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    command = (
        os.getenv("SINOCLAW_COPILOT_ACP_COMMAND", "").strip()
        or os.getenv("COPILOT_CLI_PATH", "").strip()
        or "copilot"
    )
    raw_args = os.getenv("SINOCLAW_COPILOT_ACP_ARGS", "").strip()
...
def resolve_external_process_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    if not resolved_command and not base_url.startswith("acp+tcp://"):
        raise AuthError(
            ...
            "Install GitHub Copilot CLI or set SINOCLAW_COPILOT_ACP_COMMAND/COPILOT_CLI_PATH.",
            provider=provider_id,
...
def _logout_default_provider_from_config() -> Optional[str]:
    ...
    `sinoclaw logout` historically keyed off auth.json.active_provider only.
...
def _prompt_model_selection(
    ...
    """
    from sinoclaw_cli.models import _format_price_per_mtok
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from sinoclaw_cli.config import save_config, load_config
...
def login_command(args) -> None:
    """Deprecated: use 'sinoclaw model' or 'sinoclaw setup' instead."""
    print("The 'hermes login' command has been removed.")
    print("Use 'hermes auth' to manage credentials,")
    print("'sinoclaw model' to select a provider, or 'sinoclaw setup' for full setup.")
    raise SystemExit(0)
...
def _login_openai_codex(
    ...
    *,
    ...
    """OpenAI Codex login via device code flow. Tokens stored in ~/.sinoclaw/auth.json."""

    del args, pconfig  # kept for parity with other provider login helpers

    # Check for existing Sinoclaw-owned credentials
    if not force_new_login:
        try:
            ...
            if isinstance(_resolved_key, str) and _resolved_key and not _codex_access_token_is_expiring(_resolved_key, 60):
                print("Existing Codex credentials found in Sinoclaw auth store.")
                try:
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            ...
            print("Sinoclaw will create its own session to avoid conflicts with Codex CLI / VS Code.")
            try:
            ...
            if do_import in ("y", "yes"):
                ...
                base_url = os.getenv("SINOCLAW_CODEX_BASE_URL", "").strip().rstrip("/") or DEFAULT_CODEX_BASE_URL
                config_path = _update_config_for_provider("openai-codex", base_url)
                print()
                print("Credentials imported. Note: if Codex CLI refreshes its token,")
                print("Sinoclaw will keep working independently with its own session.")
                print(f"  Config updated: {config_path} (model.provider=openai-codex)")
                return

    # Run a fresh device code flow — Sinoclaw gets its own OAuth session
    print()
    print("Signing in to OpenAI Codex...")
    print("(Sinoclaw creates its own session — won't affect Codex CLI or VS Code)")
    print()

    creds = _codex_device_code_login()

    # Save tokens to Sinoclaw auth store
...
def _minimax_save_auth_state(auth_state: Dict[str, Any]) -> None:
    """Persist MiniMax OAuth state to Sinoclaw auth store (~/.sinoclaw/auth.json)."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
...
def _minimax_oauth_login(
    *, region: str = "global", open_browser: bool = True,
    ...
    print(f"Starting Sinoclaw login via MiniMax ({region}) OAuth...")
...
def resolve_minimax_oauth_runtime_credentials(
    *, min_token_ttl_seconds: int = MINIMAX_OAUTH_REFRESH_SKEW_SECONDS,
    ...
    if not state or not state.get("access_token"):
        raise AuthError(
            "Not logged into MiniMax OAuth. Run `sinoclaw model` and select "
            "MiniMax (OAuth).",
...
def _nous_device_code_login(
    *,
    ...
    print(f"Starting Sinoclaw login via {pconfig.name}...")
    print(f"Portal: {portal_base_url}")
    if insecure:
    ...
    try:
        ...
        if exc.code == "subscription_required":
            ...
            print()
            print("After subscribing, run `sinoclaw model` again to finish setup.")
            raise SystemExit(1)
        raise
...
def _login_nous(args, pconfig: ProviderConfig) -> None:
    ...
    try:
        ...
        if not selected_model:
            ...
            print("  Run `sinoclaw model` again to switch to Nous Portal.")
            return
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target) or should_reset_config:
        ...
        if should_reset_config and os.getenv("OPENROUTER_API_KEY"):
            print("Sinoclaw will use OpenRouter for inference.")
        elif should_reset_config:
            print("Run `sinoclaw model` or configure an API key to use Sinoclaw.")
        else:
