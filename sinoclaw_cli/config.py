"""
Configuration management for Sinoclaw Agent.

Config files are stored in ~/.sinoclaw/ for easy access:
- ~/.sinoclaw/config.yaml  - All settings (model, toolsets, terminal, etc.)
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    "SINOCLAW_LANGFUSE_SAMPLE_RATE",
    "SINOCLAW_LANGFUSE_MAX_CHARS",
    "SINOCLAW_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
from sinoclaw_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("SINOCLAW_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_sinoclaw_home() / ".managed"
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade sinoclaw-agent"
...
def get_container_exec_info() -> Optional[dict]:
    ...
    container, or SINOCLAW_DEV=1 is set.
...
# Re-export from sinoclaw_constants — canonical definition lives there.
from sinoclaw_constants import get_sinoclaw_home  # noqa: F811,E402
from utils import atomic_replace

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_sinoclaw_home() / "config.yaml"
...
def _secure_dir(path):
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("SINOCLAW_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    When Sinoclaw runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("SINOCLAW_CONTAINER") or os.environ.get("SINOCLAW_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set SINOCLAW_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def ensure_sinoclaw_home():
    """Ensure ~/.sinoclaw directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_sinoclaw_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_sinoclaw_home_managed(home: Path):
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Sinoclaw-level retry loop that wraps the whole call.  Lower
    ...
    "terminal": {
        ...
        # skipped. When empty, Sinoclaw auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Sinoclaw sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Sinoclaw
        ...
        # bundled Sinoclaw image, which drops to the `sinoclaw` user via gosu).
    ...
    "checkpoints": {
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.sinoclaw/checkpoints/`` size (MB).  When
    ...
    "tts": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.sinoclaw/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.sinoclaw/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # always goes to ~/.sinoclaw/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${SINOCLAW_SKILL_DIR} and ${SINOCLAW_SESSION_ID} in SKILL.md
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Sinoclaw Agent*" header.
    ...
    # stored approval from ~/.sinoclaw/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or SINOCLAW_ACCEPT_HOOKS=1.
    ...
    "cron": {
        ...
        # Also overridable via SINOCLAW_CRON_MAX_PARALLEL env var.
    ...
    "code_execution": {
        ...
        #     sinoclaw-agent's own python (sys.executable). Maximum isolation
    ...
    # Logging — controls file logging to ~/.sinoclaw/logs/.
    ...
    # update model picker lists without shipping a sinoclaw-agent release.
    ...
    # Session storage — controls automatic cleanup of ~/.sinoclaw/state.db.
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'sinoclaw model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Sinoclaw auth store)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. SINOCLAW_BOT)",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: sinoclaw-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'sinoclaw-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    # SINOCLAW_TOOL_PROGRESS and SINOCLAW_TOOL_PROGRESS_MODE are deprecated —
    # now configured via display.tool_progress in config.yaml (off|new|all|verbose).
    # Gateway falls back to these env vars for backward compatibility.
    "SINOCLAW_TOOL_PROGRESS": {
    ...
    "SINOCLAW_PREFILL_MESSAGES_FILE": {
    ...
    "SINOCLAW_EPHEMERAL_SYSTEM_PROMPT": {
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Sinoclaw versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``sinoclaw_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'sinoclaw setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Sinoclaw won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'sinoclaw doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("SINOCLAW_HOME", "~/.sinoclaw")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("SINOCLAW_TOOL_PROGRESS")
            old_mode = get_env_value("SINOCLAW_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in ("false", "0", "no"):
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from SINOCLAW_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in ("new", "all"):
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from SINOCLAW_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("SINOCLAW_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from SINOCLAW_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$SINOCLAW_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_sinoclaw_home() / "plugins"
    ...
    #   3. Creates `~/.sinoclaw/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_sinoclaw_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_sinoclaw_home() / "logs" / "curator"
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _sinoclaw_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.sinoclaw/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    ...
    (which change ``SINOCLAW_HOME`` and therefore ``get_config_path()``)
    don't collide.
    """
    with _CONFIG_LOCK:
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.sinoclaw/config.yaml."""
    with _CONFIG_LOCK:
        if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.sinoclaw/.env.
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Sinoclaw env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.sinoclaw/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.sinoclaw/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.sinoclaw/.env and os.environ.
...
def reload_env() -> int:
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Sinoclaw vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.sinoclaw/.env or environment."""
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if missing_config:
            ...
            print("    Run 'sinoclaw config migrate' to add them")
