...
TIPS = [
    ...
    "sinoclaw chat -t web,terminal enables only specific toolsets for a focused session.",
    "sinoclaw chat -s github-pr-workflow preloads a skill at launch.",
    "sinoclaw chat -q \"query\" runs a single non-interactive query and exits.",
    "sinoclaw chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "sinoclaw chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "hermes --yolo bypasses all dangerous command approval prompts for the entire session.",
    "sinoclaw chat --source telegram tags the session for filtering in sinoclaw sessions list.",
    "sinoclaw -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "sinoclaw doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "sinoclaw dump outputs a compact setup summary — great for bug reports.",
    "sinoclaw config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "sinoclaw config edit opens config.yaml in your default editor.",
    "sinoclaw config check scans for missing or stale configuration options.",
    "sinoclaw sessions browse opens an interactive session picker with search.",
    "sinoclaw sessions stats shows session counts by platform and database size.",
    "sinoclaw sessions prune --older-than 30 cleans up old sessions.",
    "sinoclaw skills search react --source skills-sh searches the skills.sh public directory.",
    "sinoclaw skills check scans installed hub skills for upstream updates.",
    ...
    "sinoclaw backup creates a zip backup of your entire Sinoclaw home directory.",
    ...
    "Profile names become shell commands — 'sinoclaw profile create coder' creates the 'coder' command.",
    "sinoclaw profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "sinoclaw skills install official/security/1password installs optional skills from the repo.",
    ...
    "Context files (.sinoclaw.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "SOUL.md at ~/.sinoclaw/SOUL.md is the agent's primary identity — customize it to shape behavior.",
    "Sinoclaw loads project context from .sinoclaw.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    "Checkpoints use shadow repos in ~/.sinoclaw/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "sinoclaw chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "sinoclaw plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "sinoclaw login supports OAuth-based auth for Nous and OpenAI Codex providers.",
    ...
    "sinoclaw status --deep runs deeper diagnostic checks across all components.",
    ...
    "The skills audit log at ~/.sinoclaw/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Each profile gets its own subprocess HOME at SINOCLAW_HOME/home/ — isolated git, ssh, npm, gh configs.",
    "SINOCLAW_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in SINOCLAW_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.sinoclaw/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'sinoclaw profile create ls' would be rejected.",
    "sinoclaw profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "sinoclaw claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    '/reload reloads ~/.sinoclaw/.env into the running session — pick up new API keys without restarting.',
    ...
    "SINOCLAW_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.sinoclaw/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.sinoclaw/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'sinoclaw curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "sinoclaw curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'sinoclaw curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'sinoclaw auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    # --- TUI & Dashboard ---
    'SINOCLAW_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "SINOCLAW_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'SINOCLAW_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'SINOCLAW_WRITE_SAFE_ROOT restricts write_file and patch to a directory prefix; writes outside require approval.',
    'SINOCLAW_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'SINOCLAW_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.sinoclaw/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.sinoclaw/skills/ so drop-in skills appear without restarting the session.',
    ...
    'sinoclaw chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'sinoclaw chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'sinoclaw chat --ignore-user-config skips ~/.sinoclaw/config.yaml — reproducible bug reports and CI runs.',
    "sinoclaw chat --source tool tags programmatic chats so they don't clutter sinoclaw sessions list.",
    'sinoclaw dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'sinoclaw sessions rename <ID> "new title" renames any past session; sinoclaw sessions delete <ID> removes one.',
    'sinoclaw import restores a session export or profile archive produced by sessions export or profile export.',
    'sinoclaw fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'sinoclaw pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'sinoclaw setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'sinoclaw status --deep runs the full health sweep across every component; plain sinoclaw status is the quick view.',
    ...
    'SINOCLAW_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.sinoclaw/plugins/ — trust-gated by design.',
    ...
    'SINOCLAW_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'SINOCLAW_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'SINOCLAW_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'SINOCLAW_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.sinoclaw/dashboard-plugins/.',
]
