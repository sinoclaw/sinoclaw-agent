...
def tail_log(
    ...
    *,
    ...
    if not log_path.exists():
        ...
        print(f"(Logs are created when Sinoclaw runs — try 'sinoclaw chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_sinoclaw_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_sinoclaw_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_sinoclaw_home()}/logs/")
        return

    print(f"Log files in {display_sinoclaw_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'sinoclaw chat' to generate logs)")
