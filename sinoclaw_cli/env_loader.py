"""Helpers for loading Sinoclaw .env files consistently across entrypoints."""
...
# load_sinoclaw_dotenv() calls (user env + project env, gateway hot-reload,
...
def _sanitize_loaded_credentials() -> None:
    ...
    for key, value in list(os.environ.items()):
        ...
        print(
            ...
            "  provider's dashboard and run `sinoclaw setup` (or edit the "
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    We delegate to ``sinoclaw_cli.config._sanitize_env_lines`` which
    already knows all valid Sinoclaw env-var names and can split
    concatenated lines correctly.
    """
    if not path.exists():
        return
    try:
        from sinoclaw_cli.config import _sanitize_env_lines
    except ImportError:
