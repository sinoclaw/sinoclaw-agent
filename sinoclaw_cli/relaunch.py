"""
Unified self-relaunch for Sinoclaw CLI.

Preserves critical flags (--tui, --dev, --profile, --model, etc.) across
process replacement so that ``sinoclaw sessions browse`` or post-setup relaunch
...
from sinoclaw_cli._parser import (
...
def resolve_sinoclaw_bin() -> Optional[str]:
    """Find the sinoclaw entry point.

    Priority:
      1. ``sys.argv[0]`` if it resolves to a real executable.
      2. ``shutil.which("hermes")`` on PATH.
      3. ``None`` → caller should fall back to ``python -m sinoclaw_cli.main``.
    ...
    ``hermes.exe`` on PATH or the ``sys.executable -m sinoclaw_cli.main``
    fallback.
    """
    argv0 = sys.argv[0]
    ...
    *,
    ...
    Args:
        ...
        original_argv: The original argv to scan for flags (defaults to
            ``sys.argv[1:]``).
    """
    bin_path = resolve_sinoclaw_bin()

    if bin_path:
        argv = [bin_path]
    else:
        argv = [sys.executable, "-m", "sinoclaw_cli.main"]
...
def relaunch(
    ...
    *,
    ...
    """Replace the current process with a fresh sinoclaw invocation.
    ...
    relaunch contract wants: "run sinoclaw again as if the user had typed
    the new argv".
    ...
    *emulates* exec by spawning the child and exiting the parent, but
    ...
    ``python -m sinoclaw_cli.main``) or a ``.cmd`` batch file, and both
    ...
    new sinoclaw started" — just with two PIDs in play instead of one.
    """
