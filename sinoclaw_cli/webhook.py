"""sinoclaw webhook — manage dynamic webhook subscriptions from the CLI.
...
def _subscriptions_path() -> Path:
    return _sinoclaw_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: sinoclaw gateway run
"""
...
def webhook_command(args):
    """Entry point for 'sinoclaw webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: sinoclaw webhook {subscribe|list|remove|test}")
        print("Run 'sinoclaw webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (sinoclaw gateway run).\n")
...
def _cmd_test(args):
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (sinoclaw gateway run)")
