...
def auth_add_command(args) -> None:
    ...
    if provider == "anthropic":
        ...
        creds = anthropic_mod.run_sinoclaw_oauth_login_pure()
        if not creds:
        ...
        entry = PooledCredential(
            ...
            priority=0,
            source=f"{SOURCE_MANUAL}:sinoclaw_pkce",
...
def auth_remove_command(args) -> None:
    ...
    # If this was a singleton-seeded credential (OAuth device_code, sinoclaw_pkce),
    ...
    # re-seeded on the next load_pool() call.
    ...
    elif removed.source == "sinoclaw_pkce" and provider == "anthropic":
        from sinoclaw_constants import get_sinoclaw_home
        oauth_file = get_sinoclaw_home() / ".anthropic_oauth.json"
        if oauth_file.exists():
            oauth_file.unlink()
            print("Cleared Sinoclaw Anthropic OAuth credentials")
        ...
        print("Run `sinoclaw auth add anthropic` to re-enable if needed.")
