...
* We write two files: a shared ``gateway.cmd`` wrapper script (cwd + env + the
  actual ``python -m sinoclaw_cli.main gateway run --replace`` invocation) and
...
def get_task_name() -> str:
    ...
    from sinoclaw_cli.gateway import _profile_suffix
...
def get_task_script_path() -> Path:
    ...
    script_dir = Path(get_sinoclaw_home()) / "gateway-service"
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - exports SINOCLAW_HOME, PYTHONIOENCODING, VIRTUAL_ENV
      - invokes ``python -m sinoclaw_cli.main [--profile X] gateway run --replace``
    ...
    lines.append(f'set "SINOCLAW_HOME={sinoclaw_home}"')
    lines.append('set "PYTHONIOENCODING=utf-8"')
    # VIRTUAL_ENV lets the gateway's own python detection find the venv
    # if someone imports sinoclaw_constants-based logic during startup.
    venv_dir = str(Path(python_path).resolve().parent.parent)
    lines.append(f'set "VIRTUAL_ENV={venv_dir}"')

    prog_args = [python_path, "-m", "sinoclaw_cli.main"]
    if profile_arg:
...
def _write_task_script() -> Path:
    ...
    sinoclaw_home = str(Path(get_sinoclaw_home()).resolve())
    profile_arg = _profile_arg(sinoclaw_home)

    content = _build_gateway_cmd_script(python_path, working_dir, sinoclaw_home, profile_arg)
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    argv = [python_exe, "-m", "sinoclaw_cli.main"]
    if profile_arg:
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``pythonw.exe -m sinoclaw_cli.main gateway run --replace``
    ...
    log_dir = Path(get_sinoclaw_home()) / "logs"
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        from sinoclaw_cli.config import get_sinoclaw_home
        print(f"    type {Path(get_sinoclaw_home()).resolve()}\\logs\\gateway.log")
        print(f"    type {Path(get_sinoclaw_home()).resolve()}\\logs\\gateway-stdio.log")


def _print_next_steps() -> None:
    from sinoclaw_cli.config import get_sinoclaw_home

    sinoclaw_home = Path(get_sinoclaw_home()).resolve()
    print()
    print("Next steps:")
    print("  sinoclaw gateway status                      # Check status")
    print(f"  type {sinoclaw_home}\\logs\\gateway.log       # View logs")
...
def stop() -> None:
    ...
    _assert_windows()
    from sinoclaw_cli.gateway import kill_gateway_processes
