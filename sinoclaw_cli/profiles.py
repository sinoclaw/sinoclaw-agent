...
The "default" profile is ``~/.sinoclaw`` itself — backward compatible,
...
_PROFILE_DIRS = [
    ...
    # See sinoclaw_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
# default profile (``~/.sinoclaw``).  Named profiles never contain these
# directories at root, so the exclusion is gated to avoid silently dropping
# user data from a named-profile source.
#
# Rationale per item:
#   sinoclaw-agent  — git repo checkout (~84 MB source + ~3 GB venv)
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    is_default_source = source_resolved == _get_default_sinoclaw_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.sinoclaw) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "sinoclaw-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_sinoclaw_home() / "active_profile"
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_sinoclaw_home()
...
def create_profile(
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.sinoclaw)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.sinoclaw/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "SINOCLAW_HOME": str(profile_dir)},
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    try:
        ...
        from sinoclaw_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["SINOCLAW_HOME"] = old_home
        elif "SINOCLAW_HOME" in os.environ:
            del os.environ["SINOCLAW_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.sinoclaw/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
...
def get_active_profile_name() -> str:
    ...
    resolved = sinoclaw_home.resolve()

    default_resolved = _get_default_sinoclaw_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.sinoclaw itself — its parent is ~/ and its
        # directory name is ".sinoclaw", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    # dir.  Importing as "default" would target ~/.sinoclaw itself — disallow
    ...
    with tempfile.TemporaryDirectory(prefix="sinoclaw_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    candidates = [
        new_dir / "honcho.json",
        _get_default_sinoclaw_home() / "honcho.json",
...
_sinoclaw_profiles() {
    local profiles_dir="$HOME/.sinoclaw/profiles"
...
_sinoclaw_completion() {
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_sinoclaw_profiles)" -- "$cur"))
        return
    fi
...
complete -F _sinoclaw_completion hermes
'''
...
_hermes() {
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.sinoclaw/profiles" ]]; then
        profiles+=("${(@f)$(ls $HOME/.sinoclaw/profiles 2>/dev/null)}")
    fi
