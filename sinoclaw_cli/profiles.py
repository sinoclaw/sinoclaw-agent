...
Usage::
    ...
    sinoclaw profile create coder --clone  # also copy config, .env, SOUL.md
...
_RESERVED_NAMES = frozenset({
    "sinoclaw", "default", "test", "tmp", "root", "sudo",
})
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    if name in _SINOCLAW_SUBCOMMANDS:
        return f"'{name}' conflicts with a sinoclaw subcommand"
...
def set_active_profile(name: str) -> None:
    ...
    if name != "default" and not profile_exists(name):
        raise FileNotFoundError(
            ...
            f"Create it with: sinoclaw profile create {name}"
        )
...
# Add to ~/.bashrc: eval "$(sinoclaw completion bash)"
...
complete -F _sinoclaw_completion sinoclaw
'''


def generate_zsh_completion() -> str:
    """Generate a zsh completion script for sinoclaw profile names."""
    return '''#compdef sinoclaw
# Sinoclaw Agent profile completion
# Add to ~/.zshrc: eval "$(sinoclaw completion zsh)"
...
def resolve_profile_env(profile_name: str) -> str:
    ...
    if profile_name != "default" and not profile_dir.is_dir():
        raise FileNotFoundError(
            ...
            f"Create it with: sinoclaw profile create {profile_name}"
        )
