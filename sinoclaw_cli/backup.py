...
from sinoclaw_constants import get_default_sinoclaw_root, get_sinoclaw_home, display_sinoclaw_home
...
_EXCLUDED_DIRS = {
    "sinoclaw-agent",     # the codebase repo — re-clone instead
...
def run_backup(args) -> None:
    """Create a zip backup of the Sinoclaw home directory."""
    sinoclaw_root = get_default_sinoclaw_root()

    if not sinoclaw_root.is_dir():
        print(f"Error: Sinoclaw home directory not found at {sinoclaw_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"sinoclaw-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"sinoclaw-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_sinoclaw_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(sinoclaw_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(sinoclaw_root)
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(sinoclaw_root)
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_sinoclaw_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (sinoclaw_root / "config.yaml").exists()
        has_env = (sinoclaw_root / ".env").exists()
        ...
        sinoclaw_root.mkdir(parents=True, exist_ok=True)
        ...
        for member in members:
            ...
            target = sinoclaw_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(sinoclaw_root.resolve())
            except ValueError:
        ...
        profiles_dir = sinoclaw_root / "profiles"
        restored_profiles = []
...
def _quick_snapshot_root(sinoclaw_home: Optional[Path] = None) -> Path:
    home = sinoclaw_home or get_sinoclaw_home()
    return home / _QUICK_SNAPSHOTS_DIR


def create_quick_snapshot(
    label: Optional[str] = None,
    sinoclaw_home: Optional[Path] = None,
) -> Optional[str]:
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(sinoclaw_home)
    if not root.exists():
        return []
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(sinoclaw_home), keep=keep)
...
def run_quick_backup(args) -> None:
    ...
    if snap_id:
        ...
        print(f"  {len(snaps)} snapshot(s) stored in {display_sinoclaw_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, sinoclaw_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``sinoclaw_root`` to ``out_path``.
...
def _pre_update_backup_dir(sinoclaw_home: Optional[Path] = None) -> Path:
    home = sinoclaw_home or get_sinoclaw_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of SINOCLAW_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<SINOCLAW_HOME>/backups/pre-update-<timestamp>.zip`` and
    ...
    result = _write_full_zip_backup(out_path, sinoclaw_root)
    if result is None:
        return None
