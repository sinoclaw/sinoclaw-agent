...
``<root>`` is the **shared Sinoclaw root** (the parent of any active
...
``sinoclaw -p <profile>`` joins the same board as the dispatcher that
...
the profile's Sinoclaw home but are otherwise isolated: a worker spawned
...
* ``board=`` argument passed directly to :func:`connect` / :func:`init_db`
  ...
  ``?board=...`` query param).
* ``SINOCLAW_KANBAN_BOARD`` env var (used by the dispatcher to pin workers
  to the board their task lives on — workers cannot see other boards).
* ``SINOCLAW_KANBAN_DB`` env var (pins the DB file path directly — legacy
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``sinoclaw kanban boards
  switch <slug>``. When absent, the active board is ``default``.

In standard installs ``<root>`` is ``~/.sinoclaw``. In Docker / custom
deployments where ``SINOCLAW_HOME`` points outside ``~/.sinoclaw`` (e.g.
``/opt/hermes``), ``<root>`` is ``SINOCLAW_HOME``. Legacy env-var
overrides still work:

* ``SINOCLAW_KANBAN_DB`` — pin the database file path directly.
* ``SINOCLAW_KANBAN_WORKSPACES_ROOT`` — pin the workspaces root directly.
* ``SINOCLAW_KANBAN_HOME`` — pin the umbrella root that anchors kanban
  paths. Useful for tests and unusual deployments.

The dispatcher injects ``SINOCLAW_KANBAN_DB``,
``SINOCLAW_KANBAN_WORKSPACES_ROOT``, and ``SINOCLAW_KANBAN_BOARD`` into
...
# enough that kebab-case names like ``atm10-server`` or ``sinoclaw-agent``
...
def kanban_home() -> Path:
    """Return the shared Sinoclaw root that anchors the kanban board.

    Resolution order:

    1. ``SINOCLAW_KANBAN_HOME`` env var when set and non-empty (explicit
       override for tests and unusual deployments).
    2. ``get_default_sinoclaw_root()``, which already returns ``<root>``
       when ``SINOCLAW_HOME`` is ``<root>/profiles/<name>``, and returns
       ``SINOCLAW_HOME`` directly for Docker / custom deployments.

    The kanban board is shared across profiles **by design** (see the
    module docstring). Resolving the kanban paths through the active
    profile's ``SINOCLAW_HOME`` would silently fork the board per profile,
    which breaks the dispatcher / worker handoff.
    """
    override = os.environ.get("SINOCLAW_KANBAN_HOME", "").strip()
    if override:
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``sinoclaw kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    1. ``SINOCLAW_KANBAN_BOARD`` env var (set by the dispatcher on worker
       spawn, or manually for ad-hoc overrides).
    2. ``<root>/kanban/current`` on disk (set by ``sinoclaw kanban boards
    ...
    """
    env = os.environ.get("SINOCLAW_KANBAN_BOARD", "").strip()
    if env:
        try:
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``sinoclaw kanban boards switch <typo>`` returns an error
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    ...
    1. ``SINOCLAW_KANBAN_DB`` env var — pins the path directly. Honoured for
    ...
    """
    override = os.environ.get("SINOCLAW_KANBAN_DB", "").strip()
    if override:
        return Path(override).expanduser()
...
def workspaces_root(board: Optional[str] = None) -> Path:
    ...
    ``SINOCLAW_KANBAN_WORKSPACES_ROOT`` pins the path directly (highest
    ...
    """
    override = os.environ.get("SINOCLAW_KANBAN_WORKSPACES_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``sinoclaw kanban log`` unambiguous even when multiple
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
      ``SINOCLAW_KANBAN_DB`` env → ``SINOCLAW_KANBAN_BOARD`` env →
    ...
    # See sinoclaw_state._WAL_INCOMPAT_MARKERS for detection logic.
    from sinoclaw_state import apply_wal_with_fallback
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``sinoclaw kanban init`` and the
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from sinoclaw_cli.profiles import normalize_profile_name
...
def create_task(
    ...
    *,
    ...
    each name to ``sinoclaw --skills ...`` alongside the built-in
    ...
    # `sinoclaw --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``sinoclaw kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``sinoclaw kanban complete <ready-task>
...
def complete_task(
    ...
    *,
    ...
    completion (``sinoclaw kanban complete <id>``) works without requiring
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``sinoclaw kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    if not rows:
        return False
    try:
        from sinoclaw_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``sinoclaw -p <profile> chat -q ...`` subprocess.
    ...
    ``SINOCLAW_KANBAN_DB`` / ``SINOCLAW_KANBAN_BOARD`` / workspaces_root env
    ...
    env = dict(os.environ)
    if task.tenant:
        env["SINOCLAW_TENANT"] = task.tenant
    env["SINOCLAW_KANBAN_TASK"] = task.id
    env["SINOCLAW_KANBAN_WORKSPACE"] = workspace
    if task.current_run_id is not None:
        env["SINOCLAW_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["SINOCLAW_KANBAN_CLAIM_LOCK"] = task.claim_lock
    # Pin the shared board + workspaces root the dispatcher resolved, so
    # that even when the worker activates a profile (`sinoclaw -p <name>`
    # rewrites SINOCLAW_HOME), its kanban paths still match the
    # dispatcher's. Belt-and-braces with the `get_default_sinoclaw_root()`
    # resolution in `kanban_home()` — symmetric resolution is the norm,
    # but unusual symlink / Docker layouts are caught here too.
    env["SINOCLAW_KANBAN_DB"] = str(kanban_db_path(board=board))
    env["SINOCLAW_KANBAN_WORKSPACES_ROOT"] = str(workspaces_root(board=board))
    # Board slug — the final defense-in-depth pin. If the worker ever
    # resolves kanban paths without the DB / workspaces env vars, the
    # board slug still forces it to the right directory.
    resolved_board = _normalize_board_slug(board) or get_current_board()
    env["SINOCLAW_KANBAN_BOARD"] = resolved_board
    # SINOCLAW_PROFILE is the author the kanban_comment tool defaults to.
    # `sinoclaw -p <assignee>` activates the profile, but the env var is
    # what the tool reads — set it explicitly here so comments are
    # attributed correctly regardless of how the child loads config.
    env["SINOCLAW_PROFILE"] = profile_arg
    ...
    # `sinoclaw kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Install Sinoclaw Agent or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``sinoclaw kanban daemon`` is systemd-friendly.
...
def list_profiles_on_disk() -> list[str]:
    ...
    try:
        from sinoclaw_constants import get_default_sinoclaw_root
        default_root = get_default_sinoclaw_root()
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``sinoclaw kanban assignees`` for the terminal.
