...
goal satisfied by the assistant's last response?". If not, Sinoclaw feeds a
...
- When a real user message arrives mid-loop it preempts the continuation
  ...
  will say ``done``).
- This module has zero hard dependency on ``cli.SinoclawCLI`` or the gateway
...
def _get_session_db() -> Optional[Any]:
    """Return a SessionDB instance for the current SINOCLAW_HOME.

    SessionDB has no built-in singleton, but opening a new connection per
    /goal call would thrash the file. We cache one instance per
    ``sinoclaw_home`` path so profile switches still pick up the right DB.
