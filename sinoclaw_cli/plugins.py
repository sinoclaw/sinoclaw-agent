...
1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with sinoclaw-agent;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.sinoclaw/plugins/<name>/``
3. **Project plugins** – ``./.sinoclaw/plugins/<name>/`` (opt-in via
   ``SINOCLAW_ENABLE_PROJECT_PLUGINS``)
4. **Pip plugins**     – packages that expose the ``sinoclaw_agent.plugins``
   entry-point group.
...
ENTRY_POINTS_GROUP = "sinoclaw_agent.plugins"

_NS_PARENT = "sinoclaw_plugins"
...
class PluginManifest:
    ...
    #              in ~/.sinoclaw/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``sinoclaw plugins list``. For a flat plugin at
...
class PluginContext:
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``sinoclaw honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``sinoclaw <subcommand>``
    ...
    def register_skill(
        ...
        ``~/.sinoclaw/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        #
        # Repo-shipped plugins live next to sinoclaw_cli/. Two layouts are
        ...
        # 2. User plugins (~/.sinoclaw/plugins/)
        user_dir = get_sinoclaw_home() / "plugins"
        ...
        for manifest in winners.values():
            ...
            # Built-in backends auto-load — they ship with sinoclaw and must
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `sinoclaw plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``sinoclaw_plugins.<slug>``.

        The module slug is derived from ``manifest.key`` so category-namespaced
        plugins (``image_gen/openai``) import as
        ``sinoclaw_plugins.image_gen__openai`` without colliding with any
        future ``tts/openai``.
        """
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="sinoclaw-plugin-command-await",
        daemon=True,
    )
    thread.start()
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``sinoclaw tools`` TUI so plugin-provided toolsets appear
