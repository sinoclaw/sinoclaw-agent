...
- Keep data: Remove code but keep ~/.sinoclaw/ (configs, sessions, logs)
"""
...
def remove_wrapper_script():
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains sinoclaw_cli reference)
                content = wrapper.read_text()
                if 'sinoclaw_cli' in content or 'sinoclaw-agent' in content:
                    wrapper.unlink()
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``sinoclaw gateway run`` processes
    ...
    try:
        from sinoclaw_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from sinoclaw_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``SINOCLAW_HOME`` and ``SINOCLAW_GIT_BASH_PATH``
...
def _sinoclaw_path_markers(sinoclaw_home: Path) -> list[str]:
    ...
    markers = [root + "\\sinoclaw-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if SINOCLAW_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "hermes".  We don't want to catch
    # unrelated entries like "csinoclaw-foo" or "ephermeral", so we look for
...
def remove_path_from_windows_registry(sinoclaw_home: Path) -> list[str]:
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _sinoclaw_path_markers(sinoclaw_home)
            kept: list[str] = []
...
def remove_sinoclaw_env_vars_windows() -> list[str]:
    """Delete SINOCLAW_HOME and SINOCLAW_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("SINOCLAW_HOME", "SINOCLAW_GIT_BASH_PATH"):
                try:
...
def remove_portable_tooling_windows(sinoclaw_home: Path) -> list[Path]:
...
def _is_default_sinoclaw_home(sinoclaw_home: Path) -> bool:
    """Return True when ``sinoclaw_home`` points at the default (non-profile) root."""
    try:
        from sinoclaw_constants import get_default_sinoclaw_root
        return sinoclaw_home.resolve() == get_default_sinoclaw_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    #    Use `python -m sinoclaw_cli.main` so we don't depend on a `hermes`
    #    wrapper that may be half-removed mid-uninstall.
    sinoclaw_invocation = [_sys.executable, "-m", "sinoclaw_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                sinoclaw_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's SINOCLAW_HOME directory.
    try:
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.sinoclaw/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.sinoclaw/ for future reinstall
    """
    project_root = get_project_root()
    sinoclaw_home = get_sinoclaw_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie SINOCLAW_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_sinoclaw_home(sinoclaw_home)
    ...
    print(f"  Config:  {sinoclaw_home / 'config.yaml'}")
    print(f"  Secrets: {sinoclaw_home / '.env'}")
    print(f"  Data:    {sinoclaw_home / 'cron/'}, {sinoclaw_home / 'sessions/'}, {sinoclaw_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their SINOCLAW_HOME dirs. Otherwise
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in sinoclaw_home so the marker matching is
        ...
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(sinoclaw_home))))
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.sinoclaw/, just remove the sinoclaw-agent subdir
            if sinoclaw_home in project_root.parents or project_root.parent == sinoclaw_home:
    ...
    #     under SINOCLAW_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(sinoclaw_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(sinoclaw_home)
    ...
    # 5. Optionally remove ~/.sinoclaw/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile SINOCLAW_HOME dirs live under
        ...
        try:
            if sinoclaw_home.exists():
                shutil.rmtree(sinoclaw_home)
                log_success(f"Removed {sinoclaw_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {sinoclaw_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {sinoclaw_home}")
    ...
    if not full_uninstall:
        ...
        print(f"  {sinoclaw_home}/")
        print()
        print("To reinstall later with your existing settings:")
        if _is_windows():
            print(color("  irm https://raw.githubusercontent.com/NousResearch/sinoclaw-agent/main/scripts/install.ps1 | iex", Colors.DIM))
        else:
            print(color("  curl -fsSL https://raw.githubusercontent.com/NousResearch/sinoclaw-agent/main/scripts/install.sh | bash", Colors.DIM))
        print()
