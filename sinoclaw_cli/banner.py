...
Pure display functions with no SinoclawCLI state dependency.
"""
...
from sinoclaw_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE
...
# (e.g. nix-built sinoclaw — no local git history to count against).
UPDATE_AVAILABLE_NO_COUNT = -1

_UPSTREAM_REPO_URL = "https://github.com/sinoclaw/sinoclaw-agent.git"
...
def check_for_updates() -> Optional[int]:
    ...
    cache_file = sinoclaw_home / ".update_check"
    ...
    if embedded_rev:
        ...
        # $SINOCLAW_HOME/sinoclaw-agent/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = sinoclaw_home / "sinoclaw-agent"
        if not (repo_dir / ".git").exists():
            return None
...
_RELEASE_URL_BASE = "https://github.com/sinoclaw/sinoclaw-agent/releases/tag"
...
def format_banner_version_label() -> str:
    ...
    base = f"Sinoclaw Agent v{VERSION} ({RELEASE_DATE})"
...
def build_welcome_banner(console: Console, model: str, cwd: str,
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else SINOCLAW_CADUCEUS
    except Exception:
        _bskin = None
        _hero = SINOCLAW_CADUCEUS
    left_lines = ["", _hero, ""]
    ...
    left_lines.append(f"[{accent}]{model_short}[/]{ctx_str} [dim {dim}]·[/] [dim {dim}]Sinoclaw Team[/]")
    ...
    try:
        ...
        if behind is not None and behind != 0:
            from sinoclaw_cli.config import get_managed_update_command, recommended_update_command
            if behind > 0:
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else SINOCLAW_AGENT_LOGO
        console.print(_logo)
        console.print()
