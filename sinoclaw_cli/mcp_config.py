...
configuration in ~/.sinoclaw/config.yaml under the ``mcp_servers`` key.
"""
...
from sinoclaw_cli.config import (
    ...
    get_env_value,
    save_env_value,
    get_sinoclaw_home,  # noqa: F401 — used by test mocks
)
...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    from sinoclaw_cli.cli_output import prompt as _shared_prompt
...
def cmd_mcp_add(args):
            ...
            if auth_type == "header" or not auth_type:
                ...
                if existing_key:
                    ...
                    if api_key:
                        ...
                        _success(f"Saved to {display_sinoclaw_home()}/.env as {env_key}")
    ...
    print()
    _success(f"Saved '{name}' to {display_sinoclaw_home()}/config.yaml ({tool_count}/{total} tools enabled)")
...
def cmd_mcp_configure(args):
    ...
    if not _sys.stdin.isatty():
        print("Error: 'sinoclaw mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
...
def mcp_command(args):
    ...
    if handler:
        ...
        _info("sinoclaw mcp serve                              Run as MCP server")
        _info("sinoclaw mcp add <name> --url <endpoint>        Add an MCP server")
        _info("sinoclaw mcp add <name> --command <cmd>         Add a stdio server")
        _info("sinoclaw mcp add <name> --preset <preset>       Add from a known preset")
        _info("sinoclaw mcp remove <name>                      Remove a server")
        _info("sinoclaw mcp list                               List servers")
        _info("sinoclaw mcp test <name>                        Test connection")
        _info("sinoclaw mcp configure <name>                   Toggle tools")
        print()
