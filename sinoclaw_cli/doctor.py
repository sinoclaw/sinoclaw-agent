"""
Doctor command for sinoclaw CLI.

Diagnoses issues with Sinoclaw Agent setup.
"""
...
from sinoclaw_cli.config import get_project_root, get_sinoclaw_home, get_env_path
...
load_sinoclaw_dotenv(sinoclaw_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")

from sinoclaw_cli.colors import Colors, color
from sinoclaw_cli.models import _SINOCLAW_USER_AGENT
from sinoclaw_cli.vercel_auth import describe_vercel_auth
from sinoclaw_constants import OPENROUTER_MODELS_URL
...
from sinoclaw_constants import is_termux as _is_termux
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.sinoclaw/.env contains provider auth/base URL settings."""
...
def _is_kanban_worker_env_gate(item: dict) -> bool:
    ...
    if item.get("name") != "kanban":
        return False
    if os.environ.get("SINOCLAW_KANBAN_TASK"):
        return False
...
def _doctor_tool_availability_detail(toolset: str) -> str:
    ...
    if toolset == "kanban" and not os.environ.get("SINOCLAW_KANBAN_TASK"):
...
def run_doctor(args):
    ...
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'sinoclaw setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'sinoclaw setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'sinoclaw setup' to create one")
    ...
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from sinoclaw_cli.config import get_compatible_custom_providers as _compatible_custom_providers
                from sinoclaw_cli.providers import (
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom", "openrouter"):
                try:
                    from sinoclaw_cli.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if pconfig and getattr(pconfig, "auth_type", "") == "api_key":
                        ...
                        if not configured:
                            check_fail(
                                ...
                                "(check ~/.sinoclaw/.env or run 'sinoclaw setup')",
                            )
                            issues.append(
    ...
    for subdir_name in expected_subdirs:
        subdir_path = sinoclaw_home / subdir_name
    ...
    soul_path = sinoclaw_home / "SOUL.md"
    if soul_path.exists():
    ...
    memories_dir = sinoclaw_home / "memories"
    ...
    state_db_path = sinoclaw_home / "state.db"
    ...
    wal_path = sinoclaw_home / "state.db-wal"
    if wal_path.exists():
        try:
    ...
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "sinoclaw"
        ...
        _cmd_link = _cmd_link_dir / "sinoclaw"

        if _venv_bin is None:
            check_warn(
                "Venv entry point not found",
                "(sinoclaw not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/sinoclaw → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/sinoclaw points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/sinoclaw → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/sinoclaw — run 'sinoclaw doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/sinoclaw exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/sinoclaw not found",
                    "(hermes command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/sinoclaw → {_venv_bin}")
                    fixed_count += 1
    ...
    if terminal_env == "vercel_sandbox":
        ...
        if importlib.util.find_spec("vercel") is not None:
            ...
            check_fail("vercel SDK not installed", "(pip install 'sinoclaw-agent[vercel]')")
            issues.append("Install the Vercel optional dependency: pip install 'sinoclaw-agent[vercel]'")
    ...
    if _safe_which("node"):
        ...
        if agent_browser_ok and not _is_termux():
            try:
                ...
                # to eagerly load in every `sinoclaw doctor` invocation.
        ...
        if _key:
            ...
            try:
                ...
                _headers = {
                    ...
                    "User-Agent": _SINOCLAW_USER_AGENT,
                }
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'sinoclaw setup' to configure missing API keys for full tool access")
    except Exception as e:
    ...
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: sinoclaw skills list)")
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                check_warn("Honcho config not found", "run: sinoclaw memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                check_fail("Honcho API key or base URL not set", "run: sinoclaw memory setup")
        ...
        try:
            ...
            if mem0_key:
                ...
                check_fail("Mem0 API key not set", "(set MEM0_API_KEY in .env or run sinoclaw memory setup)")
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: sinoclaw memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: sinoclaw memory setup")
        except Exception as _e:
    ...
    # =========================================================================
    try:
        from sinoclaw_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
                ...
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "sinoclaw -p" in content:
                            _m = _re.search(r"sinoclaw -p (\S+)", content)
