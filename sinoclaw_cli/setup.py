...
from sinoclaw_cli.nous_subscription import get_nous_subscription_features
...
def _print_setup_summary(config: dict, sinoclaw_home):
    ...
    if subscription_features.image_gen.managed_by_nous:
        ...
        try:
            ...
            from sinoclaw_cli.plugins import _ensure_plugins_discovered
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'sinoclaw setup tts'"))
    else:
...
def setup_terminal_backend(config: dict):
    ...
    if selected_backend == "local":
        ...
        print_info("Requires the optional SDK: pip install 'sinoclaw-agent[vercel]'")
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                print_warning("Install failed — run manually: pip install 'sinoclaw-agent[vercel]'")
                if result.stderr:
                    print_info(f"  Error: {result.stderr.strip().splitlines()[-1]}")
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into SINOCLAW_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("SINOCLAW_MAX_ITERATIONS")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # SINOCLAW_MAX_ITERATIONS from agent.max_turns at startup.
...
def _write_slack_manifest_and_instruct():
    """Generate the Slack manifest, write it under SINOCLAW_HOME, and print
    ...
    """
    try:
        from sinoclaw_cli.slack_cli import _build_full_manifest
...
def _setup_qqbot():
    ...
    from sinoclaw_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def setup_gateway(config: dict):
    ...
    if any_messaging:
        ...
        if supports_systemd and has_legacy_sinoclaw_units():
            print_legacy_unit_warning()
            print()
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``sinoclaw_cli.auth`` — lists every supported
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    if section_key == "model":
        ...
        from sinoclaw_cli.gateway import _all_platforms, _platform_status
...
def run_setup_wizard(args):
    ...
    if is_existing:
        ...
        print_info("")
        print_info("Tip: jump straight to a section with 'sinoclaw setup model|terminal|")
...
def _run_first_time_quick_setup(config: dict, sinoclaw_home, is_existing: bool):
    ...
    the user can customize later via ``sinoclaw setup <section>``.
    """
