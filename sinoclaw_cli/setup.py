...
_DOCS_BASE = "https://sinoclaw-agent.nousresearch.com/docs"
...
from sinoclaw_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Sinoclaw Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from sinoclaw_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'sinoclaw setup')")


def _print_setup_summary(config: dict, sinoclaw_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'sinoclaw setup' to configure"))
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'sinoclaw setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'sinoclaw setup terminal'"))
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'sinoclaw setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {sinoclaw_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('sinoclaw setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('sinoclaw setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('sinoclaw setup terminal', Colors.GREEN)} Change terminal backend")
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            from sinoclaw_cli.auth_commands import auth_add_command
    ...
    if _vision_needs_setup:
        ...
        if _vision_idx == 0:  # OpenRouter
            ...
            print_info("Skipped — add later with 'sinoclaw setup' or configure AUXILIARY_VISION_* settings")
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.sinoclaw/.env."
            )
        ...
        if not existing:
            ...
            if api_key:
                ...
                print_warning(
                    ...
                    f"sinoclaw setup model or {_dhh()}/.env to use xAI TTS. "
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'sinoclaw setup tts')."""
    _setup_tts_provider(config)
...
def setup_terminal_backend(config: dict):
    ...
    if selected_backend == "local":
        ...
        print_info("  When using Sinoclaw via Telegram/Discord, this is where")
        print_info(
...
def _apply_default_agent_settings(config: dict):
    ...
    save_env_value("SINOCLAW_MAX_ITERATIONS", "90")
    ...
    print_info("  Run `sinoclaw setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    # ── Max Iterations ──
    current_max = get_env_value("SINOCLAW_MAX_ITERATIONS") or str(
    ...
    try:
        ...
        if max_iter > 0:
            save_env_value("SINOCLAW_MAX_ITERATIONS", str(max_iter))
...
def _setup_slack():
    ...
    print()
    print_info("   Full guide: https://sinoclaw-agent.nousresearch.com/docs/user-guide/messaging/slack/")
    print()
...
def _setup_whatsapp():
    ...
    print_info("Requires Node.js. Run 'sinoclaw whatsapp' for guided setup.")
    print()
    if prompt_yes_no("Enable WhatsApp now?", True):
        save_env_value("WHATSAPP_ENABLED", "true")
        print_success("WhatsApp enabled")
        print_info("Run 'sinoclaw whatsapp' to choose your mode (separate bot number")
        print_info("or personal self-chat) and pair via QR code.")


def _setup_weixin():
    """Configure Weixin (personal WeChat) via iLink Bot API QR login."""
    from sinoclaw_cli.gateway import _setup_weixin as _gateway_setup_weixin
    _gateway_setup_weixin()


def _setup_signal():
    """Configure Signal via gateway setup."""
    from sinoclaw_cli.gateway import _setup_signal as _gateway_setup_signal
    _gateway_setup_signal()


def _setup_email():
    """Configure Email via gateway setup."""
    from sinoclaw_cli.gateway import _setup_email as _gateway_setup_email
    _gateway_setup_email()


def _setup_sms():
    """Configure SMS (Twilio) via gateway setup."""
    from sinoclaw_cli.gateway import _setup_sms as _gateway_setup_sms
    _gateway_setup_sms()


def _setup_dingtalk():
    """Configure DingTalk via gateway setup."""
    from sinoclaw_cli.gateway import _setup_dingtalk as _gateway_setup_dingtalk
    _gateway_setup_dingtalk()


def _setup_feishu():
    """Configure Feishu / Lark via gateway setup."""
    from sinoclaw_cli.gateway import _setup_feishu as _gateway_setup_feishu
    _gateway_setup_feishu()


def _setup_wecom():
    """Configure WeCom (Enterprise WeChat) via gateway setup."""
    from sinoclaw_cli.gateway import _setup_wecom as _gateway_setup_wecom
    _gateway_setup_wecom()


def _setup_wecom_callback():
    """Configure WeCom Callback (self-built app) via gateway setup."""
    from sinoclaw_cli.gateway import _setup_wecom_callback as _gw_setup
    _gw_setup()
...
def _setup_qqbot():
    ...
    print_info("Connects Sinoclaw to QQ via the Official QQ Bot API (v2).")
...
def _setup_qqbot():
    ...
    from sinoclaw_cli.gateway import _PLATFORMS
    qq_platform = next((p for p in _PLATFORMS if p["key"] == "qqbot"), None)
    if qq_platform:
        from sinoclaw_cli.gateway import _setup_standard_platform
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://sinoclaw-agent.nousresearch.com/docs/user-guide/messaging/webhooks/")
    print()
...
def setup_gateway(config: dict):
    ...
    if not selected:
        print_info("No platforms selected. Run 'sinoclaw setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                print_info("  Or run in foreground:  sinoclaw gateway")
        else:
            from sinoclaw_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   sinoclaw gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   sinoclaw gateway              # Run in foreground")
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from sinoclaw_cli.config
    ...
    if section_key == "model":
        ...
        if not has_key:
            ...
            try:
                from sinoclaw_cli.auth import get_active_provider
...
def _load_openclaw_migration_module():
    """Load the openclaw_to_sinoclaw migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_sinoclaw", _OPENCLAW_SCRIPT
    )
...
def _offer_openclaw_migration(sinoclaw_home: Path) -> bool:
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      sinoclaw setup           — full or quick (auto-detected)
      sinoclaw setup model     — just model/provider
      sinoclaw setup tts       — just text-to-speech
      sinoclaw setup terminal  — just terminal backend
      sinoclaw setup gateway   — just messaging platforms
      sinoclaw setup tools     — just tool configuration
      sinoclaw setup agent     — just agent settings
    """
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Sinoclaw Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    if is_existing:
        ...
        if choice == 0:
            # Quick setup
            _run_quick_setup(config, sinoclaw_home)
            return
        elif choice == 1:
            # Full setup — fall through to run all sections
            pass
        elif choice == 7:
            print_info("Exiting. Run 'sinoclaw setup' again when ready.")
            return
        elif 2 <= choice <= 6:
            ...
            if section:
                ...
                func(config)
                save_config(config)
                _print_setup_summary(config, sinoclaw_home)
            return
    else:
        # ── First-Time Setup ──
        print()
        ...
        migration_ran = _offer_openclaw_migration(sinoclaw_home)
        if migration_ran:
            config = load_config()
        ...
        if setup_mode == 0:
            _run_first_time_quick_setup(config, sinoclaw_home, is_existing)
            return
...
def _resolve_sinoclaw_chat_argv() -> Optional[list[str]]:
    """Resolve argv for launching ``sinoclaw chat`` in a fresh process."""
    sinoclaw_bin = shutil.which("sinoclaw")
    if sinoclaw_bin:
        return [sinoclaw_bin, "chat"]

    try:
        if importlib.util.find_spec("sinoclaw_cli") is not None:
            return [sys.executable, "-m", "sinoclaw_cli.main", "chat"]
    except Exception:
        pass
...
def _offer_launch_chat():
    ...
    chat_argv = _resolve_sinoclaw_chat_argv()
    if not chat_argv:
        print_info("Could not relaunch Sinoclaw automatically. Run 'sinoclaw chat' manually.")
        return

    os.execvp(chat_argv[0], chat_argv)


def _run_first_time_quick_setup(config: dict, sinoclaw_home, is_existing: bool):
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'sinoclaw setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    sinoclaw setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  sinoclaw setup gateway")
    print()
...
def _run_quick_setup(config: dict, sinoclaw_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'sinoclaw setup' and choose 'Full Setup' to reconfigure,")
