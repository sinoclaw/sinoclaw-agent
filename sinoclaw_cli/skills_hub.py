#!/usr/bin/env python3
"""
Skills Hub CLI — Unified interface for the Sinoclaw Skills Hub.
...
def do_search(query: str, source: str = "all", limit: int = 10,
    ...
    c.print("[dim]Use: sinoclaw skills inspect <identifier> to preview, "
            "sinoclaw skills install <identifier> to install[/]\n")
...
def do_browse(page: int = 1, page_size: int = 20, source: str = "all",
    ...
    c.print("[dim]Tip: 'sinoclaw skills search <query>' searches deeper across all registries[/]\n")
...
def do_inspect(identifier: str, console: Optional[Console] = None) -> None:
    ...
    if bundle and "SKILL.md" in bundle.files:
        ...
        c.print(Panel(preview, title="SKILL.md Preview", subtitle="sinoclaw skills install <id> to install"))
    ...
    if target == "github":
        ...
        if not auth.is_authenticated():
            c.print("[bold red]Error:[/] GitHub authentication required.\n"
                    f"Set GITHUB_TOKEN in {display_sinoclaw_home()}/.env or run 'gh auth login'.\n")
            return
    ...
    try:
        resp = httpx.post(
            ...
            json={
                ...
                "body": f"Submitting the `{skill_name}` skill via Sinoclaw Skills Hub.\n\n"
                        f"This skill was scanned by the Sinoclaw Skills Guard before submission.",
...
def skills_command(args) -> None:
    ...
    if action == "browse":
        ...
        _console.print("Usage: sinoclaw skills [browse|search|install|inspect|list|check|update|audit|uninstall|publish|snapshot|tap]\n")
        _console.print("Run 'sinoclaw skills <command> --help' for details.\n")
