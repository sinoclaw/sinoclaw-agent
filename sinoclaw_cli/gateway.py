        ...
        for scope_args in [["systemctl", "--user"], ["systemctl"]]:
            try:
                result = subprocess.run(
                    scope_args + ["list-units", "sinoclaw-gateway*",
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``sinoclaw gateway status`` from falsely counting the ``hermes`` CLI that
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    # scan (e.g. ``sinoclaw gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    exclude_pids = exclude_pids | _get_ancestor_pids()
    pids: list[int] = []
    patterns = [
        "sinoclaw_cli.main gateway",
        "sinoclaw_cli.main --profile",
        "sinoclaw_cli.main -p",
        "sinoclaw_cli/main.py gateway",
        "sinoclaw_cli/main.py --profile",
        "sinoclaw_cli/main.py -p",
        "sinoclaw gateway",
        "gateway/run.py",
    ]
    current_home = str(get_sinoclaw_home().resolve())
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        # the command doesn't advertise *some other* profile. SINOCLAW_HOME
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to Sinoclaw profiles via PID files."""
    _exclude = set(exclude_pids or set())
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "sinoclaw_cli.main"]
    if profile != "default":
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}sinoclaw gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}sinoclaw gateway restart{scope_flag}")
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    print("  This is usually a manual foreground/tmux/nohup run, so `sinoclaw gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``sinoclaw gateway status`` output so users with
...
def _gateway_list() -> None:
    ...
    """
    try:
        from sinoclaw_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
from sinoclaw_constants import is_container, is_termux, is_wsl
...
_SERVICE_BASE = "sinoclaw-gateway"
SERVICE_DESCRIPTION = "Sinoclaw Agent Gateway - Messaging Platform Integration"
...
def _profile_suffix() -> str:
    ...
    home = get_sinoclaw_home().resolve()
    default = get_default_sinoclaw_root().resolve()
    if home == default:
        return ""
...
def _profile_arg(sinoclaw_home: str | None = None) -> str:
    ...
    import re
    from sinoclaw_constants import get_default_sinoclaw_root
    home = Path(sinoclaw_home or str(get_sinoclaw_home())).resolve()
    default = get_default_sinoclaw_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
...
# Legacy service names from older Sinoclaw installs that predate the
...
def _find_legacy_sinoclaw_units() -> list[tuple[str, Path, bool]]:
    ...
    Detects unit files installed by older Sinoclaw versions that used a
...
def print_legacy_unit_warning() -> None:
    ...
    if not legacy:
        return
    print_warning("Legacy Sinoclaw gateway unit(s) detected from an older install:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
        print_info(f"    {path}  ({scope} scope)")
    print_info("  These run alongside the current sinoclaw-gateway service and")
    print_info("  cause SIGTERM flap loops — both try to use the same bot token.")
    print_info("  Remove them with:")
    print_info("    sinoclaw gateway migrate-legacy")


def remove_legacy_sinoclaw_units(
    ...
    print()
    print("Legacy Sinoclaw gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: sinoclaw gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo sinoclaw gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    sinoclaw gateway uninstall")
    print_info("    sudo sinoclaw gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            print_warning("  System service install requires sudo, so Sinoclaw can't create it from this user session.")
            if run_as_user:
                print_info(f"  After setup, run: sudo sinoclaw gateway install --system --run-as-user {run_as_user}")
            else:
                print_info("  After setup, run: sudo sinoclaw gateway install --system --run-as-user <your-user>")
            print_info("  Then start it with: sudo sinoclaw gateway start --system")
            return scope, False
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Sinoclaw often sets ``HOME`` to a profile-scoped directory, but
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.sinoclaw/sinoclaw-agent  -> /home/alice/.sinoclaw/sinoclaw-agent
...
def _sinoclaw_home_for_target_user(target_home_dir: str) -> str:
    ...
    # Profile or subdir of ~/.sinoclaw → preserve the relative structure
    try:
        relative = current_hermes.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.sinoclaw) — keep as-is
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        sinoclaw_home = _sinoclaw_home_for_target_user(home_dir)
        profile_arg = _profile_arg(sinoclaw_home)
...
User={username}
Group={group_name}
ExecStart={python_path} -m sinoclaw_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run --replace
...
"""
    ...
    sinoclaw_home = str(get_sinoclaw_home().resolve())
    profile_arg = _profile_arg(sinoclaw_home)
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Sinoclaw install")
    return True
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo sinoclaw gateway uninstall --system")
    print_info("         sinoclaw gateway install")
    print_info("         sinoclaw gateway start")
...
def systemd_install(force: bool = False, system: bool = False, run_as_user: str | None = None):
    ...
    if has_legacy_sinoclaw_units():
        print()
        print_legacy_unit_warning()
        print()
        if prompt_yes_no("Remove the legacy unit(s) before installing?", True):
            remove_legacy_sinoclaw_units(interactive=False)
            print()
    ...
    print()
    print("Next steps:")
    print(f"  {'sudo ' if system else ''}sinoclaw gateway start{scope_flag}              # Start the service")
    print(f"  {'sudo ' if system else ''}sinoclaw gateway status{scope_flag}             # Check status")
...
def _require_service_installed(action: str, system: bool = False) -> None:
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}sinoclaw gateway install{scope_flag}")
        sys.exit(1)
...
def systemd_stop(system: bool = False):
    ...
    try:
        ...
        print(
            ...
            "check `sinoclaw gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `sinoclaw gateway status` or logs for final state."
            )
            return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {'sudo ' if system else ''}sinoclaw gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}sinoclaw gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(f"  Run after the start-limit window expires: {'sudo ' if system else ''}sinoclaw gateway restart{scope_flag}")
        ...
        print(f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}sinoclaw gateway start{scope_flag}")
...
def generate_launchd_plist() -> str:
    ...
    working_dir = str(PROJECT_ROOT)
    sinoclaw_home = str(get_sinoclaw_home().resolve())
    log_dir = get_sinoclaw_home() / "logs"
    ...
    prog_args = [
        ...
        "<string>-m</string>",
        "<string>sinoclaw_cli.main</string>",
    ]
    if profile_arg:
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print("↻ Updated gateway launchd service definition to match the current Sinoclaw install")
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  sinoclaw gateway status             # Check status")
    from sinoclaw_constants import display_sinoclaw_home as _dhh
...
def launchd_stop():
    ...
    # `sinoclaw gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Sinoclaw install")
    else:
        print("⚠ Service definition is stale relative to the current Sinoclaw install")
        print("  Run: sinoclaw gateway start")
    ...
    if deep:
        log_file = get_sinoclaw_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def _guard_official_docker_root_gateway() -> None:
    ...
    if not hasattr(os, "geteuid") or os.geteuid() != 0:
        return
    if _truthy_env(os.getenv("SINOCLAW_ALLOW_ROOT_GATEWAY")):
        return
    if not _is_official_docker_checkout():
        return

    print_error(
        "Refusing to run the Sinoclaw gateway as root inside the official Docker image."
    )
    print(
        "  The image entrypoint normally drops privileges to the 'hermes' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/hermes/docker/entrypoint.sh before the Sinoclaw command."
    )
    print(
        "  Running the gateway as root can leave root-owned files in "
        "$SINOCLAW_HOME and break later non-root dashboard/gateway runs."
    )
    print("  Set SINOCLAW_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk.")
    sys.exit(1)
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False):
    ...
    # process (via ``sinoclaw gateway install`` → Scheduled Task / Startup
    ...
    # Wrong. When the user runs `sinoclaw gateway start` from a PowerShell
    ...
    # /restart) rather than through `sinoclaw gateway restart` which already
    ...
    # `sinoclaw gateway start/restart` — leaving the gateway vulnerable to
    ...
    print("┌─────────────────────────────────────────────────────────┐")
    print("│           ⚕ Sinoclaw Gateway Starting...                 │")
    print("├─────────────────────────────────────────────────────────┤")
    ...
    # SINOCLAW_GATEWAY_EXIT_DIAG env var (default: on while we're still
    ...
    def _exit_diag(tag: str, **extra: object) -> None:
        if os.environ.get("SINOCLAW_GATEWAY_EXIT_DIAG", "1") != "1":
            return
        try:
            from sinoclaw_constants import get_sinoclaw_home as _ghh
            log_dir = _ghh() / "logs"
            log_dir.mkdir(parents=True, exist_ok=True)
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and Sinoclaw will log in directly",
    ...
    },
    {
        ...
        "vars": [
            ...
            {"name": "MATTERMOST_HOME_CHANNEL", "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)", "password": False,
             "help": "Channel ID where Sinoclaw delivers cron results and notifications."},
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your Sinoclaw agent",
        ...
        "vars": [
            {"name": "EMAIL_ADDRESS", "prompt": "Email address", "password": False,
             "help": "The email address Sinoclaw will use (e.g., hermes@gmail.com)."},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. Sinoclaw connects via the BlueBubbles REST API and receives",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and Sinoclaw will connect automatically over WebSocket",
        ],
        "vars": [
...
def _all_platforms() -> list[dict]:
    ...
    ``sinoclaw setup gateway`` without needing the gateway to be running.
    ...
    # User-installed platform plugins under ~/.sinoclaw/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_sinoclaw_home() / "whatsapp" / "session" / "creds.json"
...
def _setup_whatsapp():
    ...
    from sinoclaw_cli.main import cmd_whatsapp
    import argparse
...
def _is_service_installed() -> bool:
    ...
    if supports_systemd_services():
        ...
        from sinoclaw_cli import gateway_windows
...
def _setup_weixin():
    ...
    print()
    print_info("  1. Sinoclaw will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info("  3. Sinoclaw will store the returned account_id/token in ~/.sinoclaw/.env.")
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `sinoclaw gateway setup`.")
        return
    ...
    import asyncio
    try:
        credentials = asyncio.run(qr_login(str(get_sinoclaw_home())))
    except KeyboardInterrupt:
        print()
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info("    signal-cli link -n \"SinoclawAgent\"")
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``sinoclaw_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from sinoclaw_cli import setup as _s
    return {
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.sinoclaw/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.sinoclaw/.env: {', '.join(required)}")
    else:
            ...
            if supports_systemd_services() and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: sinoclaw gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if prompt_yes_no(f"  Install the gateway as a {platform_name} service?{wsl_note} (runs in background, starts on boot)", True):
                    try:
                        ...
                        print_info("  You can try manually: sinoclaw gateway install")
                else:
                    print_info("  You can install later: sinoclaw gateway install")
                    if supports_systemd_services():
                        print_info("  Or as a boot-time service: sudo sinoclaw gateway install --system")
                    print_info("  Or run in foreground:  sinoclaw gateway run")
            elif is_wsl():
                ...
                if is_termux():
                    ...
                    print_info("  Run in foreground: sinoclaw gateway run")
                    print_info(f"  Or start it manually in the background (best effort): nohup sinoclaw gateway run >{_dhh()}/logs/gateway.log 2>&1 &")
                else:
                    print_info("  Service install not supported on this platform.")
                    print_info("  Run in foreground: sinoclaw gateway run")
    else:
        print()
        print_info("No platforms configured. Run 'sinoclaw gateway setup' when ready.")
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``sinoclaw gateway install|uninstall|start|stop|restart``
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: sinoclaw gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
            ...
            print()
            print("To run the gateway: sinoclaw gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: sinoclaw gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: sinoclaw gateway stop")
            sys.exit(1)
        if supports_systemd_services():
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: sinoclaw gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if not service_available:
            ...
            if supports_systemd_services():
                ...
                if linger_ok is not True:
                    ...
                    print()
                    print("  Then restart the gateway:")
                    print("    sinoclaw gateway restart")
                    return
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: sinoclaw gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  sinoclaw gateway install")
                else:
                    print("To install as a service:")
                    print("  sinoclaw gateway install")
                    print("  sudo sinoclaw gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  sinoclaw gateway run      # Run in foreground")
                if is_termux():
                    ...
                    print("  sinoclaw gateway install  # Install as user service")
                    print("  sudo sinoclaw gateway install --system  # Install as boot-time system service")
        ...
        if not supports_systemd_services() and not is_macos():
            ...
            return
        remove_legacy_sinoclaw_units(interactive=not yes, dry_run=dry_run)
