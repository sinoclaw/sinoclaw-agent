"""
Status command for sinoclaw CLI.

Shows the status of all Sinoclaw Agent components.
"""
...
from sinoclaw_cli.auth import AuthError, resolve_provider
from sinoclaw_cli.colors import Colors, color
from sinoclaw_cli.config import get_env_path, get_env_value, get_sinoclaw_home, load_config
from sinoclaw_cli.models import provider_label
from sinoclaw_cli.nous_subscription import get_nous_subscription_features
from sinoclaw_cli.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``sinoclaw config``'s
...
def show_status(args):
    """Show status of all Sinoclaw Agent components."""
    show_all = getattr(args, 'all', False)
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ Sinoclaw Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    nous_label = "logged in" if nous_logged_in else "not logged in (run: sinoclaw auth add nous --type oauth)"
    print(
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: sinoclaw model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: sinoclaw auth add minimax-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: sinoclaw model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from sinoclaw_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    if terminal_env == "ssh":
        ...
        sdk_label = "installed" if sdk_ok else "missing (install: pip install 'sinoclaw-agent[vercel]')"
    ...
    try:
        from sinoclaw_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
        ...
        if snapshot.has_process_service_mismatch:
            ...
            print("  Start with:   sinoclaw gateway")
    ...
    jobs_file = get_sinoclaw_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    sessions_file = get_sinoclaw_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'sinoclaw doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'sinoclaw setup' to configure", Colors.DIM))
    print()
