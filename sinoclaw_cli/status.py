...
def show_status(args):
    ...
    try:
        from sinoclaw_cli.auth import get_nous_auth_status, get_codex_auth_status, get_qwen_auth_status
    ...
    print(
        ...
        f"{'logged in' if nous_logged_in else 'not logged in (run: sinoclaw model)'}"
    )
    if nous_logged_in:
    ...
    if not terminal_env:
        ...
        # (sinoclaw status doesn't go through cli.py's config loading)
        try:
            _cfg = load_config()
    ...
    if _is_termux():
        try:
            from sinoclaw_cli.gateway import find_gateway_pids
        ...
        if is_container():
            ...
            try:
                from sinoclaw_cli.gateway import get_service_name
                _gw_svc = get_service_name()
            except Exception:
                _gw_svc = "sinoclaw-gateway"
            try:
        ...
        from sinoclaw_cli.gateway import get_launchd_label
        try:
            result = subprocess.run(
