"""Slash command definitions and autocomplete for the Sinoclaw CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.sinoclaw/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update Sinoclaw Agent to the latest version", "Info",
               gateway_only=True),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`sinoclaw_cli.plugins.PluginContext.register_command`. They behave
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``sinoclaw skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``sinoclaw skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``sinoclaw skills list`` and
...
def slack_app_manifest(request_url: str = "https://sinoclaw-agent.local/slack/commands") -> dict[str, Any]:
