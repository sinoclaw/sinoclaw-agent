"""CLI for the Sinoclaw Kanban board — ``sinoclaw kanban …`` subcommand.

Exposes the full 15-verb surface documented in the design spec
(``docs/sinoclaw-kanban-v1-spec.pdf``).  All DB work is delegated to
...
def _check_dispatcher_presence() -> tuple[bool, str]:
    ...
    - ``running=True``: a gateway is alive for this SINOCLAW_HOME and its
    ...
    Used by ``sinoclaw kanban create`` (and callers) to warn when a task
    ...
    if pid and not dispatch_on:
        return (
            ...
            "standalone daemon (`sinoclaw kanban daemon --force`)."
        )
    return (
        False,
...
def build_parser(parent_subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    ...
    kanban_parser = parent_subparsers.add_parser(
        ...
        description=(
            "Durable SQLite-backed task board shared across Sinoclaw profiles. "
            "Tasks are claimed atomically, can depend on other tasks, and "
            "are executed by a named profile in an isolated workspace. "
            "See https://sinoclaw-agent.nousresearch.com/docs/user-guide/features/kanban "
            "or docs/sinoclaw-kanban-v1-spec.pdf for the full design."
        ),
    )
    # --- global --board flag ---
    # Applies to every subcommand below. When set, scopes all reads and
    # writes to that board's DB. When omitted, resolves via the
    # SINOCLAW_KANBAN_BOARD env var, then the persisted current-board
    ...
    kanban_parser.add_argument(
        ...
        help=(
            ...
            "(set via `sinoclaw kanban boards switch <slug>` or the "
            "SINOCLAW_KANBAN_BOARD env var). Use `sinoclaw kanban boards list` "
            "to see all boards."
        ),
    )
    ...
    p_list.add_argument("--mine", action="store_true",
                        help="Filter by $SINOCLAW_PROFILE as assignee")
    ...
    p_comment.add_argument("--author", default=None,
                           help="Author name (default: $SINOCLAW_PROFILE or 'user')")
    ...
    p_daemon = sub.add_parser(
        "daemon",
        help="DEPRECATED — dispatcher now runs in the gateway. Use `sinoclaw gateway start`.",
    )
    ...
    p_asg = sub.add_parser(
        ...
        help="List known profiles + per-profile task counts "
             "(union of ~/.sinoclaw/profiles/ and current assignees on the board)",
    )
    ...
    p_specify.add_argument(
        ...
        help="Author name recorded on the audit comment "
             "(default: $SINOCLAW_PROFILE or 'specifier')",
    )
    p_specify.add_argument(
        "--json",
        action="store_true",
...
def kanban_command(args: argparse.Namespace) -> int:
    """Entry point from ``sinoclaw kanban …`` argparse dispatch.
    ...
    if not action:
        ...
        if parser is not None:
            ...
            print(
                "usage: sinoclaw kanban <action> [options]\n"
    ...
    # env-var pin for the duration of this call. Using SINOCLAW_KANBAN_BOARD
    ...
    if board_override:
        ...
        if normed != kb.DEFAULT_BOARD and not kb.board_exists(normed):
            print(
                ...
                f"Create it with `sinoclaw kanban boards create {normed}`.",
                file=sys.stderr,
            )
            return 1
        os.environ["SINOCLAW_KANBAN_BOARD"] = normed
    ...
    # SINOCLAW_HOME. Previously only `init` and `daemon` triggered
...
def _profile_author() -> str:
    ...
    for env in ("SINOCLAW_PROFILE_NAME", "SINOCLAW_PROFILE"):
...
def _dispatch_boards(args: argparse.Namespace) -> int:
    """Handle ``sinoclaw kanban boards <action>``.
    ...
    DB, so a fresh SINOCLAW_HOME that has never called ``kanban init``
...
def _cmd_boards_list(args: argparse.Namespace) -> int:
    ...
    if not boards:
        print("(no boards — create one with `sinoclaw kanban boards create <slug>`)")
        return 0
    ...
    if len(boards) > 1:
        print("Switch boards with `sinoclaw kanban boards switch <slug>`.")
    return 0
...
def _cmd_boards_create(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "switch", False):
        ...
        print(f"  Use `sinoclaw kanban boards switch {meta['slug']}` to make it current.")
    return 0
...
def _cmd_init(args: argparse.Namespace) -> int:
    ...
    if profiles:
        ...
        print("No profiles found under ~/.sinoclaw/profiles/.")
        print("Create one with `sinoclaw -p <name> setup` before assigning tasks.")
    print()
    print("Next step: start the gateway so ready tasks actually get picked up.")
    print("  sinoclaw gateway start")
    print()
    print(
...
def _cmd_assignees(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "json", False):
        ...
        return 0
    if not data:
        print("(no assignees — create a profile with `sinoclaw -p <name> setup`)")
        return 0
    # Header
...
def _cmd_show(args: argparse.Namespace) -> int:
    ...
    if task.result:
        ...
        # Surface it at top-level so a glance at ``sinoclaw kanban show <id>``
...
def _worker_run_id_for(task_id: str) -> Optional[int]:
    if os.environ.get("SINOCLAW_KANBAN_TASK") != task_id:
        return None
    raw = os.environ.get("SINOCLAW_KANBAN_RUN_ID")
    if not raw:
        return None
    try:
        return int(raw)
...
def _cmd_daemon(args: argparse.Namespace) -> int:
    ...
    if not getattr(args, "force", False):
        print(
            ...
            "\n"
            "    sinoclaw gateway start       # starts the gateway + embedded dispatcher\n"
            "\n"
        ...
        if health_state["bad_ticks"] >= HEALTH_WINDOW:
            ...
            if now - health_state["last_warn_at"] >= 300:
                print(
                    ...
                    f"`sinoclaw kanban list --status ready` / "
                    f"`sinoclaw kanban list --status blocked` for recent "
    ...
    def _ready_queue_nonempty() -> bool:
        ...
        task whose assignee maps to a real Sinoclaw profile (i.e. one the
...
def _cmd_specify(args: argparse.Namespace) -> int:
    ...
    from sinoclaw_cli import kanban_specify as spec
