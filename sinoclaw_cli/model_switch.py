...
# The previous substring check (`"sinoclaw" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``sinoclaw-brain:qwen3-14b-ctx16k`` that just
# happen to carry "sinoclaw" in their tag but are fully tool-capable.
#
...
def list_authenticated_providers(
    ...
    from sinoclaw_cli.models import OPENROUTER_MODELS, _PROVIDER_MODELS
    ...
    for pid, overlay in SINOCLAW_OVERLAYS.items():
        ...
        sinoclaw_slug = _mdev_to_sinoclaw.get(pid, pid)
        ...
        if not has_creds:
            try:
                ...
                if store and (
                    pid in providers_store or sinoclaw_slug in providers_store
                    or pid in pool_store or sinoclaw_slug in pool_store
                ):
                    has_creds = True
