...
- ``sinoclaw_cli.providers``        -- canonical provider identity + overlays
- ``sinoclaw_cli.model_normalize``  -- per-provider name formatting
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from sinoclaw_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'sinoclaw model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'sinoclaw doctor' — config issues detected:"
    ...
    sinoclaw_warn = _check_sinoclaw_model_warning(new_model)
    if sinoclaw_warn:
        warnings.append(sinoclaw_warn)
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from sinoclaw_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from sinoclaw_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    for sinoclaw_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        pconfig = PROVIDER_REGISTRY.get(sinoclaw_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (SINOCLAW_OVERLAYS) with proper auth store checking.
        ...
        if not has_creds:
            try:
                from sinoclaw_cli.auth import _load_auth_store
                store = _load_auth_store()
                if store and sinoclaw_id in store.get("credential_pool", {}):
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
    ...
    for pid, overlay in SINOCLAW_OVERLAYS.items():
        ...
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(sinoclaw_slug)
        elif overlay.extra_env_vars:
            has_creds = any(os.environ.get(ev) for ev in overlay.extra_env_vars)
        # Also check api_key_env_vars from PROVIDER_REGISTRY for api_key auth_type
        if not has_creds and overlay.auth_type == "api_key":
            for _key in (pid, sinoclaw_slug):
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or sinoclaw_slug in providers_store):
                    has_creds = True
        ...
        if not has_creds:
            try:
                ...
                pool = load_pool(sinoclaw_slug)
                if pool.has_credentials():
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", sinoclaw_slug, exc)
        ...
        # configured.
        if not has_creds and sinoclaw_slug == "anthropic":
            try:
                ...
                if (sinoclaw_creds and sinoclaw_creds.get("accessToken")) or \
        ...
        if sinoclaw_slug in {"copilot", "copilot-acp"}:
            model_ids = provider_model_ids(sinoclaw_slug)
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
            ...
            try:
                ...
                model_ids = _ids if _ids is not None else (curated.get(sinoclaw_slug, []) or curated.get(pid, []))
            except Exception:
            ...
            if sinoclaw_slug in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(sinoclaw_slug, model_ids)
        total = len(model_ids)
        top = model_ids[:max_models]

        results.append({
            "slug": sinoclaw_slug,
            "name": get_label(sinoclaw_slug),
            "is_current": sinoclaw_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "hermes",
        ...
        seen_slugs.add(sinoclaw_slug.lower())
        _record_builtin_endpoint(sinoclaw_slug)
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`sinoclaw_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from sinoclaw_cli.models import fetch_openrouter_models
