...
Usage:
    python -m sinoclaw_cli.main web          # Start on http://127.0.0.1:9119
    python -m sinoclaw_cli.main web --port 8080
"""
...
# In-browser Chat tab (/chat, /api/pty, …).  Off unless ``sinoclaw dashboard --tui``
...
# streamed to a per-action log file under ``~/.sinoclaw/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_sinoclaw_home() / "logs"

# Short ``name`` (from the URL) → absolute log file path.
_ACTION_LOG_FILES: Dict[str, str] = {
    "gateway-restart": "gateway-restart.log",
    "sinoclaw-update": "sinoclaw-update.log",
}
...
def _spawn_sinoclaw_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``sinoclaw <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``sinoclaw_cli.main`` module so the action
    ...
    cmd = [sys.executable, "-m", "sinoclaw_cli.main", *subcommand]
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "SINOCLAW_NONINTERACTIVE": "1"},
    }
...
async def restart_gateway():
    """Kick off a ``sinoclaw gateway restart`` in the background."""
    try:
        proc = _spawn_sinoclaw_action(["gateway", "restart"], "gateway-restart")
...
async def update_hermes():
    """Kick off ``sinoclaw update`` in the background."""
    try:
        proc = _spawn_sinoclaw_action(["update"], "sinoclaw-update")
    except Exception as exc:
        _log.exception("Failed to spawn sinoclaw update")
        raise HTTPException(status_code=500, detail=f"Failed to start update: {exc}")
    return {
        "ok": True,
        "pid": proc.pid,
        "name": "sinoclaw-update",
    }
...
# in sinoclaw_cli/config.py — listed here for deterministic ordering in the UI.
...
async def set_model_assignment(body: ModelAssignment):
    ...
    Writes to ``~/.sinoclaw/config.yaml`` — applies to **new** sessions only.
...
# the canonical ``sinoclaw auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    if read_sinoclaw_oauth_credentials:
        try:
            sinoclaw_creds = read_sinoclaw_oauth_credentials()
        except Exception:
            sinoclaw_creds = None
    if sinoclaw_creds and sinoclaw_creds.get("accessToken"):
        return {
            "logged_in": True,
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    ...
    },
    {
        ...
        "name": "Nous Portal",
        "flow": "device_code",
        "cli_command": "sinoclaw auth add nous",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from sinoclaw_cli import auth as hauth
        if provider_id == "nous":
...
async def list_oauth_providers():
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("sinoclaw_pkce", "claude_code", ...)
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    if provider_id in ("anthropic", "claude-code"):
        ...
        try:
            from sinoclaw_cli.auth import clear_provider_auth
...
#          → persists to ~/.sinoclaw/.anthropic_oauth.json AND credential pool
...
# Guarded so sinoclaw web still starts if anthropic_adapter is unavailable;
...
def _submit_anthropic_pkce(session_id: str, code_input: str) -> Dict[str, Any]:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "sinoclaw-dashboard/1.0",
        },
        method="POST",
    )
    try:
...
async def _start_device_code_flow(provider_id: str) -> Dict[str, Any]:
    ...
    """
    if provider_id == "nous":
        from sinoclaw_cli.auth import _request_device_code, PROVIDER_REGISTRY
        import httpx
...
def _nous_poller(session_id: str) -> None:
    ...
    from sinoclaw_cli.auth import _poll_for_token, refresh_nous_oauth_from_state
    ...
    try:
        ...
        from sinoclaw_cli.auth import persist_nous_credentials
...
async def get_logs(
    ...
    from sinoclaw_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_sinoclaw_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from sinoclaw_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_sinoclaw_home()
...
def _resolve_profile_dir(name: str) -> Path:
    ...
    from sinoclaw_cli import profiles as profiles_mod
    try:
...
def _profile_setup_command(name: str) -> str:
    ...
    return "sinoclaw setup" if name == "default" else f"{name} setup"
...
async def get_skills():
    ...
    from sinoclaw_cli.skills_config import get_disabled_skills
    config = load_config()
...
async def toggle_skill(body: SkillToggle):
    from sinoclaw_cli.skills_config import get_disabled_skills, save_disabled_skills
    config = load_config()
    disabled = get_disabled_skills(config)
    if body.enabled:
...
#
# The endpoint spawns the same ``sinoclaw --tui`` binary the CLI uses, behind
...
try:
    from sinoclaw_cli.pty_bridge import PtyBridge, PtyUnavailableError
...
def _resolve_chat_argv(
    ...
    Default: whatever ``sinoclaw --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``SINOCLAW_TUI_RESUME`` env var —
    matching what ``sinoclaw_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    `sidecar_url` (when set) is forwarded as ``SINOCLAW_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).
    """
    from sinoclaw_cli.main import PROJECT_ROOT, _make_tui_argv
    ...
    env.setdefault("SINOCLAW_TUI_DISABLE_MOUSE", "1")

    if resume:
        latest_resume, _latest_path = _session_latest_descendant(resume)
        if latest_resume:
            resume = latest_resume
        env["SINOCLAW_TUI_RESUME"] = resume

    if sidecar_url:
        env["SINOCLAW_TUI_SIDECAR_URL"] = sidecar_url
...
# SINOCLAW_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Returns a string like ``"/sinoclaw"`` (no trailing slash) or ``""`` when
...
def mount_spa(application: FastAPI):
    ...
    and the SPA's runtime ``__SINOCLAW_BASE_PATH__`` honour that prefix
    ...
    def _serve_index(prefix: str = ""):
        ...
        token_script = (
            f'<script>window.__SINOCLAW_SESSION_TOKEN__="{_SESSION_TOKEN}";'
            f"window.__SINOCLAW_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
            f'window.__SINOCLAW_BASE_PATH__="{prefix}";</script>'
        )
        if prefix:
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.sinoclaw/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.sinoclaw/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.
    """
    themes_dir = get_sinoclaw_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.sinoclaw/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    from sinoclaw_cli.plugins import get_bundled_plugins_dir
    bundled_root = get_bundled_plugins_dir()
    search_dirs = [
        (get_sinoclaw_home() / "plugins", "user"),
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    plugins_root_resolved = (get_sinoclaw_home() / "plugins").resolve()
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from sinoclaw_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from sinoclaw_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from sinoclaw_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from sinoclaw_cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"sinoclaw_dashboard_plugin_{plugin['name']}"
