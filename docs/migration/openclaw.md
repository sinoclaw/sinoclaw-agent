# Migrating from OpenClaw to Sinoclaw Agent

This guide covers how to import your OpenClaw settings, memories, skills, and API keys into Sinoclaw Agent.

## Three Ways to Migrate

### 1. Automatic (during first-time setup)

When you run `sinoclaw setup` for the first time and Sinoclaw detects `~/.openclaw`, it automatically offers to import your OpenClaw data before configuration begins. Just accept the prompt and everything is handled for you.

### 2. CLI Command (quick, scriptable)

```bash
sinoclaw claw migrate                      # Preview then migrate (always shows preview first)
sinoclaw claw migrate --dry-run            # Preview only, no changes
sinoclaw claw migrate --preset user-data   # Migrate without API keys/secrets
sinoclaw claw migrate --yes                # Skip confirmation prompt
```
...
```
> Migrate my OpenClaw setup to Sinoclaw
```
...
|------|--------|-------------|
| SOUL.md | `~/.openclaw/workspace/SOUL.md` | `~/.sinoclaw/SOUL.md` |
| Memory entries | `~/.openclaw/workspace/MEMORY.md` | `~/.sinoclaw/memories/MEMORY.md` |
| User profile | `~/.openclaw/workspace/USER.md` | `~/.sinoclaw/memories/USER.md` |
| Skills | `~/.openclaw/workspace/skills/` | `~/.sinoclaw/skills/openclaw-imports/` |
| Command allowlist | `~/.openclaw/workspace/exec_approval_patterns.yaml` | Merged into `~/.sinoclaw/config.yaml` |
| Messaging settings | `~/.openclaw/config.yaml` (TELEGRAM_ALLOWED_USERS, MESSAGING_CWD) | `~/.sinoclaw/.env` |
| TTS assets | `~/.openclaw/workspace/tts/` | `~/.sinoclaw/tts/` |
...
| Item | Source | Destination |
|------|--------|-------------|
| Telegram bot token | `openclaw.json` channels config | `~/.sinoclaw/.env` |
| OpenRouter API key | `.env`, `openclaw.json`, or `openclaw.json["env"]` | `~/.sinoclaw/.env` |
| OpenAI API key | `.env`, `openclaw.json`, or `openclaw.json["env"]` | `~/.sinoclaw/.env` |
| Anthropic API key | `.env`, `openclaw.json`, or `openclaw.json["env"]` | `~/.sinoclaw/.env` |
| ElevenLabs API key | `.env`, `openclaw.json`, or `openclaw.json["env"]` | `~/.sinoclaw/.env` |
...
- **TTS provider**: OpenClaw renamed "edge" to "microsoft" — both are recognized and mapped to Sinoclaw' "edge"
...
By default, the migration **will not overwrite** existing Sinoclaw data:

- **SOUL.md** — skipped if one already exists in `~/.sinoclaw/`
...
- **API keys** — skipped if the key is already set in `~/.sinoclaw/.env`
...
For executed migrations, the full report is saved to `~/.sinoclaw/migration/openclaw/<timestamp>/`.

## Post-Migration Notes

- **Skills require a new session** — imported skills take effect after restarting your agent or starting a new chat.
- **WhatsApp requires re-pairing** — WhatsApp uses QR-code pairing, not token-based auth. Run `sinoclaw whatsapp` to pair.
- **Archive cleanup** — after migration, you'll be offered to rename `~/.openclaw/` to `.openclaw.pre-migration/` to prevent state confusion. You can also run `sinoclaw claw cleanup` later.
...
```bash
sinoclaw claw migrate --source /path/to/.openclaw
```

### "Migration script not found"
The migration script ships with Sinoclaw Agent. If you installed via pip (not git clone), the `optional-skills/` directory may not be present. Install the skill from the Skills Hub:
```bash
sinoclaw skills install openclaw-migration
```

### Memory overflow
If your OpenClaw MEMORY.md or USER.md exceeds Sinoclaw' character limits, excess entries are exported to an overflow file in the migration report directory. You can manually review and add the most important ones.
...
The migration checks all four. If keys use `source: "file"` or `source: "exec"` SecretRefs, they can't be resolved automatically — add them via `sinoclaw config set`.
